...
| 2026-09-03 | `screenCustomer()`'s own unconditional override-wipe (found while fixing the gap above) — every call, not just a sync sweep, silently clears a compliance officer's `overridden_at`/`override_reason` with no guard, unlike the shipment-level equivalent. | **Real, confirmed gap, broader in scope than the sync-staleness issue above — not fixed this pass** | Any of `screenCustomer()`'s other call sites (customer create, a plain profile edit, the manual re-check button) already silently discards an existing override today, independent of anything sanctions-sync-related — verified live as a side effect of the fix above (the pre-existing manual re-check route flipped an overridden customer straight back to `HIT` with zero warning). The identical unconditional-clear exists in both the monolith's local-mode write and the standalone Customer Service's own `PUT /internal/customers/:id/screening`. A real fix needs to decide *when* re-screening should legitimately reset an override (e.g., the customer's own name actually changed) versus when it shouldn't (an unrelated field, like phone number, was edited) — a genuine design question, not a one-line guard, so deliberately left open rather than rushed. |
