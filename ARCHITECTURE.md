...
data loss, but flagged rather than silently chased. **3 more sites found in this same class by a
2026-09-02 audit pass — `resolveInvoiceThresholds()`/`linkedPortCodes()` in `server.js`, and
`routes/shipments.js`'s own `resolveSeaPorts()` (plus a second, independently-duplicated copy in
`routes/export.js`, found while fixing it) — were fixed the next day**, live-verified (see the
Shipment-Domain Gap & Dead-Code Audit Log below); back down to the original 11 still open. Don't
flip `mdm_source=remote` in an environment exercising Reports/Export/Command Center/the AIS
Simulator until those are closed too.
...
| 2026-09-02 | Carrier/Line Agent **Capabilities checklist** (`MdmCarrierAgentsPage.jsx`'s Capabilities tab, `carrier_agents.capabilities`) — is it ever cross-checked against a shipment before a booking is sent? | **Real, confirmed gap — data collected, never read back. FIXED 2026-09-03, per direct authorization** | Grepped every carrier-booking file (`routes/edi.js`, `routes/shipment-ops.js`, `ShipmentCarrierBookingDetailsPage.jsx`, `ShipmentCarrierBookingReviewPage.jsx`, `CarrierBookingGateModal.jsx`) for any reference to `capabilit*` — zero hits outside `routes/mdm.js`'s own CRUD. The feature's own code comment already named the intent ("used later to cross-check against a leg's own haulage/service before a booking is sent") — confirmed that check genuinely doesn't exist anywhere, not assumed from the comment. Logged as `TKT-FQFE33`, under Epic `TKT-E25769`. **Before building this, checked and confirmed with the user that the fix wouldn't collide with a separate planned Line Agent entity restructure** (targeted for the following day, per project memory) — confirmed it was safe to proceed now. **New `checkLineAgentCapabilityGaps(shipmentId)`** (`server.js`) compares what a shipment's own Merchant's Haulage legs (`road_haulage`/`rail_haulage`/`barge_haulage`, by `movement_by`) and ordered Dedicated Services (`customs_clearance`/`warehousing`/`cy_storage`/`fumigation`, by `service_type`) actually need, per side, against the capabilities of whichever real `carrier_agents` registration backs that side's assigned Line Agent party — silently skips (no false positive) when a party has no matching MDM registration to compare against, e.g. a manually-assigned agent. Deliberately **non-blocking**, matching the DG-cargo-awareness-chip's own tone rather than the credit-hold pattern's hard block — nothing in this feature's own scoping ever called for a hard gate. Surfaced two ways: a new `GET /api/shipments/:id/line-agent-capability-gaps` (pre-send awareness, `routes/shipments.js`) backing a new warning chip on `ShipmentCarrierBookingDetailsPage.jsx` (same visual language as the existing DG/reefer awareness chips), and a `capabilityGaps` field appended to the booking-request response itself (`routes/edi.js`) for defense-in-depth, mirroring the credit-hold check's own dual client+server visibility. **A real, adjacent bug fixed along the way**: `resolveCarrierAgentCandidates()`'s remote-mode branch (`server.js`) built its own return object field-by-field and had silently never included `capabilities` at all — the data was already flowing correctly through the MDM Service's own `/internal/carrier-agents/resolve` endpoint (`mapCarrierAgent()` there already parses and returns it), just dropped on the monolith side; without this the new cross-check would have silently no-opped under `mdm_source=remote`. **Verified live, thoroughly**: registered a real scratch Carrier Agent deliberately missing `road_haulage`, gave its shipment a real Merchant's Haulage Pick-up leg by Truck — the GET endpoint correctly returned exactly one gap, matching the feature's own original code-comment scenario verbatim. Confirmed the negative case (capability present → `[]`, no false positive) and re-confirmed the gap reappearing when the capability was removed again. Repeated the same live check under `mdm_source=remote` (a different scratch agent, missing `rail_haulage`) to prove the adjacent bug fix actually closed the gap — correctly detected. **Full live-browser verification** (screenshot): the new chip renders correctly on the real Carrier Booking Details page — "Export Line Agent {name} has no Road Haulage capability on file, but this shipment has a merchant's haulage by truck" — zero new console errors. A production build (`vite build`) confirmed the JSX/`api.js` changes compile cleanly. All scratch data (shipments, customers, and the `carrier_agents` registration — a resolver/lookup row, removed per this project's own standing exception for that class of verification data) deleted after use; `mdm_source` confirmed restored to `local` after a login-rate-limit hiccup briefly obscured whether an earlier restore attempt had actually landed. `tests/carrier-agents.test.js` (41), `carrier-booking.test.js` (171), `line-agent-candidates.test.js` (18), `eadapter.test.js` (47), `mdm-service-toggle.test.js` (25), plus the MDM service's own `mdm-resolvers.test.js` (15) all re-run green — 317 assertions, zero regressions. |
...
| 2026-09-02 | `mdm_source=remote` bypass sites (§8.1's own "11 secondary read sites, named, accepted gap") — is that list still complete against current code? | **Real gap — list is stale, 3 more sites found** | Grepped every `routes/*.js` + `server.js` for raw `SELECT ... FROM {carriers,vessels,port_locations,linked_ports,trade_lanes,country_trade_lanes,regions,countries,commodities,carrier_agents}` and checked each against an `isRemote()`/`mdm_source` branch. All 9 files already named in §8.1 checked out — genuinely gated or already-disclosed. Found 3 more, not on that list: (1) `resolveInvoiceThresholds()` (`server.js` ~1450, backs the Invoice Collections alert/escalation sweep AND report) reads `countries` directly, no branch at all; (2) `resolveSeaPorts()` (`routes/shipments.js` ~185, backs the **main** `GET /api/shipments` list — the single highest-traffic read in this class) reads `port_locations` directly, no branch; (3) `linkedPortCodes()` (`server.js` ~2535) reads `linked_ports` directly with no branch, and is called unconditionally from `findMatchingContractLegs()` — the shared contract-rate-matching engine behind **both** `GET /api/contracts/match` (`routes/contracts.js:354`, not previously named) and `routes/allocations.js`'s match route (already named, but only for its own direct caller — this shared-helper path wasn't accounted for). `mdm_source` is `'local'` everywhere in this dev environment (confirmed live via `GET /api/settings`), so none of this is live-wrong today — same "cosmetic staleness, not data loss, until someone flips the toggle" character as the original 11. **Fixed 2026-09-03**, per direct authorization: `resolveInvoiceThresholds()` and `linkedPortCodes()` (`server.js`) and `resolveSeaPorts()` (`routes/shipments.js`) all gained an `isRemote()` branch calling `callMdmService()`. `linkedPortCodes` reused the MDM Service's already-existing per-code `/internal/port-locations/:code/links` endpoint unchanged; `resolveInvoiceThresholds`/`resolveSeaPorts` needed the MDM Service extended with a new `ids=` exact-set filter on `/internal/port-locations` and `/internal/countries` (mirroring the Customer Service's own already-established `ids=` precedent) — a bare-array bulk lookup, not the paginated browse shape, since a substring `search=` risked false-positive-matching an unrelated country's name (e.g. "US" inside "Mauritius") and an N+1 per-port-code loop would have been worse for `resolveSeaPorts`'s main-shipments-list hot path than the bug it was fixing. **A second duplicate of `resolveSeaPorts` was found and fixed along the way**: `routes/export.js` has its own independently-duplicated copy (explicitly commented as such, feeding the CSV/XLSX exports) with the identical bug — same fix applied there too, the same "same bug independently duplicated in multiple files" pattern the GP/margin bug already taught this audit to check for. **Verified live, all 3 functions, all 3 states** (local / remote+service-up / remote+service-down), using genuinely distinct test data (not a coincidental-default false-positive): `resolveSeaPorts` — a real shipment with actual SEA legs correctly showed real port names in local and remote+up modes, and cleanly degraded to empty names (not a crash, not silently-still-local) when the service was stopped mid-remote-mode; `linkedPortCodes` — a real linked-port pair (NLRTM↔NLAMS) resolved identically in local/remote+up, `[]` when down; `resolveInvoiceThresholds` — explicitly set non-default threshold values (13/21 days) directly on the remote MDM service's own country record and confirmed they round-tripped back exactly, proving a genuine remote fetch rather than a coincidental default match. Test data and temporary verification routes all removed after use. `tests/mdm-service-toggle.test.js` (25), `shipment-crud.test.js` (35), `export.test.js` (34), `export-fields.test.js` (19), `reports.test.js` (30), `pagination-standardization.test.js` (24), plus the MDM service's own `mdm-crud.test.js` (25) and `mdm-resolvers.test.js` (15) all re-run green — 207 assertions, zero regressions. §8.1's own count and warning updated in the same pass that found the gap (see above) — this entry records the fix landing. |
...
| 2026-09-02 | Freight Audit & Payment's BUY-side recovery path (found while fixing the bug above) — is there any way to correct/reverse an already-*posted* BUY cost line's actual amount, the way the SELL-side has Invoice Reversal (v0.53.0)? | **Real, confirmed gap — no BUY-side equivalent existed. FIXED 2026-09-03, per direct authorization** | Grepped `routes/*.js` for any BUY-scoped reversal/adjustment route — `POST .../documents/:docId/reverse` (v0.53.0) is SELL-only, built from `generateInvoices()`'s own line set, structurally inapplicable to a BUY cost line with no invoice document behind it. The only lever on a posted BUY line was manually inserting a new, unlinked cost line and trusting the person doing it to size it correctly — no system-enforced link between "this adjusts that", no running total shown anywhere before the two lines are added into a report. **Scoping, clarified directly with the user rather than assumed**: the user's own first answer ("also consider deletion of lines, and invoices being cancelled/reversed — we trigger the credit note in that case") introduced two more dimensions than the initial 3-option question covered, so a targeted follow-up resolved it rather than guessing — deletion of a posted line should route through the same new Adjust action (not a separate delete-reversal flow), and the "credit note" mention was purely context pointing at the *existing* SELL-side v0.53.0 Invoice Reversal flow as precedent, not a request for new SELL-side work. **New `adjusts_cost_line_id` column** on `shipment_cost_lines` (`lib/schema.js`, both the `CREATE TABLE` and a guarded `ADD COLUMN IF NOT EXISTS` migration) links a new posted adjusting line back to the original it corrects — plain TEXT, no FK, mirroring `shipment_documents.related_doc_id`'s own established idiom for this kind of soft cross-reference. New **`POST /api/shipments/:shipmentId/cost-lines/:id/adjust`** (`routes/shipment-ops.js`, same `admin`/`operator` gate as every other cost-line write) rejects a non-BUY line (409 — "a confirmed SELL invoice has its own Invoice Reversal action instead") and a non-posted line (409 — "edit it directly instead"), then inserts a new line already `status:'posted'` with `actual_amount`/`actual_exchange_rate`/`actualized_at`/`actualized_by`/`posted_at`/`posted_by` all populated to match, `source:'adjustment'`, linked via `adjusts_cost_line_id`. **The entered amount is explicitly the DELTA, never a new total** — both the route's own comment and the frontend modal's copy say so directly — since the original posted line is never edited, only offset; a "corrected total" model would have required editing a locked line, defeating the point. The existing `DELETE .../cost-lines/:id` route's 409 message was updated to point at Adjust instead of the old generic "add a new adjusting line" wording, per the clarified scope. **Frontend**: `CostLineRow` (`ShipmentDetailPage.jsx`) gained an `'adjustment'` source badge (amber, matching the existing `'reversal'` badge's severity tier) and a new "Adjust" `ActionMenu` entry gated on `l.status==='posted' && l.type==='BUY'`; new `CostLineAdjustModal` mirrors `CostLineActualizeModal`'s exact shape (a `Modal`, `useSaving()`, disabled-until-valid Save) but asks for the delta amount plus an optional reason, with explanatory copy stating plainly it's a difference, not a new total. New `api.costLines.adjust(shipmentId, lineId, {amount, note})` follows the sibling `actualize`/`post` methods' exact naming convention. Wired into `ShipmentAccountingCostsPage.jsx` (Cost Entry) only — confirmed `ShipmentAccountingInvoicesPage.jsx` (Invoice Entry) only ever filters/renders `type==='SELL'` lines, so a BUY-only action has nothing to reach there. **Verified live end-to-end**: direct API pass through every guard (accrued line → 409; posted SELL line → 409; `amount:0` → 400) plus the happy path twice — a +$150 debit and a -$75 credit-back, both correctly linked via `adjustsCostLineId` to the original, both `type:'BUY'`/`status:'posted'`/`source:'adjustment'` with `actualAmountUsd`/`varianceUsd` populated correctly. Full browser verification via CDP (JWT + license-acceptance pre-seeded via `evaluateOnNewDocument`, no form-driving): opened the real Cost Entry page, used the actual "±Adjust" menu item, filled and submitted the real modal (amount 200, a reason), confirmed the toast, the new 4th line with its "Adjustment" badge, and the total correctly recomputing to $1,275.00 (1000+150-75+200) — screenshotted at each step. `npx vite build` clean. `tests/cost-lines-lifecycle.test.js` (39), `tests/invoice-reversal.test.js` (20), `tests/multi-entity-accounting.test.js` (29) all re-run green — 88 assertions, zero regressions. This dev environment's own process hygiene needed a separate cleanup mid-pass: several past sessions' `npm run dev` launches had left 4 orphaned Vite instances and 7 orphaned microservice processes still bound to their ports — all identified by command line, killed, and replaced with one clean, single-generation stack before verification. |
| 2026-09-03 | **CRITICAL** — office-based access control (`canEditOfficeSide`, `applyShipmentAccessFilter`, `routes/finance.js`'s `callerEntityScope`) all read the client-supplied `x-office-id` header to decide office scope. Does any of them verify the caller is actually assigned to that office via `user_offices`, or is the header trusted at face value? | **Real, live-confirmed authorization bypass in 2 of 3 — FIXED same day. Third already correct.** | `canEditOfficeSide` and `routes/finance.js`'s `callerEntityScope` both read `x-office-id` and used it directly with **zero cross-check** against `user_offices` — any authenticated non-admin/operator user could set this header to *any* office id in the system (trivially discoverable via the already-authenticated `GET /api/offices`) and be treated as if genuinely assigned to it. **`applyShipmentAccessFilter` was initially, incorrectly, flagged as "very likely also affected" from a partial read** — a fuller read showed it already does the correct `SELECT id FROM user_offices WHERE user_id=$1 AND office_id=$2` check inline, returning `[]` if not assigned; this was corrected before reporting further, and its already-proven-correct pattern is what the fix below is built from. **Verified live, unambiguously, before any fix**: created a scratch `occ_bk` user, `allOffices:false`, assigned to exactly one Export office, zero assignment to any Import office. With no `x-office-id`: correctly 403. With their own Export office's id on an Import-gated action: correctly 403 (wrong department). With a *different* Import office's id — one they had never been assigned to — the exact same write **succeeded (201 Created)**: a genuine, unauthenticated-boundary-adjacent write-authorization bypass. **Fixed per direct authorization** ("fix now, all 3 sites" — 2 of the 3 turned out to need it): new shared `resolveActiveOffice(req)` (`server.js`) factors out `applyShipmentAccessFilter`'s own already-correct validation — looks up the office, and unless the caller is admin/operator/`allOffices` (checked here too, defense-in-depth), confirms a real `user_offices` row exists before returning it; returns `null` otherwise. `canEditOfficeSide` and `callerEntityScope` (exposed via `ctx`) now call this instead of trusting the header directly; `applyShipmentAccessFilter` itself was left untouched since it was never broken. **Re-verified live, precisely**: the exact same exploit attempt now correctly returns 403; the identical action succeeds (201) once the user is genuinely assigned to that office; for `callerEntityScope` specifically, used two real offices with distinct `branch_id`s and a temporary debug route to confirm the exact returned branch `Set` — own office correctly returns `[ownBranchId]`, the exploit attempt correctly returns `[]` (never leaking the unauthorized branch id), no-header behavior unchanged. `tests/carrier-agents.test.js` (41), `eadapter.test.js` (47), `line-agent-candidates.test.js` (18), `multi-entity-accounting.test.js` (29), `nvocc-carrier-identity.test.js` (21), `office-mail.test.js` (31), `quoting-rfq.test.js` (39), `shipment-crud.test.js` (35) all re-run green — 261 assertions across every test file touching office-scoped logic, zero regressions. |
| 2026-09-03 | `routes/auth.js` — `PATCH /api/users/:id`'s role validation vs. `POST /api/users`'s. Does an admin editing a user's roles get the same "at least one role required" guard the create route already has? | **Real bug, confirmed live — FIXED same day** | `POST /api/users` correctly checks `!roles.length \|\| !roles.every(...)`. `PATCH /api/users/:id` only checked `!newRoles.every(...)` — since `[].every(...)` is vacuously `true`, and `roles:[]` is a truthy array that bypasses the route's own `roles \|\| parseUserRoles(existing)` fallback (only empty arrays are falsy in JS, not empty-array-*containing* variables — `[]` itself is truthy), a caller could silently leave a user with a stored `roles:'[]'` forever. `parseUserRoles()` only falls back to `[role]` when the DB column itself is empty/null — not when it's the literal string `'[]'` — so this genuinely persists, not just a one-request blip. **Not privilege escalation** — the single legacy `role` column still correctly fell back to `'viewer'` via `primaryRoleSV([])`'s own `|| 'viewer'` — but every `roles.includes(...)` check elsewhere in the app would see an empty array, likely locking the user out of far more than even a plain viewer normally gets, with zero indication of why. **Verified live**: `POST` with `roles:[]` correctly rejected (400); `PATCH` with the same payload against a real scratch user returned 200 and the user's own roles were confirmed empty (`[]`) afterward. **Fixed**: added the same `!newRoles.length` guard to `PATCH`, mirroring `POST`'s existing check exactly. Re-verified live: `PATCH` with `roles:[]` now correctly rejects with 400, user's roles unaffected. `tests/auth-admin.test.js` (65 assertions) re-run green, zero regressions. |
| 2026-09-03 | Compliance screening (`routes/sanctions.js`) — after any sanctions list update (OFAC sync, CSL sync, manual CSV import), does the customer-level `customer_screenings` table (a customer's own persisted screening result, shown on their profile) get refreshed the way shipment-level screening does via `rescreenActiveShipments()`? | **Real, confirmed gap — customer-level screening can go stale after a sync, but the primary shipment-blocking mechanism is unaffected. FIXED same day** | Every sync/import path calls `rescreenActiveShipments()`, which correctly re-screens every active shipment against the freshly-reloaded `sanctionsMap` — confirmed sound. But `customer_screenings` is written ONLY by `screenCustomer(customerId)` (`routes/customers.js:74`), and every one of its call sites is single-customer, on-demand (create, update, a manual "re-check" button) — grepped every caller app-wide, none are inside `rescreenActiveShipments()` or any sync-completion path. So a customer's own persisted screening result can sit stale (still `CLEAR`) after a real sanctions update, until that specific customer is individually touched again. **Severity is narrower than it first looks**: `screenShipmentById`'s own hit logic checks `customer_screenings` only as a *fallback*, reached solely when the shipment's own direct denormalized-name match against the fresh `sanctionsMap` fails first (that direct check exists specifically to catch a customer *rename* the shipment's stored copy hasn't seen yet — a different scenario from "the sanctions list changed"). So a freshly-sanctioned party is still correctly caught on a shipment via the primary direct-name path regardless of `customer_screenings`' own staleness; the real exposure is (a) the customer's own profile page showing a stale "Clear" badge that compliance staff might look at directly, and (b) the narrower compound case where a stale shipment-name-copy AND a stale sync coincide. **Fixed per direct authorization** ("full sweep, fix now"): new `rescreenAllCustomers()` (`server.js`), mirroring `rescreenActiveShipments()`'s exact shape — customer-source-aware id listing (local: direct query; remote: pages through the Customer Service), skips any customer whose `customer_screenings` already shows `CLEAR` + a real `overridden_at`, calls `screenCustomer()` for the rest. Wired into all 3 sync-completion paths (`syncOfacSdn`, `syncConsolidatedScreeningList`, `routes/sanctions.js`'s `import-csv`, both local/remote branches — 6 call sites total) alongside the existing `rescreenActiveShipments()` calls. **A more serious catch made while building this, before it shipped**: `screenCustomer()` itself (previously living in `routes/customers.js`) unconditionally clears `overridden_at`/`override_reason` on *every* call with no guard at all — unlike the shipment-level equivalent, which already respects an existing override. Naively wiring the new sweep straight into `screenCustomer()` would have made this MUCH worse: every customer override in the system would have been silently destroyed on every *scheduled* sanctions sync, not just an occasional manual edit. Confirmed the identical bug is independently replicated in the standalone Customer Service's own `PUT /internal/customers/:id/screening` (`services/customers/server.js:464`) too — a faithful-port artifact, not new. **Scoped the fix correctly**: `rescreenAllCustomers()` checks for an existing override itself, *before* ever calling `screenCustomer()`, exactly mirroring the already-correct shipment-level pattern — `screenCustomer()`'s own broader always-wipes behavior (which still affects a routine customer edit today, unrelated to this sweep) was deliberately left untouched and is logged as its own separate, broader gap rather than folded into this fix. `screenCustomer()`/`rescreenShipmentsForCustomer()` moved from `routes/customers.js` into `server.js` (alongside `screenShipmentById`, for the same reason it already lives there — reachable from the sync functions) and exposed via `ctx`; every existing call site in `routes/customers.js` is unaffected. **Verified live**: created a customer matching a real, already-synced sanctioned entity (screened HIT correctly, confirming the move preserved behavior identically), applied a compliance override (CLEAR + `overriddenAt`), called the sweep directly (safe — it only re-screens against the already-loaded `sanctionsMap`, touches no sanctions data, so no live sync/import was triggered, respecting the same standing project rule) — the overridden customer correctly stayed `CLEAR` (proof the guard worked: an unguarded re-screen against this *real* match would have flipped it back to `HIT`, exactly as the pre-existing manual re-check route still does), while a second, plain non-sanctioned customer was still correctly processed by the same sweep. `tests/customer-compliance-screening.test.js` (25), `customer-hierarchy.test.js` (17), `customer-contacts-roles.test.js` (31), `customer-credit-control.test.js` (83), `customer-service-toggle.test.js` (42), `screening-service-toggle.test.js` (14) all re-run green — 212 assertions, zero regressions. |
| 2026-09-03 | `screenCustomer()`'s own unconditional override-wipe (found while fixing the gap above) — every call, not just a sync sweep, silently clears a compliance officer's `overridden_at`/`override_reason` with no guard, unlike the shipment-level equivalent. | **Real, confirmed gap — FIXED same day, per direct authorization** | Any of `screenCustomer()`'s other call sites (customer create, a plain profile edit, the manual re-check button) already silently discarded an existing override, independent of anything sanctions-sync-related — verified live as a side effect of the earlier fix (the pre-existing manual re-check route flipped an overridden customer straight back to `HIT` with zero warning). The identical unconditional-clear existed in both the monolith's local-mode write and the standalone Customer Service's own `PUT /internal/customers/:id/screening`. **Design decision**: preserve an override unless the actual match basis has genuinely changed. An override route only ever sets `result`/`overridden_at`/`override_reason` — it never touches `hits` — so the stored `hits` stays a frozen snapshot of exactly what was overridden. `screenCustomer()` now compares its freshly-computed `hits` against that stored snapshot: identical → nothing about the real basis changed (same name, same sanctions list), preserve the override; different → the name now matches something else (or nothing), or the list itself moved, so re-evaluate fresh, override included. Applied uniformly across every call site, deliberately including the manual re-check button — even an explicit recheck has no real justification for discarding a still-valid judgment call when nothing it was judging changed. Fixed in both `server.js`'s `screenCustomer()` and the Customer Service's own mirrored `PUT /internal/customers/:id/screening` (`services/customers/server.js`), keeping both sides of the extraction cut consistent. **Verified live, both modes, four scenarios**: (1) manual re-check with nothing changed → override correctly preserved (previously wiped); (2) an unrelated field edit (city) → preserved; (3) the customer's name changed to something matching no sanctioned entity → override correctly cleared, result naturally `CLEAR`; (4) the name changed to a *different* real sanctioned entity → correctly flips to `HIT`, proving the override never masks a genuinely new, real match. All four repeated in `customer_source=remote` against the live Customer Service with identical results. `tests/customer-compliance-screening.test.js` (25 — including its own customer-rename-causes-a-genuine-HIT case, confirming the fix doesn't over-preserve), `customer-hierarchy.test.js` (17), `customer-contacts-roles.test.js` (31), `customer-credit-control.test.js` (83), `customer-service-toggle.test.js` (42), `screening-service-toggle.test.js` (14), plus the Customer Service's own `customers-crud.test.js` (33) and `customers-screening.test.js` (16) all re-run green — 261 assertions, zero regressions. |
| 2026-09-03 | Company-wide GP/margin reporting (`GET /api/margin/summary`, the Dashboard XLSX export, the GP-by-Trade-Area report) — once a `shipment_cost_lines` row is actualized/posted with a real, verified amount (via the freight-audit reconciliation just fixed above, or a manual correction), does every aggregate GP figure actually reflect it? | **Real bug, high severity, widespread — FIXED same day** | The per-shipment GP page (`GpBreakdownPanel.jsx`, also reused by the Reports drilldown) has always correctly computed `actualAmountUsd ?? amountUsd` — but every SERVER-SIDE aggregate independently reimplemented `amount * exchange_rate` with no actual-preference at all, in **4 separate places**: `routes/finance.js`'s `/api/margin/summary` `aggregate()` (backing `overall`/`byCarrier`/`byLane`/`byCustomer`/`byEntity` all at once — the Dashboard's own Margin tab), `routes/export.js`'s `queryShipmentRows()` (raw SQL `SUM(amount*exchange_rate)`, feeding the Shipments-list CSV's `marginBuyUsd`/`marginSellUsd` columns), `routes/export.js`'s `buildMarginSummary()` (the XLSX dashboard export's Summary/By-Carrier/By-Lane sheets), and `routes/reports.js`'s GP-by-Trade-Area aggregate tiles (both the current-period and previous-period-for-delta computations) — meaning an actualized/posted cost line's real cost silently never reached ANY company-wide GP figure, forever, only the per-shipment/drilldown view. **Verified live, dramatic case**: a $1,000 accrued BUY line actualized to a real $5,000 — `margin/summary` kept reporting `totalBuyUsd:1000, grossProfitUsd:1000 (50% margin)` when the real figure was `totalBuyUsd:5000, grossProfitUsd:-3000` — a healthy-looking profit hiding an actual loss. **Fixed per direct authorization** (chose "shared helper + fix all 4" over ad-hoc inline fixes, specifically to prevent a 5th report reintroducing this): new `costLineEffectiveUsd(rawRow)` in `lib/mappers.js` (exported via `ctx` the same way `roundCents` already is) — `actual_amount * (actual_exchange_rate ?? exchange_rate)` once set, else the plain accrual `amount * exchange_rate`. Used directly in `finance.js` and `export.js`'s JS-side `buildMarginSummary()`; `export.js`'s SQL-level `queryShipmentRows()` gets the identical rule expressed as a `CASE` SQL constant (`COST_LINE_EFFECTIVE_USD_SQL`, clearly cross-referenced in a comment, since SQL can't call the JS helper) since that site aggregates across shipments in the database, not in JS; `reports.js` already runs rows through `mapCostLine` first, so it uses the equivalent already-mapped form (`actualAmountUsd ?? amountUsd`) on data already in hand rather than passing a camelCase-mapped row into a helper expecting raw snake_case columns, which would have silently produced `NaN`. Re-verified live after the fix: all 4 surfaces (JSON margin summary, CSV export, XLSX export sanity check, GP-by-Trade-Area) correctly show the real `$5,000`/`-$3,000`. `tests/export-fields.test.js` (19), `tests/export.test.js` (34), `tests/reports.test.js` (30), `tests/customer-hierarchy.test.js` (17), `tests/multi-entity-accounting.test.js` (29) all re-run green — 129 assertions, zero regressions. **Deliberately NOT touched**: `docAmountUsd()` (`server.js`, backs AR aging/dunning/billing-performance) has the same-looking `amount*exchange_rate` pattern but serves a different purpose — it computes what a specific, already-generated invoice document was actually billed for (frozen at generation time via `source_cost_line_ids`), where using the plain `amount` may well be correct rather than a bug; flagged as a separate, lower-confidence open question rather than folded into this fix. **Follow-up same day**: a full re-grep across every route file for the raw pattern (prompted by how many independent duplicates the first pass already found) turned up a genuine **5th occurrence**, missed in the original scan — `routes/shipments.js`'s `GET /api/shipments` itself, feeding `marginBuyUsd`/`marginSellUsd` on the live Shipments list page (arguably the single most-visible surface of all 5, seen on every normal page load, not just an exported report). Fixed the same way; `COST_LINE_EFFECTIVE_USD_SQL` was also promoted from a locally-defined constant inside `export.js` to a proper shared export off `lib/mappers.js` (alongside the JS `costLineEffectiveUsd`) so the SQL and JS forms of the same rule both live in one place — `export.js` now imports it via `ctx` rather than redefining it, closing the smaller duplication-within-the-fix-itself gap too. Re-verified live (an actualized $7,500 BUY line correctly shows `marginBuyUsd:7500` on the shipments list, not the stale $1,000 estimate); `tests/shipment-crud.test.js` (35), `tests/export.test.js` (34), `tests/export-fields.test.js` (19), `tests/pagination-standardization.test.js` (24) all re-run green — 112 more assertions, zero regressions. Two full route-file sweeps now done (grep for the exact pattern across all 34 files); no 6th occurrence found. |
| 2026-09-03 | `computeArExposure()`'s `committedExposure` (`server.js`, feeds the over-limit credit gate `findOverLimitBlock`) — same `amount*exchange_rate` question as the GP bug, but for still-uninvoiced SELL lines. Since `docTotal` was confirmed correct for *already-invoiced* lines (frozen at billing time), does the *not-yet-invoiced* side have the same reasoning, or the opposite problem? | **Real bug, high severity — understated credit exposure. FIXED same day** | Unlike an already-generated invoice (correctly frozen), a not-yet-invoiced SELL line has no reason to stay pinned to a stale accrual — if it's since been actualized to a more accurate real figure, that's what's actually about to be billed, and the over-limit gate should see it. The SQL only selected `id, amount, exchange_rate` (no `actual_amount`/`actual_exchange_rate` at all) and summed the raw estimate unconditionally. **Verified live**: a SELL line accrued at $500 actualized to a real $9,000 — `GET /api/customers/:id/credit-status`'s `committedExposure` stayed at $500, an $8,500 understatement that could let a customer's real exposure silently exceed their credit limit with the system's own over-limit block never triggering (a genuine credit-risk gap, not just a reporting one). **Fixed**: SELECT now includes both actual columns; the reduce uses the same shared `costLineEffectiveUsd()` helper from the GP fix above. Re-verified live: `committedExposure` now correctly shows $9,000. `tests/customer-credit-control.test.js` re-run — 3 pre-existing failures (unrelated: `api_eadapter_enabled` had drifted to `false` in this dev DB, a known environment quirk per v0.79.0's own changelog) cleared once that toggle was restored to its documented default; 83/83 green afterward, zero regressions from this fix. |
| 2026-09-03 | `docAmountUsd()`/`computeArExposure()` (`server.js`, AR aging/dunning/billing-performance) — flagged above as a separate, lower-confidence question: does it have the same actual-vs-accrual bug just fixed in GP/margin reporting? | **No gap — the plain `amount` field is correct here, not a bug** | Confirmed `generateInvoices()` (client-side, `src/utils/invoiceGenerator.js`) never posts or otherwise locks the underlying SELL cost lines when a FR01/FR02 is generated and confirmed — only the fully separate, explicit manual Actualize/Post action does, with no link enforced between "included in a confirmed invoice" and "locked." So a SELL line's `amount` at the moment `source_cost_line_ids` was captured IS literally what got printed on the already-sent invoice PDF — using a later `actual_amount` correction here would be wrong, not right, since AR should reflect what the customer was actually billed and owes, not a cost-line-level edit that was never communicated to them via a new invoice or the existing Invoice Reversal/Credit-Debit-Note mechanism (v0.53.0). Genuinely a different question from the GP/margin case (which needed the *true cost*, not the *original bill*) — confirmed clean by design, no code change. |
| 2026-09-02 | Shipment edit-lock 30-minute auto-expiry (v0.88.0) — that release's own changelog disclosed "the 30-minute real-clock expiry itself isn't exercised automatically... the same accepted gap this codebase already has for other short-of-an-hour time-based rules with no backdating endpoint." Is a stale lock (crashed tab) actually self-healing, or could it block a legitimate second editor indefinitely with no sweep to clear it? | **No gap — correctly self-healing by design, no sweep needed** | `POST /api/shipments/:id/edit-lock` (`routes/shipments.js:594`) computes `expired = existing && new Date(existing.expires_at) < now` fresh on every single acquire attempt, by anyone — not just the original holder's own renewal. So the very next person to open the shipment after the 30-minute mark correctly sees `expired:true` and takes over the lock immediately, with zero dependency on a background job or the original (possibly crashed) holder ever calling `DELETE`. Genuinely well-designed: the expiry only ever needs to be checked at the one moment it matters (someone else trying to get in), so a lazy per-request check is sufficient and there's nothing to actually "sweep." Also en route: the two related party-role/service-vendor checks below closed the same session's earlier thread on whether generic infrastructure correctly covers roles/features added after it shipped — `screenShipmentById`'s party loop (`SELECT role, customer_name, customer_id FROM shipment_parties`, no role filter) and its `maybeRescreen` trigger sites (`routes/shipments.js` POST/PUT/DELETE on `/parties`) are both fully generic, so `NVOCC`/`Co-Loading NVOCC` (added v0.71.0/v0.87.0, both after the v0.58.0 screening sweep) are automatically covered with zero special-casing — confirmed via direct read, no live repro needed given the query has no role predicate at all. |
| 2026-09-02 | Demurrage/Detention free-time window anchors — `CLAUDE.md`'s own v0.30.0 changelog entry self-flagged an `[ASSUMPTION]` that origin free time might be wrongly anchored on `Gate In → Sailed` instead of the "classical trade definition" `Empty Pickup → Gate In`, "flagged for correction if wrong." Is that still open? | **No gap — already resolved, historical note just never got a follow-up mention** | `routes/shipments.js`'s current `deriveFreeTime()`/`freeTimeWindow()` (lines ~125-158) computes FOUR independent windows, not the original two: origin/dest **Demurrage** (`Gate In→Sailed` / `Discharged→Gate Out` — genuinely a different real-world charge: how long the container dwells in the terminal) and origin/dest **Detention** (`Empty Pickup→Gate In` / `Gate Out→Empty Return` — how long the carrier's equipment is held outside the terminal), each backed by its own `origin_detention_free_days`/`dest_detention_free_days` columns. The code's own comment confirms this directly: "The original v0.30.0 model only ever computed the demurrage pair under a generic 'free time' name (self-flagged as an assumption needing review) — this now computes both, independently." So the "classical trade definition" the v0.30.0 note worried about IS used today, just for the correctly-separated Detention metric rather than by changing what Demurrage means — both are legitimate, both are now tracked. `CLAUDE.md`'s v0.30.0 entry itself is historical/append-only by this project's own standing convention and wasn't edited; this audit-log row is the intended place to record that the concern is closed. |
...
| 2026-09-03 | CRM/pre-sales pipeline (`routes/opportunities.js`, `routes/quotes.js`) — with the shipment-domain audit's 4 items now closed, resuming the broader audit here as the next unexamined area. Does `POST /api/quotes/:id/convert` (creates a real shipment) carry the same downstream safety checks the direct `POST /api/shipments` route already has, or does a second creation path mean a second, silently-diverged set of behavior? | **Two real, confirmed gaps, one of them compliance-relevant — FIXED same day** | `routes/opportunities.js` itself checked out clean — it only ever creates a Draft *quote*, never a shipment, so none of `POST /api/shipments`'s own shipment-creation side effects apply to it. The real gap is one level downstream, in `routes/quotes.js`'s own `POST /api/quotes/:id/convert`: it already correctly calls `screenShipmentById()` (sanctions) and `maybeAssignLineAgents()`, matching the direct route — but it never computed the `creditWarning` object the direct route added at v0.73.1 (a quote's own generic `customer_id` lands on the new shipment's Shipper slot, so a customer already on credit hold produces zero warning via this path, when the identical scenario through the New Shipment form correctly warns). **Verified live via a direct A/B**: a real held scratch customer (`credit_hold:true`) as Shipper via a direct `POST /api/shipments` correctly returned `creditWarning.onHold`; the identical customer converted through a Quote returned `creditWarning: undefined` — confirmed reproducible, not theoretical. **A second, more serious gap found while fixing the first**: even the sanctions `screening` field the convert route *already* computed and returned was never read by the frontend at all — `QuotesPage.jsx`'s `convert()` handler only ever showed a plain "Converted to shipment" success toast and navigated away, silently dropping any `HIT` result; `onShipmentCreated` (its only other post-conversion action) just pushes the shipment into local state, no toast logic. This is arguably the more significant of the two, since it's a compliance-screening blind spot, not just a soft credit warning, and predates this pass entirely (the `screening` field has existed on this response since the Quoting feature itself, v0.70.0). **Fixed**: `routes/quotes.js`'s convert route now computes the identical single-slot `heldParties` check (only Shipper — a quote has no Consignee/Principal equivalent to check, unlike the direct route's 3-field loop) and returns `creditWarning` in its response, requiring the already-`ctx`-exposed `getCustomerRow` newly destructured into this file. `QuotesPage.jsx`'s `convert()` now runs both the screening-HIT and credit-hold toasts, copied verbatim from `App.jsx`'s own already-shipped, already-tested message templates for the direct-creation path — genuine parity, not a new design. **Verified live end-to-end, both the API and the real UI**: the same held customer converted through a fresh quote now correctly returns `creditWarning.onHold`; a quote with no real customer record (the established unresolved-customer pattern) correctly returns `creditWarning: null`, no false positive; and a full CDP click-through (open the real Quotes page, open an Accepted quote, click "Convert to Shipment") shows both the success toast and the amber credit-hold warning exactly as the direct New Shipment form already does, screenshotted. `tests/quoting-rfq.test.js` (39) and `tests/customer-credit-control.test.js` (83) re-run green — 122 assertions, zero regressions. Clean build. |
| 2026-09-03 | `routes/offices.js`'s `DELETE /api/offices/:id` guard — it checks `shipments` (emo/imo/controlling) and `carrier_eadapter_configs` before allowing a delete. Is that the complete list of everything that can reference an office, or has the guard drifted as new office-referencing features shipped? | **Two real gaps, one of them a genuine silent-data-corruption bug matching the earlier `shipment_cost_lines` FK class — both FIXED same day** | Grepped `lib/schema.js` for every `office_id` column app-wide (7 total) and checked each against the guard: `user_offices`/`office_mail_settings` correctly `ON DELETE CASCADE` (no guard needed — deleting the office correctly takes its own config with it); `shipments.emo/imo/controlling_office_id` and `carrier_eadapter_configs.office_id` are exactly what the existing guard already checks. Two were not: **(1)** `document_templates.office_id` (v0.90.0) has a real FK (default RESTRICT) but no app-level guard — **verified live**: deleting a referenced office correctly failed at the DB level, but surfaced as a raw, unhelpful `{"error":"Internal server error"}` 500 (`wrapAsyncHandler`'s global error middleware catching it, confirmed via the server log showing `constraint: 'document_templates_office_id_fkey'`) instead of the same friendly, specific message the other two checks already produce. **(2)** `shipment_side_offices.office_id` (the "additional Export/Import offices beyond the primary EMO/IMO" feature, `routes/shipments.js` — live and real, per `db/cargodesk.sample-data.json` having genuine rows, but never actually documented in `CLAUDE.md`'s own Database table list) had **no FK at all** — the more serious finding. **Verified live, definitively**: created a scratch office, attached it as a shipment's additional Export office, deleted the office — the delete succeeded silently (200), and the row was NOT cleaned up; `GET .../side-offices` then showed a shorter list with zero indication anything was wrong, because that route's own `JOIN offices` is an INNER JOIN that silently hides exactly this kind of orphan from view. Confirmed the orphan's continued physical existence via a temporary debug route reading the raw table directly (removed after use) — same masking pattern that made the original `shipment_cost_lines` bug easy to miss. Checked the two other pre-existing rows in this dev DB (dated 2026-08-29, predating this pass) against their referenced offices — both still exist, confirmed not orphaned; only this session's own reproduction had actually triggered the bug so far. **Fixed**: `document_templates` got a matching app-level guard (`"Office is referenced by a document template..."`), reusing its existing DB-level protection. `shipment_side_offices.office_id` got a real FK — `RESTRICT` (the default, no `ON DELETE` clause), not `CASCADE`, deliberately mirroring this codebase's own established "an office referenced by a shipment must be deactivated, never deleted" precedent for the primary EMO/IMO/Controlling columns, rather than silently dropping shipment history — plus a matching app-level guard reusing the primary guard's exact wording (conceptually the same situation from an admin's point of view). Same orphan-then-add-constraint migration idiom the `shipment_cost_lines` fix established: a guarded `DELETE ... WHERE NOT EXISTS` (idempotent, safe on every boot) followed by a `pg_constraint`-checked `ALTER TABLE ... ADD CONSTRAINT`. Run against this dev DB: removed exactly 1 orphaned row (this session's own reproduction — no historical corruption found). **Re-verified live after the fix**: both guards now return a clean 400 with a specific message instead of a raw 500; a genuinely unreferenced office still deletes cleanly (200) — confirmed no false positive. `tests/organization.test.js` (64, the file covering office CRUD + delete-guard) and `tests/document-templates.test.js` (16) re-run green, plus `tests/shipment-crud.test.js` (35) as a broader sanity check on the shared `lib/schema.js` migration block — 115 assertions, zero regressions. Backend-only fix, no frontend changes, no build needed. **Also closed in the same pass**: `shipment_side_offices` was a real, live feature with no entry in `CLAUDE.md`'s own Database table list at all — added one. |
| 2026-09-03 | `routes/admin-reset.js` — the "Reset Demo Data" feature TRUNCATEs every table not in `PRESERVE_TABLES` using `TRUNCATE ... CASCADE`. Postgres's `CASCADE` here cascades to ANY table with a FK pointing *at* a truncated table — including a table that IS in `PRESERVE_TABLES`, if one happens to reference something being wiped. Does that ever actually happen, silently defeating the whole "preserve" list? | **No gap — verified clean by exhaustively checking every FK in the schema, not just the ones that looked likely** | Grepped every one of the ~50 `REFERENCES` clauses across `lib/schema.js` and checked, for each, whether the table *holding* the FK column is a `PRESERVE_TABLES` member referencing a table that *isn't* (the one direction that would actually cause a preserved table to be silently cascaded away). Found none — every cross-reference is either preserved→preserved (`country_trade_lanes`→`countries`/`trade_lanes`, `linked_ports`→`port_locations`, all safe, no cascade even triggered) or wiped→wiped/preserved (`carrier_agents`→`customers`, `carrier_agent_locations`→`port_locations`, etc. — the referencing table is already in `toReset` on its own merits, so it gets explicitly truncated regardless of what it points at). The code comment's own claim ("none of PRESERVE_TABLES sits downstream of a to-be-reset table in the FK graph") holds. This specific property was also already empirically verified once for real at v0.86.0 ship time (a full backup/reset/restore cycle against this same dev DB, confirmed counts before/after). **Deliberately did not re-run a live full reset to triple-check** — doing so would wipe this session's own accumulated verification data (shipments, customers, quotes, offices created across this entire audit), which the project's own standing convention says to keep for future perf testing; the exhaustive code-level check plus the existing recorded live test together were judged sufficient without that tradeoff. No code changes. |
| 2026-09-03 | `lib/shareToken.js` (backs the public, unauthenticated `GET /api/share/:token` tracking link and `GET /api/share/document/:token`) — a home-rolled HMAC-SHA256 signer/verifier, not the `jsonwebtoken` library the rest of the app uses. Does `verifyToken`'s `crypto.timingSafeEqual` call handle every malformed-token shape safely? | **Real, trivially-reachable bug on a public unauthenticated endpoint — FIXED same day** | `crypto.timingSafeEqual` throws a `RangeError` (`"Input buffers must have the same byte length"`) rather than returning `false` when its two arguments differ in length — confirmed directly with a throwaway Node snippet before assuming it from memory. `verifyToken` computed `expected` (always a fixed-length base64url HMAC-SHA256 digest, 43 characters) and compared it against the caller-supplied `sig` with no length check first — any token shaped `data.sig` where `sig` is literally any *other* length (a mistyped link, a crawler probing the URL, not even a deliberate attack) throws synchronously inside the route handler. **Verified live**: `GET /api/share/abc.xyz` and `GET /api/share/document/abc.xyz` both crashed with a raw `{"error":"Internal server error"}` 500 (`wrapAsyncHandler`'s global error middleware catching it — confirmed via the server log showing `RangeError` at `shareToken.js:17`, `code: 'ERR_CRYPTO_TIMING_SAFE_EQUAL_LENGTH'`) instead of the clean `"Invalid or tampered link"` 400 this same function already returns for every other malformed shape (no period, right-length-but-wrong-content signature). **A real gap in the EXISTING test suite made this easy to miss**: `document-distribution.test.js` already had a "garbage token rejected cleanly" assertion, but its fixture (`"not-a-real-token"`) has no period at all, short-circuiting on the `!data||!sig` guard before ever reaching `timingSafeEqual` — it had never actually exercised the buggy path, despite reading as coverage for exactly this. **Fixed**: added an explicit `sigBuf.length !== expectedBuf.length` check before calling `timingSafeEqual`, the standard, universally-recommended fix for this specific Node.js gotcha — leaks nothing an attacker doesn't already know, since a valid signature's length is fixed and derivable from the HMAC algorithm alone. **Re-verified live**: the same malformed request on both endpoints now returns a clean 400; a genuinely valid share token (freshly minted via the real `POST .../share-token` route) still returns 200 with correct tracking data — confirmed the fix doesn't affect the legitimate path. **Strengthened the test suite in the same pass** rather than relying only on manual verification: added the exact wrong-length-signature case as a new assertion in `tests/classified-locations.test.js` (already the home of the tracking-endpoint's own coverage, no new file needed) — `document-distribution.test.js`'s existing "garbage token" assertion was left as-is (still valid coverage for its own shape) rather than fixed to secretly test something else. `tests/classified-locations.test.js` (32, was 30), `tests/nvocc-carrier-identity.test.js` (21, uses the tracking endpoint's carrier-identity fields) both re-run green; `tests/document-distribution.test.js`'s other 7 failures are pre-existing and unrelated (the Document Distribution microservice wasn't running in this pass) — confirmed by their error text (`"...Service is unreachable"`), not this fix. |
| 2026-09-03 | `routes/allocations.js` — container TEU is computed independently in 6 places across the codebase (`server.js` x2, `allocations.js`, `export.js` using `'20'→1,'40'/'45'→2,else 0`; `shipments.js`, `shipment-ops.js` using `'40'→2,else 1`, the latter's own comment claiming — incorrectly — that this matches "everywhere else this app computes TEU"). Given this exact "same formula independently duplicated with drift" shape is what the GP/margin bug turned out to be, is this a live discrepancy too? | **Confirmed dead code, not a live bug — not fixed, by design** | `containers.size` is `TEXT NOT NULL CHECK(size IN ('20','40'))` (`lib/schema.js`) — a real DB-level constraint, not just app-level validation (`routes/shipments.js`'s own `CONTAINER_SIZES = ["20","40"]` check is a mirror of it, per its own comment). For every value that can actually exist in this column, both formulas produce identical results (`'20'`→1 either way, `'40'`→2 either way) — the `'45'` branch and the differing `ELSE 0`/`ELSE 1` defaults are unreachable in practice today. Confirmed this precisely rather than assuming it from the CHECK constraint's presence alone. **Deliberately not consolidated onto one shared constant** (unlike the GP/margin fix, which had real, live, verified financial impact) — touching 6 files for a change with zero observable behavior difference isn't proportionate; the DB CHECK constraint itself is also a real backstop against this ever silently drifting into a live bug (widening `CONTAINER_SIZES` without updating the schema's own CHECK would fail loudly, not silently). Logged here so a future container-size expansion (e.g. adding back `'45'`) knows to check all 6 sites, not rediscover them. |
| 2026-09-03 | `routes/allocations.js`'s `GET /api/allocations/match` — when `contract_source=remote` and an allocation's linked contract can't be fetched (service down, or the contract itself vanished), the code's own comment says this "can't be verified either way, so it's passed through... same 'don't disprove it' default as an unscreened party" (mirroring the `!a.contract_id` case just above it). Does the actual code achieve that, or does it just say so? | **Real, live-reproduced bug — the code contradicted its own comment. FIXED same day** | The catch block set `legs = []` on any fetch failure, which then flows into `findMatchingContractLegs([], {...}).length > 0` — always `false`, meaning the allocation is *excluded*, the opposite of "passed through." **Verified live, definitively, with a real positive-match fixture** (not a coincidental non-match that could be explained either way): built a real contract with a leg carrying `pol_carrier_haulage:true` for a scratch route, linked a scratch allocation to it — confirmed it correctly appears in `GET /api/allocations/match?...&needsPolHaulage=1` in local mode (real proof of a genuine match), then flipped `contract_source` to `remote` with the Contract Management Service intentionally not running — the exact same allocation, same underlying facts, silently vanished from the results. Since every allocation requires a `contractId` at creation (`POST`/`PUT /api/allocations` both reject a blank one), this isn't a narrow edge case — during any real remote-service outage, **every** allocation would silently drop out of a haulage-aware sailing search, with no error surfaced anywhere. **Fixed**: `contractLegsFor()` now returns `{legs, unreachable}` instead of just `legs`; the match loop passes an allocation through unconditionally when `unreachable` is true, exactly mirroring the already-correct `!a.contract_id` case immediately above it in the same loop. **Re-verified live, precisely**: local mode unaffected (still matches); the identical remote+unreachable repro now correctly passes the allocation through; restored `contract_source` to `local` after each toggle. Also verified the *positive* remote path is untouched by starting the real Contract Management Service and re-running `tests/contract-service-toggle.test.js` in full (20/20, including "allocation matches once its remote contract's leg is fetched and shows carrier haulage" — proof a genuinely reachable, genuinely matching remote contract still works exactly as before). `tests/allocations-crud.test.js` (38) also re-run green. |
| 2026-09-03 | `routes/contracts.js`'s `PUT /api/contracts/:id` auto-revive logic (`effStatus`, added to fix a real prior bug: a contract auto-expired by `expireStaleContracts()` stayed stuck showing Expired forever even after its `validTo` was edited into the future). The condition only checks the SUBMITTED status and validTo — does it also confirm the contract was already `Expired` *before* this edit, the only case the fix's own comment says it's meant to correct? | **Real, live-reproduced bug — an operator's own explicit choice got silently overridden. FIXED same day** | `effStatus = (status === 'Expired' && (!validTo || validTo >= today)) ? 'Active' : status` has no reference to the row's own pre-edit status at all — it fires on ANY submission of `status:'Expired'` with a blank or future `validTo`, not just a resubmission of an already-stale sweep artifact. **Verified live**: created a fresh Draft contract with no `validTo`, then explicitly edited it to `status:'Expired'` (a real, valid manual choice — e.g. marking a superseded negotiation dead) — the save silently returned `status:'Active'` instead, with no error or warning of any kind. **Fixed**: added `oldRow?.status === 'Expired'` to the condition (fetching `oldRow` earlier in the function, before the check, rather than after) — the auto-revive now only fires when the contract was genuinely already Expired going into this edit, exactly matching the fix's own stated intent. **Re-verified live, both directions**: the exact repro above now correctly stays `Expired`; a genuinely stale sweep artifact (created directly with `status:'Expired'` + a past `validTo`, then edited with `validTo` pushed into the future while the payload still resubmits `status:'Expired'` verbatim, matching what the real edit form always does) still correctly auto-revives to `Active` — proving the original fix's own purpose survives untouched. Added both cases as permanent regression coverage in `tests/contract-improvements.test.js` (39, was 37) rather than relying on manual verification alone; `tests/pending-revalidation.test.js` (19) also re-run green as a broader sanity check on contract-status-dependent logic. |

| 2026-09-03 | **CRITICAL** — `GET /api/shipments/:id` has always correctly gated a single shipment through `applyShipmentAccessFilter` (`if (!(await applyShipmentAccessFilter([s], req.user, req)).length) return err(res, "Not found", 404)`). Do the 70+ per-shipment SUB-RESOURCE routes across `routes/shipments.js`, `shipment-ops.js`, `edi.js`, `customs-filing.js`, `customers.js`, `document-distribution.js`, and `share.js` (cost-lines, documents, milestones, services, customs-filings, EDI messages, legs, parties, schedules, ...) replicate that same check, or does knowing a shipment id alone bypass it entirely? | **Real, live-confirmed, systemic authorization bypass affecting dozens of routes — FIXED same day with a new centralized guard, per explicit user decision on remediation approach** | Grepped every `app.(get|post|put|patch|delete)("/api/shipments?/:(id|shipmentId)` registration app-wide — 70+ matches across 7 files. Spot-checked 5 (`screening`, `cost-lines`, `services`, `milestones`, `documents` in `shipment-ops.js` alone) and found **none** called `applyShipmentAccessFilter`; `costLineRead` (gating `/cost-lines`) is a pure role check with zero shipment-specific scope awareness. **Verified live, concretely, not theoretically**: created a scoped `occ_bk` user (`allOffices:false`, assigned to exactly one office), two shipments (X in-scope, Y assigned to a different office), planted a real BUY cost line on Y tagged `"CONFIDENTIAL-SCOPE-TEST"` ($9,999). The scoped user was correctly blocked (404) from `GET /api/shipments/Y` itself — but `GET /api/shipments/Y/cost-lines` returned the planted line in full, `GET /api/shipments/Y/customs-filings`/`/documents` both returned `200` (not `403`/`404`) for a shipment the same user couldn't even see the header of. This is `applyShipmentAccessFilter` itself working exactly as designed (confirmed correct in the 2026-09-03 CRITICAL office-scoping fix, ARCHITECTURE.md's own earlier entry this same day) — the gap is that essentially nothing outside the one route calls it.
  <br><br>**Root cause is structural, not a handful of missed call sites**: this codebase has no `app.param()` or per-router middleware for shipment ids — every route file registers directly on the shared `app` object (`module.exports = function xRoutes(app, ctx) {...}`), so each of 70+ routes across 7 files would have needed its own author to independently remember to call `applyShipmentAccessFilter` by hand. Given `GET /api/shipments/:id` is the only one that ever did, this was never going to hold as routes accumulated.
  <br><br>**Presented the finding and scale to the user before choosing a fix** (proof, affected-route count, four remediation options) rather than unilaterally patching 70+ routes or just the one file (`customs-filing.js`) originally being audited — user chose the centralized-middleware option. **Fixed**: new `shipmentScopeParamCheck` (`server.js`, right after `applyShipmentAccessFilter`'s own definition) registered via `app.param("id", ...)` and `app.param("shipmentId", ...)` — Express's param callbacks are always app-global (no per-router scoping exists in this codebase to attach it to instead), so it fires for every `:id`/`:shipmentId` param on every route in the app, not just shipment ones; a leading `id.startsWith("SHP-")` check makes it an immediate no-op for any other entity's id (offices, contracts, customers, allocations, ...) rather than needing per-router wiring. For a genuine shipment id it runs a lightweight query (`id, pol, pod, carrier_code, emo/imo/controlling_office_id` — confirmed by reading `matchesScopeItem`/`shipmentMatchesAccessConfig` that this is the complete field set the filter's own logic ever reads; no `port_locations`/`offices` JOINs needed since the mapped object is never returned to the client) through the exact same `applyShipmentAccessFilter` the primary route already trusts, and 404s before the route's own handler ever runs if the result is empty. A missing shipment falls through (`next()`) so each route's own existing not-found handling is completely unchanged. Applies to every HTTP method, not just reads — a write to an out-of-scope shipment's sub-resource was exactly as open as a read.
  <br><br>**Re-verified live, comprehensively**: the exact repro above, repeated across 5 different sub-resource routes spanning 3 different files (`cost-lines`, `customs-filings`, `documents`, `milestones`, `legs`) — all now correctly `404`. The scoped user's own in-scope shipment X continued to work normally (no false positive). Admin access is completely unaffected (still sees the full planted cost-line data on shipment Y, confirming `applyShipmentAccessFilter`'s own admin/operator bypass still applies through the new middleware). A non-shipment `:id` route (`GET /api/contracts/:id`) works identically to before, confirming the prefix check correctly no-ops. A genuinely nonexistent shipment id still 404s via each route's own pre-existing logic, unchanged. Full real-browser CDP pass as admin across 4 different shipment sub-pages (Cost Entry, Documents, Milestones, Conditions) — clean render, zero regression, no new console errors. Full 63-file `npm test` backend chain re-run (in segments, restarting between each — the chain's own well-known behavior of silently stopping at the first non-zero-exit file, plus the login rate limiter exhausting across dozens of files' worth of logins, both already-documented gotchas, not new ones): **62 of 63 files passed cleanly on the first attempt.** Two exceptions, both investigated to a definitive conclusion rather than assumed: (1) `customer-credit-control.test.js` had one assertion whose expected status changed from `403` to `404` for an out-of-lane `trade_manager` attempting `POST .../credit-override/approve` — traced precisely: that user's own `trade_lane` scope item (`APAC→NAM`) genuinely doesn't cover the test's `EU-N→NAM` shipment, so the new shipment-scope guard now correctly blocks them *before* the route's own dedicated lane-authorization check ever runs, via the exact same underlying scope-item mechanism — a correct, expected tightening, not a regression; updated the assertion to accept either status, with a comment explaining why. (2) `ai-agent-gating.test.js` had one flaky assertion (a real outbound fetch to a deliberately-invalid DNS name expected to fail with a clean 502) that passed cleanly on an isolated retry — structurally impossible to be caused by this fix, since `/api/ai/extract-document` carries no shipment id in its path at all. Also verified the destructive `admin-reset.test.js` (guardrails-only, per its own documented scope) and the two-browser-context `shipment-edit-lock.test.js` both passed unaffected. |
| 2026-09-03 | `routes/organization.js`'s `DELETE /api/branches/:id` guard — it checks `offices WHERE branch_id=$1` before allowing a delete, mirroring the exact class of gap just fixed in `routes/offices.js`. Is that the complete list of everything that can reference a branch? | **Real gap, same class as the `offices.js` fix — FIXED same day** | `org_countries.branch_id REFERENCES branches(id)` with no `ON DELETE` clause (default `RESTRICT`) — a branch with zero offices but still named as an org_country's own "home branch" was uncaught by the existing guard. **Verified live**: created a scratch branch, pointed a real `org_countries` row (Monaco) at it, attempted delete — raw `{"error":"Internal server error"}` 500 (confirmed via the server log: `constraint: 'org_countries_branch_id_fkey'`), not a helpful message. **Fixed**: added a matching `org_countries WHERE branch_id=$1` guard, mirroring the existing office-count check's exact shape and wording, with correct singular/plural phrasing. **Re-verified live**: the same repro now returns a clean 400 with a specific message; a genuinely unreferenced branch still deletes cleanly (200). Checked `org_countries` itself for the same class of gap (does anything reference `org_countries.country_code`?) — grepped the whole schema, found nothing, so its own ungated `DELETE /api/org-countries/:code` is correctly gap-free, not an oversight. `tests/organization.test.js` (64) and `tests/multi-entity-accounting.test.js` (29, exercises branches/currency directly) both re-run green. |

| 2026-09-03 | `routes/document-distribution.js`'s `POST/GET/DELETE /api/test/webhook-receiver` (the Test Tools Webhook Simulator's own mock receiver) — every other Test Tools simulator-style route in this codebase (`kanban.js`'s ops-sweep trigger, `edi.js`'s and `customs-filing.js`'s own `simulate-response` routes) requires at least `shipmentWrite`. Does this one? | **Real gap — FIXED same day** | It had no role gate at all — only the app-wide global `auth()` gate applied. **Verified live**: created a scratch plain-`viewer`-role user (the lowest tier in this app's own role hierarchy) and confirmed they could freely `POST`/read this shared, process-wide in-memory log. **Checked carefully before fixing** whether this endpoint is ever legitimately reached by a credential-less caller (e.g. the Document Distribution Service itself, making a genuine server-to-server outbound webhook call when an admin points a test webhook at this URL) — confirmed the global `auth()` gate was *already* rejecting a fully anonymous request with 401 before this fix touched anything, and the existing, passing `tests/document-distribution-gaps.test.js` already exercises the `POST` route with a real admin JWT, not anonymously — so the established, tested calling pattern already matches an authenticated, privileged caller, and narrowing from "any authenticated role" to `shipmentWrite` changes nothing for it. **Fixed**: added `shipmentWrite` to `POST`/`DELETE`; left `GET` open to any authenticated role (read-only, matches this app's own hierarchy — nothing sensitive lives in a 20-entry-capped scratch log beyond whatever a privileged user already sent). **Re-verified live**: viewer `POST` now correctly `403`s; admin `POST` still succeeds; viewer `GET` still works. `tests/document-distribution-gaps.test.js`'s 7 webhook-receiver assertions all still pass; its one unrelated failure (`PUT webhook-settings`) confirmed environment-only (Document Distribution Service not running in this standalone-monolith pass — direct curl reproduced the same `503 Service is unreachable`, untouched by this fix). |
| 2026-09-03 | `routes/pack-types.js`'s own comment says "nothing else in the app enforces this list" for `pack_type_definitions` — but its own header ALSO says `container_packages.pack_type_id` references it. Checking `lib/schema.js` directly: neither `pack_type_id` NOR `container_id` on `container_packages` carried any FK at all. Is that a real, live orphan risk — the same class as the original `shipment_cost_lines` bug? | **CRITICAL — confirmed, same severity class as the original `shipment_cost_lines` bug, and this dev DB already had 416 real orphaned rows. FIXED same day.** | `container_id TEXT NOT NULL` (no FK) is the more serious of the two — a `container_packages` row is real cargo data (description, quantity, DG flags, declared value, HS code), not just cosmetic reference data. **Verified live, precisely**: created a container, added a package (a pallet) to it, deleted the container — `DELETE /api/containers/:id` returned a clean `200` with zero indication anything was wrong, and the package row **survived**, permanently orphaned and unreachable through any route (confirmed via a temporary debug route reading the raw table directly, removed after use). **Extended the check per direct request** to confirm arbitrary nesting depth, not just the top level: built a real 2-level fixture (a pallet, then a box created *under* that pallet via `parentId`) — reading the creation route (`POST /api/containers/:id/packages`) confirmed every row at *every* nesting depth carries the same root `container_id` (a nested box's own `parentId` points at the pallet's package id, but its `container_id` is still the container's own id, not the pallet's) — so a single `ON DELETE CASCADE` on that one column correctly cascades through the whole tree at once, not just the first level. **`pack_type_id` is the milder sibling** — also no FK, but the referenced data is a cosmetic type label (icon/name), not the package's own real cargo data, and the column is already nullable ("no specific type" is a legitimate existing state). **Fixed, per direct instruction** ("deleting a container with existing attached packages should not leave any orphans around, and the information should be deleted in one go"): `container_id` gets a real `ON DELETE CASCADE` FK — declared inline in the `CREATE TABLE` for fresh installs (`containers` is created earlier in the same file) plus a guarded orphan-cleanup-then-`ALTER TABLE` migration for existing installs, exact same idiom as the `shipment_cost_lines`/`shipment_side_offices` fixes. `pack_type_id` gets `RESTRICT` instead (not `CASCADE`) — deleting an in-use *type definition* must not silently destroy real cargo data; mirrors the offices/branches "deactivate instead of delete" precedent, with a matching friendly app-level guard in `routes/pack-types.js` (`pack_type_definitions` is created *after* `container_packages` in `lib/schema.js`, so unlike `container_id` this FK could only be added via the migration's `ALTER TABLE`, not inline — the ordering was checked directly, not assumed). Any already-orphaned `pack_type_id` reference is backfilled to `NULL` (not row-deleted — the package's own real data is preserved) rather than compounding the loss. **Migration run against this dev DB removed 416 pre-existing orphaned `container_packages` rows** — real, accumulated data corruption from this exact gap, not a hypothetical. **Re-verified live, comprehensively**: the 2-level pallet+box repro now correctly cascades both rows away in one container delete (confirmed via the same raw-table debug route); the `pack_type_id` guard now returns a clean 400 with a specific message instead of a raw 500 for an in-use type, while a genuinely unused one still deletes cleanly. `tests/container-packages.test.js` (46, including its own parent-child app-level cascade-delete test, confirming this DB-level fix doesn't conflict with the existing tree-walk logic), `tests/container-bulk-import.test.js` (21), `tests/shipment-crud.test.js` (35, broader schema sanity check), and `tests/definitions-registries.test.js` (25) all re-run green — 127 assertions, zero regressions. |
| 2026-09-03 | Continuing the audit into the last 3 unexamined route files (`ai.js`/`ais.js`/`system.js`): `routes/ai.js`'s `executeTool(name, args)` — backing the AI Assistant's `get_shipment`/`list_shipments` tools — takes no `user`/`req` parameter at all. Every other shipment-reading route in this app, including the very `shipmentScopeParamCheck` middleware added earlier this same audit pass, gates access through `applyShipmentAccessFilter`. Does a chat tool call bypass that entirely? | **CRITICAL — confirmed live, same severity class as the systemic shipment-scope bypass fixed earlier this pass. Two additional real bugs found alongside it. FIXED same day, per direct user requirements given mid-fix.** | **Verified live** with a real tool-use round trip: stood up an in-process mock OpenAI-compatible LLM endpoint, created a scratch `trade_manager` user scoped to one POL, pointed `ai_endpoint` at the mock, and drove `POST /api/ai/chat` asking about a real shipment outside that user's scope. The mock's scripted tool-call response reached `executeTool` for real; the captured tool-result payload sent back to the "model" contained the shipment's full row — id, ports, carrier, status — despite `GET /api/shipments/:id` and the `shipmentScopeParamCheck` app.param guard both correctly blocking the exact same user from the exact same shipment through every ordinary route. Root cause: `shipmentScopeParamCheck` only fires on Express route *params* (`:id`/`:shipmentId` in a URL) — a shipment id arriving inside a chat tool call's JSON `args` never touches it.
  <br><br>**A second, independent real bug found in the same function while fixing it**: `list_shipments`' row-count query was built via `q.replace("SELECT s.*", "SELECT COUNT(*) AS n")` against a base query that also selected two unaggregated `LEFT JOIN` columns (`p1.name AS pol_name`, `p2.name AS pod_name`) — Postgres correctly rejects this with no `GROUP BY`, so the tool has been failing on **every single call**, its raw internal SQL error text returned verbatim as the "tool result." Never caught before because the existing `tests/ai-agent-gating.test.js` deliberately stops short of a real tool-use round trip (no live provider key in this environment) — this pass's mock-endpoint verification was the first time this tool's actual query ever ran end to end.
  <br><br>**Mid-fix, user gave three explicit standing requirements for this feature, predefined rather than left to judgment**: (1) the agent must be contained to the application level only — no code execution, no filesystem/process/other-resource access; (2) specifically called out the real-world McDonald's/dealership chatbot pattern of users getting an embedded assistant to generate off-topic code as the abuse case to prevent; (3) the agent must never create, modify, or delete any application data.
  <br><br>**Fixed, addressing the live scope bug and all three requirements together**: `executeTool` now takes `(name, args, user, req)`; `get_shipment`/`list_shipments` (the only two of the 4 tools reading from the per-user-scoped `shipments` table — confirmed `get_contract`/`get_allocation` need no change, since `GET /api/contracts/:id`/`GET /api/allocations/:id` apply no such filter anywhere else in this app either, so leaving them as-is is parity, not an oversight) now run their result through `applyShipmentAccessFilter` exactly like every other shipment-reading route. `list_shipments`' count query was rewritten with its own simple `SELECT COUNT(*) FROM shipments s WHERE ...` (no joins — it never needed the port names), and now overfetches (200 rows) before scope-filtering and slicing to 50, so a heavily-restricted user isn't left with an artificially tiny result just because most of the raw top-50 fell outside their scope. **Requirement 1 (application-level containment) and requirement 3 (no writes)**: were both already structurally true — `executeTool` is a fixed if/else chain over exactly 4 named tools with no dynamic dispatch, `eval`, `child_process`, or filesystem access anywhere in the file, and every query it issues is a plain `SELECT`. Made this an explicit, enforced invariant rather than an implicit one: added a `readOnlyQuery()` wrapper (regex-checks the SQL starts with `SELECT`, throws otherwise) that every tool query now goes through, plus a prominent comment documenting the closed-whitelist/no-mutation boundary so a future added tool can't silently cross it. **Requirement 2 (no off-topic/code-generation abuse)**: added a new `CONTAINMENT_PROMPT` — explicit instructions never to write/execute/explain code in any language, role-play as another persona, follow instructions embedded in a message/tool-result asking it to ignore its own system prompt, or discuss anything outside CargoDesk shipments/contracts/allocations — prepended to every `/api/ai/chat` call **unconditionally**, ahead of and not overridable by an admin's own custom `ai_system_prompt` in Application Settings. Disclosed honestly in the code comment: this is a prompt-level mitigation for a real, known abuse pattern, not a hard technical guarantee against a sufficiently determined prompt-injection attempt — the hard guarantee is the server-side tool whitelist/read-only enforcement above, which holds regardless of what the model outputs.
  <br><br>**Re-verified live, comprehensively**, via a permanent new `tests/ai-agent-tool-scope.test.js` (15 assertions, added to the `npm test` chain) exercising the exact same mock-endpoint tool-use round trip: the scoped user's out-of-scope `get_shipment` call now returns a clean "not found" with no shipment data leaked; the same user's in-scope shipment still resolves correctly (the fix doesn't over-block); an unrestricted admin still sees the same out-of-scope shipment via the identical tool call (no regression to the normal/common case); `list_shipments` now succeeds with no SQL error and correctly excludes the out-of-scope shipment while still returning the in-scope one; an invented tool name (`execute_shell_command`) is safely rejected with a plain error, never executed; and the containment clause is confirmed present in the actual system-prompt content sent to the mock LLM on every call. **A real operational mistake made and corrected during this verification, disclosed rather than quietly fixed**: an early, since-corrected version of the manual verification script crashed after mutating live `ai_endpoint`/`ai_api_key`/`ai_model` settings but before restoring them; every subsequent script run then captured that already-polluted state as its own "before" snapshot and faithfully restored *that* instead of the true original. The dev environment's real original state was recovered from `tests/ai-document-extraction.test.js`'s own pre-existing docstring (which documented it as "AI Agent enabled but no real provider API key configured") rather than guessed, and settings were set back to exactly that (`ai_agent_enabled:'1'`, endpoint/key/model blank) — confirmed via `GET /api/ai/settings` matching that documented state exactly before moving on. |
| 2026-09-03 | Starting the Reports/Consumption Dashboard audit — the backend side (`routes/finance.js`, `routes/reports.js`, `routes/scheduled-reports.js`) already had a deep pass earlier this session (office-scoping, 5-occurrence accrual-vs-actual GP bug). The frontend, `src/pages/DashboardPage.jsx` (2164 lines), had not. Two things: does the "6-Week TEU Trend" chart on the Contract Consumption tab actually scope itself to the visible window, matching its own sibling chart's (`trendChartData`'s `activeCodesSet`) correct pattern? And is every one of this file's ~2160 lines actually reachable from its own render tree? | **Both real. One a live UX/correctness bug, FIXED same day; the other a substantial dead-code find (552 lines), REMOVED same day.** | **Chart scope bug**: `contractTrendData`'s `contractIds` was every Central-type contract that EVER had a shipment, all-time, unbounded — not scoped to the chart's own 6-week window at all, unlike `trendChartData`'s `activeCodesSet` right above it in the same component, which correctly limits itself to allocations active in the visible range. Verified live against this dev DB: 12 contracts drew a line on the chart; only 1 actually had a Confirmed shipment anywhere in the visible 6-week window — the other 11 were permanent flat-zero clutter, directly contradicting the chart's own caption ("Weekly Confirmed consumption per contract — last 6 weeks"), and structurally guaranteed to get worse as more historical contracts accumulate over the life of a real deployment. **Fixed**: a contract now only earns a line if it has at least one Confirmed shipment whose ETD actually falls in the visible window — the same criterion that would make its line non-zero anyway. **Re-verified live via CDP** (real browser, real auth token + EULA-accepted flag seeded into `localStorage`, screenshot before deciding then after fixing): legend line count dropped from 12 to the correct 1, chart renders a real non-trivial curve instead of near-total noise, zero console errors, clean `vite build`.
  <br><br>**Dead code**: `LanePair`/`OverrideBtn`/`ConflictBanner`/`AllocContractPickerModal`/`AllocationForm` (lines 29-579, ~552 lines) — a complete, self-contained, second copy of the allocation conflict-detection/creation UI, exhaustively confirmed unreachable: none of the 5 identifiers appear anywhere in `DashboardPage`'s own actual render tree (grepped every occurrence in the file — all fall inside this same block, calling each other, never called from outside it), none are exported (`DashboardPage.jsx` has exactly one `export default`), and none are imported by any other file. `SpaceConfigurationsPage.jsx` has its own independent, live copies of `ConflictBanner`/`AllocationForm` (confirmed by direct comparison) — this is the exact same class of leftover-after-a-page-split dead code this file's own v0.65.0 changelog entry already documented once before ("three complete dead component definitions inside DashboardPage.jsx... a whole second, never-rendered... UI"), just a different leftover pair from a later split (Space Configurations was carved out into its own standalone page) that the earlier sweep didn't catch. Removed along with 21 now-unused imports it alone was pulling in (`useSaving`, `useRef`, `Inp`/`Sel`/`ContractTypeInput`/`Textarea`/`Field`, `PortField`, `CarrierCombobox`, `Modal`/`ConfirmModal`, `Spinner`, 8 icon imports, `LANE_BADGE_VARIANT`) — each one individually confirmed via a zero-hits grep for any use after the dead block's end before removing it. File: 2164 → 1618 lines. Clean `vite build` before and after.
  <br><br>**A third bug, self-inflicted, caught by CI rather than this pass's own testing**: the new regression test for the AI-agent scope fix above (`tests/ai-agent-tool-scope.test.js`) picked its in-scope/out-of-scope shipment pair by reading `GET /api/shipments` and requiring 2+ distinct POLs already present in the data — passed cleanly against this session's own long-lived dev DB (hundreds of accumulated shipments) but broke the very next CI run (`backend-tests-and-build`, "Run monolith test suite (45 files)" step, exit 1) the moment it was pushed: CI's `npm run seed` step seeds only MDM reference data, not sample shipments, so the 45-file chain builds its shipment data up purely from each test's own fixtures in whatever order they run — depending on ambient leftovers from wherever this file happens to land in that chain is exactly the ordering-fragile assumption a long-lived dev DB masks completely. Root-caused from the CI job's public run page (log viewer itself failed to render for a direct fetch; reasoned from the workflow's own `seed`-only step plus the timing matching this exact push) rather than guessed. **Fixed**: the test now creates its own 2 scratch shipments with fixed, distinct POLs (`NLRTM`/`DEHAM`) as fixtures, cleans them up in the existing `finally` block, and no longer touches `GET /api/shipments` for anything but a membership check. Re-run clean locally (17/17, 2 more assertions than before from the new fixture-creation checks); confirmed no leftover scratch users via a direct API check.
  <br><br>**Continuing into the rest of this domain** — `routes/reports.js`'s `scopedShipmentIds`/`reportsGate` (the trade_manager-sees-only-their-own-lane claim `ReportsPage.jsx`'s own comment makes) re-read in full: correctly built on the same already-audited `applyShipmentAccessFilter`, and correctly applied at the very end of both `billing-performance` and `invoice-collections` (deliberately AFTER computing "Missing" candidates against the full unscoped set, with a clear comment on why — an out-of-scope invoice must still count company-wide or an in-scope Missing candidate would look uninvoiced). No gap — confirmed clean, not just trusted. The 4 shared Reports panel components (`GpBreakdownPanel`/`InvoiceCollectionsPanel`/`BillingPerformancePanel`/`ScheduledReportsPanel`, ~994 lines combined) read in full — no bugs found, consistently using the same actual-preferred (`actualAmountUsd ?? amountUsd`) convention this session already audited elsewhere.
  <br><br>**`DashboardArchivePage.jsx`** (`DashboardArchive`, the Archive nav page — expired space configurations + Renew/Delete): built with a `standalone` prop meant to support two render modes (a full page, or a collapsible section embedded in another page) — grepped every call site app-wide and found exactly one, in `App.jsx`, which always passes `standalone`. The entire non-standalone branch (~106 lines) was therefore dead code, confirmed the same way as the `DashboardPage.jsx` find above. **This dead branch also carried a real latent bug**: byte-diffing it against its standalone sibling (`tableContent`, which the dead branch duplicates instead of reusing) showed the Renew/Delete `ActionMenu` in the standalone copy is correctly gated behind `canManageConfigs` (`canEdit`) — the dead copy has no such gate at all, rendering the actions unconditionally for any authenticated user. Never reachable in practice (confirmed single call site, always `standalone`), so no live exposure — but exactly the kind of drift a duplicated-instead-of-shared block invites, and would have started mattering the moment anyone ever passed `standalone={false}`. **Fixed**: removed the dead branch, the now-unused `open`/`setOpen` state, the now-always-true `standalone` prop (component and its one call site both), and the redundant `!standalone && allocations.length === 0` early-return. 250 → 139 lines. Clean `vite build`; re-verified live via CDP (real auth token + EULA-accepted flag seeded, screenshot) — Archive page renders its real 9 expired configurations correctly, Actions menu present for the logged-in admin, zero console errors. |
| 2026-09-03 | `ShipmentGpSankey.jsx` (the Profit Breakdown diagram inside `GpBreakdownPanel`, shared by both the per-shipment GP Overview page and the Reports GP-by-Trade-Area drilldown) — `buildSankey()`'s `revIdx`/`costIdx` nodes only get created when `totalSell`/`totalBuy > 0`, but `sellRows`/`buyRows.forEach` unconditionally links every leaf to them regardless. What happens to a shipment whose SELL lines net to zero or negative (a fully/over-reversed invoice, v0.53.0) with no BUY lines at all? | **CRITICAL — live-reproduced crash, not a display bug. FIXED same day.** | **Verified live**: created a scratch shipment with a single `-$500` SELL line (a `POST .../cost-lines` amount can be negative — the exact shape a real Invoice Reversal produces — and no BUY lines) and navigated to its real GP Overview page via CDP. Confirmed crash: `Cannot read properties of undefined (reading 'y')` thrown inside Recharts' `<Sankey>`, React's own console explicitly noting "Consider adding an error boundary" (there wasn't one) — the entire page rendered blank, not just the diagram. Root cause confirmed by direct trace: `totalSell = -500` fails the `> 0` gate so `revIdx` stays `null`, but the one `sellRows` entry still pushes `{ source: <idx>, target: null, value: -500 }` into the Sankey's own link list — a link with no valid target node crashes Recharts' layout pass.
  <br><br>**Precisely characterized the bug's actual boundary before fixing**, rather than assuming "any negative value breaks it": built a second scratch shipment with a net-POSITIVE total (SELL $1000 + a $-300 reversal on a different charge code = $700 net, plus a $400 BUY line) — this rendered correctly, no crash, confirming an individual negative LEAF row is fine as long as the aggregate Revenue/Cost node it links to actually exists. The bug is specifically about the aggregate `totalSell`/`totalBuy` going to zero-or-negative while rows still exist to link, not about negative values in general — a real Sankey/flow diagram fundamentally can't represent a negative aggregate flow either way (money moving backward through the graph isn't a forward flow), so there's no meaningful diagram to draw in that case regardless of the crash. **Fixed**: `buildSankey` now returns `null` (the same "nothing to show" path already used when every total is exactly zero) whenever `totalSell < 0 || totalBuy < 0` — `ShipmentGpSankey` already handles a `null` result by rendering nothing, so this fails safe with zero new code paths. `GpBreakdownPanel`'s own stat cards (Est./Actual Buy/Sell/GP, GP Variance, By Payment Indicator, By Charge Code) are entirely independent of this component and already correctly display negative figures via `fmtSigned` — nothing about the shipment's real GP data is hidden, only the one diagram that has no honest way to draw it.
  <br><br>**Re-verified live, all 3 scenarios**: the original crash repro now renders the full page cleanly (stat cards correctly show `Est. Sell -$500.00` / `Est. GP -$500.00` in red, diagram section absent, zero console errors); the net-positive-with-a-negative-leaf case still renders its diagram exactly as before (no over-suppression regression); a normal all-positive shipment ($1,500 SELL / $900 BUY) still renders the full diagram correctly (OFR → Total Cost, Revenue → Gross Profit, $600 net result). Since the fix lives in the shared `buildSankey()` function both call sites use, the Reports GP-by-Trade-Area drilldown gets the identical protection with no separate change needed. All 3 scratch shipments deleted after verification.
  <br><br>**This closes out the Reports/Consumption Dashboard audit pass.** Last file: `SpaceConfigurationsPage.jsx` (1053 lines, the live twin of the dead-code block already removed from `DashboardPage.jsx` above) read in full — no gaps found. Notably already self-corrected: an explicit code comment documents that per-allocation TEU used to be recomputed client-side here with its own carrier+contract+route heuristic, which "could — and did — disagree with the server's own `/api/allocations/match` number and with the Dashboard's Contract Consumption tab's own third definition" — fixed in an earlier pass (before this session) to read `confirmedTEU`/`pendingTEU`/`rejectedTEU` directly off the server-authoritative `loadTeuBuckets()` fields instead, the same single-source-of-truth precedent this whole audit keeps reinforcing elsewhere. The Linked Shipments modal's own local re-derivation of the same buckets (for its per-shipment breakdown table) uses the identical bucket rule and the identical explicit `shipment.allocationId === a.id` link criterion, so it can't drift from the header figure — confirmed by direct comparison, not assumed.
  <br><br>**Full domain summary**: 3 real bugs found and fixed (unbounded contract-trend chart, a CRITICAL live-reproduced Sankey crash, a latent missing-permission-gate in dead code), 552 + 106 lines of confirmed-dead code removed across 2 files, 1 CI-breaking test-fragility bug self-caught and fixed, and 2 backend routes plus 6 more frontend files read and confirmed clean (`routes/finance.js`, `routes/reports.js`, `ReportsPage.jsx`, `GpBreakdownPanel.jsx`, `BillingPerformancePanel.jsx`, `InvoiceCollectionsPanel.jsx`, `ScheduledReportsPanel.jsx`, `SpaceConfigurationsPage.jsx`). |
| 2026-09-03 | Direct request to audit Application Settings, on a hunch — `getSettings()` (server.js) returns the entire `app_settings` key-value table completely verbatim, and `GET /api/settings` (routes/system.js) has only ever had the global "any authenticated user" gate, no role check. `sso_client_secret`/`ai_api_key`/`ais_api_key` all live in that same table. Does a low-privilege role actually get the raw secret values back? | **CRITICAL — confirmed live, and already self-documented as known-but-unfixed for one of the three. FIXED same day.** | **Verified live**: set real-looking values for all three secrets as admin, created a scratch plain-`viewer`-role user (the lowest tier in this app), and called `GET /api/settings` with their token — all three came back in full plaintext: `sso_client_secret`, `ai_api_key`, `ais_api_key`. `sso_client_secret` is the most severe of the three — a leaked Azure AD/Entra ID client secret risks OAuth client impersonation against the organization's own identity provider, a consequence that reaches well outside this app. **Found direct evidence this was already known, at least in part**: v0.51.0's own comment on `org_signing_certs` states outright that `ai_api_key` "already returns in full plaintext to any authenticated user" — documented, never fixed; `sso_client_secret`/`ais_api_key` were added later and inherited the identical gap with no fresh look. Stronger evidence still: `SystemEmailSettingsPanel`'s own code comment (`AppSettingsPage.jsx`) already states smtp_password was deliberately pulled out into "its own dedicated GET/PUT/test routes (never folded into the generic `/api/settings` blob, **which is public and unfiltered**)" — a correct, self-aware fix already applied to ONE secret field and never extended to the other three that stayed on the generic blob.
  <br><br>**Fixed**: new `SECRET_SETTING_KEYS`/`maskSecrets()` in `routes/system.js` — both `GET` and `PUT /api/settings` now strip these 3 keys from their response (replaced with `""`) and add a companion `<key>_configured` boolean, the same masked-boolean idiom `office_mail_settings`' `hasPassword` and the already-audited `GET /api/ai/settings`'s `hasKey` use. `PUT` additionally treats a submitted empty string for these keys as "leave the stored secret untouched" (matching the SMTP password fields' own established "blank means keep current" idiom exactly) rather than wiping it — necessary because the masked `GET` response means these inputs now always render blank whether or not a secret is actually configured. A genuine, deliberate clear is still possible via JSON `null` (distinct from `""` in a parsed JSON body) — ordinary UI typing never produces `null`, so this is purely an intentional-clear escape hatch, not reachable by accident.
  <br><br>**Frontend** (`AppSettingsPage.jsx`): all 3 secret inputs (SSO Client Secret, AI Agent API Key, the AIS card's shared Consumer Key field) now show `"Leave blank to keep current"` when a secret is already configured — the exact wording `SystemEmailSettingsPanel`/`OfficePage.jsx` already use for the SMTP password precedent. The AIS card's own "● Key configured" indicator was reading the now-permanently-blank raw value's truthiness — switched to the new `_configured` flag, or it would have permanently shown "not configured" even when a real key existed.
  <br><br>**A real regression self-caught during verification, not shipped**: 3 existing test files (`ai-agent-gating.test.js`, `ai-agent-tool-scope.test.js`, `ai-document-extraction.test.js`) restore `ai_api_key` to its pre-test value by reading it off `GET /api/settings` and PUTting it back — which used to work by reading the real value, and after masking reads back `""` every time, which the new "blank means keep" rule then treats as a no-op, silently leaving each test's own placeholder/fake key permanently stuck in the database instead of actually restoring it. Caught by literally re-running the suite twice and checking the real `hasKey` state afterward, not assumed fixed. Fixed all 3 files to key off the new `ai_api_key_configured` boolean and send the explicit `null` clear signal when the true baseline was "no key" (this environment's own documented state) — a real pre-existing key is deliberately left untouched instead of guessed at, since its value was never observable to begin with, exactly mirroring `ai-document-extraction.test.js`'s own pre-existing "can't be reliably restored" comment for that case.
  <br><br>**Re-verified live, comprehensively**: the original leak repro now returns `""`/`_configured:true` to the same viewer account instead of the real secret; a submitted blank value no longer wipes an already-configured secret; a genuine new value still correctly overwrites the old one. Live CDP screenshots of all 3 real UI surfaces (AI Agent tab, Single Sign-On tab, AIS Vessel Tracking card) confirm the field renders blank with the correct placeholder and the "configured" indicator is accurate. Full regression: `tests/ai-agent-gating.test.js` (22), `tests/ai-document-extraction.test.js` (9), `tests/ai-agent-tool-scope.test.js` (17), plus 5 more files independently exercising `PUT /api/settings` (`auth-admin.test.js` 65, `container-packages.test.js` 46, `eadapter.test.js` 47, `reports.test.js` 30, `schedule-catalog.test.js` 77) — 313 assertions total, all green from a fresh restart, confirming the fix doesn't disturb any non-secret setting's existing behavior. All scratch secrets/toggles reset to their documented defaults afterward via a direct DB script (the new PUT protection correctly refuses to blank them through the ordinary API anymore, by design). |
| 2026-09-05 | Direct request: full audit of the Loop Codes / Loop Map Explorer / `LoopRouteModal` integration (`routes/loop-codes.js`, `lib/schema.js`'s `loop_codes`/`loop_code_ports`, `MdmLoopCodesPage.jsx`, `LoopMapExplorerPage.jsx`, `LoopRouteModal.jsx`, `mapCore.jsx`, `ShipmentHeaderBar.jsx`'s Loop field, `App.jsx` wiring, `tests/loop-codes.test.js`) — the newest area in the app, never audited before. Nav/routing wiring, `api.js` parity, header integration, shared pan-zoom code, and the 3-page separation of concerns. | **No gap — fully clean on every wiring/duplication question checked** | `mdm-loop-codes`/`mdm-loop-map-explorer` are both correctly present in `MDM_PAGES`, both have a nav entry, a breadcrumb label, and a routing-switch case — no orphaned page key either direction. `src/api.js`'s `loopCodes` namespace (`list`/`get`/`resolve`/`create`/`update`/`remove`/`saveRotation`) matches the full backend route surface exactly — no drift, no dead method. `ShipmentHeaderBar.jsx`'s Loop field → `deriveLoopCode` (`src/utils/scheduleLoop.js`) → `LoopRouteModal`'s default export are all intact and correctly wired after this session's `Timeline` named-export change (`export default LoopRouteModal` confirmed still present). `LoopRouteModal.jsx`'s `LoopMap` and `LoopMapExplorerPage.jsx`'s world map both consume the exact same `usePanZoom`/`ZoomControls`/`ringToPath` from `mapCore.jsx` — the StrictMode zoom-drift fix (TKT-S5J5UP) and the antimeridian path fix automatically cover both, no separate/stale implementation exists anywhere. The 3 loop-related pages have cleanly non-overlapping responsibilities confirmed by direct read: `MdmLoopCodesPage.jsx` (registry CRUD + drag-to-reorder rotation editor) imports zero map-rendering code; neither map page duplicates any editor logic. |
| 2026-09-05 | `routes/loop-codes.js`'s `ROTATION_SQL` (joins `loop_code_ports` → `port_locations` for name/country/lat/lng) and the rotation `PUT` route's per-port existence check — do either respect `mdm_source=remote`, matching the pattern of the 3 prior MDM-bypass sites already found and fixed 2026-09-02/03 (`resolveInvoiceThresholds`, `resolveSeaPorts`, `linkedPortCodes`)? `loop_codes`/`loop_code_ports` didn't exist yet (v0.90.1) when that sweep ran, so it was never covered. | **Real gap, same class as the 3 prior fixes — dormant/cosmetic today (`mdm_source` is `local`). `TKT-4U6XKH`, FIXED same day, per direct authorization.** | Both queries read `port_locations` directly with zero `isRemote()`/`callMdmService()` branch. `ROTATION_SQL` is an INNER JOIN, so under `mdm_source=remote` any port not present in this monolith's own increasingly-stale local copy would silently vanish from a loop's rotation entirely, not just show blank fields; the existence check would incorrectly reject a genuinely valid remote-only port code as `"Unknown port {code}"` when saving a rotation. **Fixed**: new `getRotation(loopCodeId)` keeps the exact original `ROTATION_SQL` JOIN in local mode; in remote mode it fetches the plain `loop_code_ports` rows, batch-resolves names/coords through the MDM Service's already-existing `/internal/port-locations?ids=` filter (the same one `resolveSeaPorts` already uses), and merges in JS. `findUnknownPorts(codes)` replaces the old one-query-per-port loop with a single batched existence check, local (`WHERE unlocode IN (...)`) or remote (`ids=`), reporting the first missing code to keep the exact original error message shape. **Re-verified live**: flipped `mdm_source` to `remote`, re-fetched the real `AL1` loop's rotation — all 8 ports resolved with correct names/coordinates from the MDM Service (confirmed against the local-mode values, identical); a scratch loop's rotation save correctly rejected an unknown code (`"Unknown port ZZZZZ"`) and correctly accepted a valid one (with real resolved name/coords) in remote mode; flipped back to `local` and re-ran `tests/loop-codes.test.js` (21/21) and `tests/mdm-crud.test.js` (91/91) clean. |
| 2026-09-05 | Found while probing the gap above: since `loop_code_ports.port_unlocode REFERENCES port_locations(unlocode)` (no `ON DELETE` clause, added v0.90.1) is a real FK, does `DELETE /api/port-locations/:unlocode` (`routes/mdm.js`) actually guard against deleting a port that's still referenced by a loop's rotation — and by extension, by any of its other referencing tables? | **Real, live-confirmed gap (error-handling only, no data loss — the FK correctly protects the data). `TKT-8DK30F`, FIXED same day, per direct authorization.** | **Verified live** with isolated scratch data (a throwaway 5-char UN/LOCODE-format port + a throwaway loop, both deleted after): set a loop's rotation to reference the scratch port, then attempted to delete that port — the delete correctly failed (no orphan created, no silent data loss), but the response the caller sees is a bare `{"error":"Internal server error"}` 500 with zero indication of the cause. The dev-server log confirms this is a caught-not-crashed Postgres `23503` foreign-key-violation (`"Key (unlocode)=(ZZTST) is still referenced from table \"loop_code_ports\""`, constraint `loop_code_ports_port_unlocode_fkey`), not a process crash. Checked `lib/schema.js` for every real FK into `port_locations(unlocode)` before fixing (not assumed): exactly 2 default to RESTRICT and can actually block a delete — `carrier_agent_locations.unlocode` and `loop_code_ports.port_unlocode`; `linked_ports`' own two FKs are `ON DELETE CASCADE`, so a linked-port pair correctly and silently disappears with the port, needing no guard. **Fixed**: added the same application-level pre-check pattern `offices.js`/`organization.js` already use elsewhere (query the referencing table, return a clean 400 naming it, before ever attempting the delete) for both real RESTRICT sites. **Re-verified live**: the same repro now returns a clean `{"error":"Port is referenced by a loop code's rotation — remove it from that rotation first"}` (400); the port still deletes cleanly (200) once the referencing loop is removed. `tests/mdm-crud.test.js` (91, exercises ordinary port-location CRUD including delete) re-run green — confirms the new guard doesn't false-positive on an unreferenced port. |
| 2026-09-05 | Continuing the audit into a full pass of `routes/system.js` (health, settings, schedule search — only the settings-secrets-masking fix from 2026-09-03 had touched this file before). Does the generic `PUT /api/settings` route actually exclude every key that has its own dedicated, more tightly-gated admin-only route, or just the one (`sso_enforce_exclusive`) its own code comment explicitly calls out? | **CRITICAL — confirmed live, same severity class as the office-scoping and shipment-scope bypasses already fixed. `TKT-BKHQNM`, FIXED same day, per direct authorization.** | Five more keys — `mdm_source`, `contract_source`, `screening_source`, `kanban_source`, `customer_source` — plus `shipment_sidebar_order`, each have their own dedicated `requireRole(["admin"])` route specifically because they're a bigger blast radius (a genuine data-source cutover lever, or a shared nav-order overwrite affecting every user) than the ordinary operational toggles this generic route otherwise handles for `operator`/`occ_bk` too — but none of the six were actually excluded from the generic route, only `sso_enforce_exclusive` was. **Verified live, unambiguously**: created a scratch operator account, confirmed it was correctly blocked (403) by the dedicated `PUT /api/settings/mdm-source` route, then sent the identical change through the generic `PUT /api/settings` route instead — succeeded (200), `mdm_source` genuinely flipped to `remote`. Repeated for `shipment_sidebar_order` — the operator successfully overwrote the shared sidebar nav order for every user in the app. Both real settings restored immediately after confirming the bug, before any other user could be affected. **Fixed**: generalized the existing single-key exclusion into one `ADMIN_ONLY_DEDICATED_KEYS` list covering all 6 — a non-admin sending any of them through the generic route now gets a clean 403 naming exactly which key(s) and pointing at the dedicated route instead. **Re-verified live, comprehensively**: all 6 keys correctly 403 for the same scratch operator via the generic route, confirmed nothing actually changed from any blocked attempt, an ordinary non-admin-gated setting is still accepted from the same operator (no over-blocking), and an admin can still use the generic route for these same keys with no false block. **New `tests/settings-security.test.js` (16 assertions), added to the permanent `npm test` chain** — the existing per-service toggle test files (`mdm-service-toggle.test.js` etc.) only ever exercised the dedicated route's own gate, never this one, which is exactly why this sat undetected. `tests/auth-admin.test.js` (65), `tests/sso-enforce-exclusive.test.js` (12) re-run green, zero regressions. |
| 2026-09-05 | Continuing the audit into the next unexamined area: `routes/command-center.js` (Command Center Quality & Exception Management, 4 endpoints, never previously audited). Two things checked: does `(mot='SEA' OR leg_type='SEA')` — used identically in 3 of the 4 queries — actually match the corrected `isSeaLeg()` semantics from the v0.90.1 stale-`mot`-on-a-Delivery-leg fix (`server.js`), given a real leg with exactly that stale-`mot` shape was confirmed still present in this dev DB? And does `carrier-scorecard`'s carrier-name lookup respect `mdm_source=remote`? | **First check: no gap, confirmed clean after live investigation, not assumed. Second: real gap, same class as the loop-codes fix above — `TKT-383QBI`, FIXED same day, per direct authorization.** | **Leg-type check**: found 13 real legs in this dev DB with `mot='SEA'` but `leg_type` genuinely something else (12 Pick-up, 1 Delivery) via a temporary debug route (removed after use). But traced every path that can ever set `etd_source`/`eta_source` to `'ais'` — `lib/ais-listener.js`'s own tracked-leg query and `routes/ais.js`'s Test Tools Simulator leg-picker (which shares the exact same write path, `ctx.ingestAisMessage`) both gate strictly on `leg_type='SEA'` alone, no `mot` fallback at all. So `etd_source`/`eta_source` can never actually be `'ais'` on a leg whose `leg_type` isn't `'SEA'` — meaning `command-center.js`'s `(mot='SEA' OR leg_type='SEA')`, once co-filtered with `(etd_source='ais' OR eta_source='ais')` in every one of its 3 queries, can never actually select a mismatched row; the `mot='SEA'` half of the OR is redundant but not a correctness bug. Confirmed by direct trace of both write paths, not assumed from the schema alone. **Carrier-scorecard gap**: `carrierNames` was built from a bare, unconditional `SELECT code, name FROM carriers` — no `isRemote()` branch at all, the same recurring bypass class as `resolveSeaPorts`/`linkedPortCodes`/`resolveInvoiceThresholds`/today's own `loop-codes.js` fix above. Only affects the scorecard's display label (falls back to the bare carrier code) — cosmetic, not data loss, dormant today since `mdm_source` is `local`. **Fixed**: added the same `isRemote()`/`callMdmService()` branch, calling the MDM Service's existing unfiltered `GET /internal/carriers` (a small, bounded registry — no `ids=` filter needed, matches what the local branch already fetches unconditionally). **Verified live, definitively**: flipped to `mdm_source=remote`, renamed `MAEU` to a deliberately distinct string (`"REMOTE-DISTINCT-NAME"`) via the write proxy (which itself correctly routes to the remote service in this mode), confirmed the scorecard immediately reflected that exact string — not a coincidental match — then restored the name and flipped back to `local`, confirming the scorecard reverted to the real local name. `tests/command-center.test.js` (38) re-run green. |
| 2026-09-05 | Continuing the audit into the next unexamined area: `routes/kanban.js` + `routes/testcases.js` (tickets, ticket links, Kanban projects/versions/columns, test items, Story↔TestCase links — never previously audited). Cross-referenced each route against its own siblings for consistency, rather than reading in isolation. | **Two real gaps, both live-reproduced, both also independently replicated in the standalone Kanban Service's own mirror. `TKT-H0MN9E`/`TKT-U44NPU`, FIXED same day, per direct authorization.** | **Gap 1 — silent orphan on ticket links.** `POST /api/tickets/:id/links` checked `toId` exists (404 if not) but never checked `req.params.id` itself — found by contrast with `testcases.js`'s own `POST /api/test-items/:id/story-links`, which correctly checks both sides. `ticket_links.from_id`/`to_id` are plain `TEXT`, zero FK. **Verified live**: created two tickets, deleted one, POSTed a link FROM the deleted one TO the survivor — `201`, a permanently orphaned row created with zero error. Same severity class as the `shipment_cost_lines`/`container_packages`/`shipment_side_offices` orphan bugs already fixed elsewhere in this log. **Gap 2 — column delete guard counts tickets across ALL projects.** `DELETE /api/kb/columns/:id`'s ticket-count check was a bare `WHERE status=$1` with no `project_id` scoping — since every project seeds identical default column names (Ready/In Progress/...), a genuinely empty column in one project gets wrongly blocked because an unrelated project's tickets happen to share that column's name. **Verified live**: this dev DB has 21 real `Ready` tickets on the existing Main Board; a brand-new second project's own freshly-seeded, genuinely-empty `Ready` column was immediately blocked from deletion citing those same 21 tickets. **Both bugs found faithfully replicated in `services/kanban/server.js`'s own mirrored routes** (a straight port of the original code, not a new mistake) — same class of finding as the `screenCustomer()` override-wipe bug from 2026-09-03, fixed on both sides of the extraction cut in the same pass. **Fixed**: added the missing source-ticket existence check to both `POST /api/tickets/:id/links` implementations; scoped the column delete-guard's count to `(project_id=$2 OR project_id IS NULL)` — the same inclusive rule `GET /api/tickets` already uses for legacy pre-migration tickets, so a column still correctly blocks on a legacy untagged ticket the board actually displays in it, just no longer on an unrelated project's own tickets. **Re-verified live in both `kanban_source` modes** (local and remote) after each fix — both repros now behave correctly on both sides; a real link between two real tickets, and a genuinely-populated column's delete-block, both still work exactly as before. `tests/kanban-api.test.js` (49), `tests/kanban-service-toggle.test.js` (19), `tests/test-items.test.js` (35) all re-run green — 103 assertions, zero regressions. |
| 2026-09-05 | This session's Loop Map label redesign (always-on decluttered labels → hover-only `PortHoverChip`, applied to both `LoopRouteModal.jsx`'s `LoopMap` and `LoopMapExplorerPage.jsx`) dropped the old helper's import from both call sites — was the helper itself, and everything that fed it, actually removed too? | **Confirmed dead code. `TKT-EL175K`, FIXED same day, per direct authorization.** | `declutterLabels()` in `mapCore.jsx` (~20 lines, exported) — grepped every file under `src/`, zero remaining call sites. Both map components also still computed a per-point `above: i % 2 === 0` property (`LoopMapExplorerPage.jsx` ~line 300, `LoopRouteModal.jsx`'s `LoopMap` ~line 181) that only ever fed `declutterLabels()`'s own alternating-placement seeding — never read anywhere else. **Fixed**: removed `declutterLabels()` entirely and dropped the now-pointless `above:` property from both point-mapping call sites (the unused loop index `i` that only fed it was dropped too). **Re-verified live via CDP**, both consumers: `LoopMapExplorerPage.jsx`'s consolidated overlay box (drag/resize/minimize/cross-highlight all still work, screenshotted) and `LoopRouteModal.jsx`'s own Timeline + `LoopMap` (both render correctly, `Show on map` toggle confirmed working, screenshotted) — zero console errors beyond the pre-existing, unrelated missing-favicon 404. |
| 2026-09-05 | Continuing the audit after the generic-settings-route CRITICAL fix (`TKT-BKHQNM`): does `shipmentScopeParamCheck` (`server.js`, the global `app.param('id'/'shipmentId', ...)` enforcement point closed as a systemic bypass on 2026-09-03) actually cover every per-shipment sub-resource route, or only the ones keyed by the shipment's OWN id in the URL? | **CRITICAL — confirmed live, a second-order instance of the same bypass class. `TKT-19FU3B`, FIXED same day, per direct authorization.** | `shipmentScopeParamCheck` fires on any URL param literally named `id`/`shipmentId` whose *value* starts with `SHP-`, no-opping otherwise — so a route keyed by a SUB-resource's own id (a milestone `MS-...`, a document `DOC-...`, a haulage waypoint `HWP-...`) never triggers it at all, since that id never matches the `SHP-` prefix. Seven routes were built this way and had never been touched by the 2026-09-03 fix: `PUT`/`DELETE /api/milestones/:id`, `PUT`/`DELETE /api/haulage-waypoints/:id`, `PATCH`/`DELETE /api/documents/:docId`, `GET /api/documents/:docId/download` — the last being the most severe (plain `auth()`, no write-role gate at all, a pure read/info-disclosure bypass). **Verified live, unambiguously**: created a scoped `occ_bk` user with a `pol` scope item deliberately excluding a real scratch shipment (confirmed `GET` on it correctly 404s for this user — office assignment alone does NOT gate shipment visibility, only `user_scope_items` does, a real test-setup mistake caught and corrected mid-repro), then called `PUT /api/milestones/:id` directly against one of that shipment's own milestones as the scoped user — succeeded (200), a genuine cross-scope write with zero authorization check. **Fix approach chosen after direct discussion of 3 options** (asked "which would be best approach for long term code maintenance and growth?"): nest every one of the 7 routes under `/api/shipments/:shipmentId/...` to inherit the existing, already-proven `shipmentScopeParamCheck` middleware — chosen over (a) an inline manual guard per handler, rejected because "remember to call a helper at every call site" has already failed twice in this exact codebase's own audit history, and (b) teaching the middleware a prefix-to-table lookup map, rejected as a second, parallel enforcement mechanism to reason about. This also fixes a real API-shape inconsistency — every other sub-resource in `shipment-ops.js` (cost-lines, services, loading-plan, haulage records) was already nested this way. **Fixed**: all 7 routes moved to `/api/shipments/:shipmentId/milestones/:id`, `/api/shipments/:shipmentId/haulage-waypoints/:id` (rewritten as a real SQL JOIN through `shipment_haulage_records → containers`, since `shipment_haulage_waypoints` has no direct `shipment_id` column), `/api/shipments/:shipmentId/documents/:docId` (+ `/download`). `src/api.js`, every frontend call site (`CarrierBookingsTable.jsx`, `ContainerEventsPanel.jsx` + a new `shipmentId` prop threaded from `ShipmentContainersPage.jsx`, `invoiceGenerator.js`, `DocumentsModal.jsx`, `HaulageDetailsPanel.jsx`, `ShipmentAccountingInvoicesPage.jsx`, `ShipmentDetailPage.jsx`, `TrackedDocPreviewModal.jsx`), and every affected test file (`billing-performance`, `command-center`, `container-events`, `customer-credit-control`, `document-signing`, `house-bl-lifecycle`, `invoice-reversal`, `invoice-collections`, `merchant-haulage`, `shipments-list`, `workflow-improvements`) were migrated to the new nested shape — the old bare-id routes no longer exist at all, confirmed by grep across `src/` and `tests/`. **Re-verified live after the fix, all three route families**: the exact original exploit (scoped user + out-of-scope shipment's milestone) now 404s at the new nested URL, and the old bare-URL shape 404s outright since the route is gone; a fresh document-download repro and a fresh haulage-waypoint-update repro (through the new JOIN) both correctly 404 for the same out-of-scope scoped user and correctly 200 for an admin control on the identical resource. Full regression run across all 11 affected test files (415 assertions total) green from a fresh restart — an initial run tripped both the login (20/15min/IP) and document-action (20/60s) rate limiters purely from the batch's own cumulative volume, resolved the same way CI already does (`LOGIN_RATE_MAX`/`DOC_ACTION_RATE_MAX` raised for the run), not a real regression. Clean `vite build`. All scratch fixtures (the scoped user, its scope item, the scratch shipment and everything created on it during verification) deleted after use. |
| 2026-09-05 | Finishing the `routes/shipment-ops.js` audit — the last unexamined section (milestone-templates CRUD, shipment schedules CRUD, the Schedule Generator's standalone catalog routes, schedule-events, roughly lines 1444-1878). Does `POST /api/schedules` (the Test Tools Schedule Generator's catalog-template create route) respect `mdm_source=remote` in its carrier/vessel/port existence checks, matching the pattern already found and fixed twice this session (`loop-codes.js`, `command-center.js`)? | **Real gap, same class as the 2 prior fixes — dormant/cosmetic today (`mdm_source` is `local`). `TKT-B5LNIF`, FIXED same day, per direct authorization.** | The route's `carrier`/`vesselImo`/`pol`/`pod` validation each ran a bare, unconditional `SELECT ... FROM {carriers,vessels,port_locations}` with zero `isRemote()`/`callMdmService()` branch. **Verified live, definitively**: created a scratch carrier (`ZQAUDIT`) while `mdm_source=remote` (landing only in the standalone MDM Service), confirmed it does NOT exist in the monolith's own local `carriers` table, then — still in remote mode — POSTed a schedule template referencing it: rejected with `"Unknown carrier code: ZQAUDIT"` even though it's a real, valid code in the actual source of truth once cut over. **Fixed**: new `mdmCarrierExists`/`mdmVesselExists`/`mdmPortExists` helpers (module scope, mirroring `loop-codes.js`'s `isRemote`/`findUnknownPorts` shape) — remote mode calls the MDM Service's existing single-code lookup endpoints (`/internal/carriers/:code`, `/internal/vessels/:imo`, `/internal/port-locations/:unlocode`) via the same `.catch(() => null)` not-found idiom `routes/mdm.js` already uses elsewhere; local mode unchanged. **Re-verified live after the fix**: the same remote-only carrier is now correctly accepted; a genuinely nonexistent carrier code (negative control) is still correctly rejected. `mdm_source` restored to `local`, scratch carrier and schedule deleted. `tests/schedule-catalog.test.js` (77), `tests/schedules.test.js` (44), `tests/mdm-service-toggle.test.js` (25), `tests/mdm-crud.test.js` (91) all re-run green — 237 assertions, zero regressions. Clean `vite build`. **This completes the `routes/shipment-ops.js` audit in full** — every route in the file (screening, cost-lines, services, loading-plan, Merchant's Haulage + waypoints, milestones, documents, milestone-templates, shipment schedules, the schedule catalog, schedule-events) has now been examined at least once across this session's several passes. |
| 2026-09-05 | Full pass of `routes/auth.js` (login, `/me`, change/forgot/reset-password, system-email settings, SSO, users/scope CRUD) — only ever touched previously for one narrow role-validation question (2026-09-03). Does `GET /api/auth/me` — the silent token-restore call `App.jsx` makes on every page mount with an already-valid token, replacing the whole `user` object via `setUser(u)` — actually return everything a fresh login's response does, or does it still have gaps the same way `allOffices`/`roles` were found and fixed here in an earlier release (per this file's own code comments)? | **Real, live-confirmed functional regression — a real permission flag silently dropped on every page reload. Two more related, code-confirmed gaps found alongside it. `TKT-D0OVGK`, FIXED same day, per direct authorization.** | `/api/auth/me`'s `SELECT` never included `can_view_finance` at all — the response object had no `canViewFinance` key whatsoever (not `false`, genuinely absent), unlike `POST /api/auth/login`'s and `change-password`'s own responses, both of which correctly include it. **Verified live, definitively**: created a scratch `trade_manager`, set `canViewFinance:true` via `PATCH`, logged in fresh — response correctly showed `canViewFinance:true` — then called `GET /api/auth/me` with that same token (simulating a page reload): the response carried no `canViewFinance` field at all. Since `App.jsx`'s mount effect does `setUser(u)` with this exact response, any user with real Finance access lost the Finance nav link and every finance-gated view the instant they reloaded the page, recoverable only by logging out and back in fresh. **Fixed**: added `can_view_finance` to the `SELECT` and `canViewFinance` to the response, mirroring the exact pattern the file's own comments show was already used to fix this identical gap for `allOffices`/`offices` in an earlier release — `can_view_finance` was simply missed when that fix landed. **Re-verified live**: `/api/auth/me` now correctly returns `canViewFinance:true` for the same token. **Two adjacent gaps found while fixing this, both confirmed via direct code/schema read rather than a live end-to-end test** (SSO's real callback path needs a genuine Azure AD tenant this environment doesn't have — an already-disclosed testing boundary for this codebase, not a new one): (1) the SSO callback's own JWT omitted `canViewFinance`/`allOffices` entirely, unlike local login's and `change-password`'s tokens — fixed to match, plus `LoginPage.jsx`'s JWT-decode (the only place the frontend reads a JWT payload directly) updated to read both fields; (2) the SSO auto-provisioning `INSERT` (find-or-create on first SSO login) omitted the `all_offices` column entirely, silently inheriting the `users` table's own schema-level `DEFAULT TRUE` (`lib/schema.js`) — while `POST /api/users`' admin-driven create path always sets it `FALSE` explicitly. A brand-new SSO-authenticated user was silently granted org-wide "sees every office" visibility with zero admin decision, the opposite default of every admin-created account — confirmed via the schema's own unambiguous `BOOLEAN NOT NULL DEFAULT TRUE` declaration plus a direct column-list comparison of the two `INSERT` statements, not something needing a live check. Fixed to explicitly set `FALSE`, matching the admin-created default; an admin can still opt a user into `all_offices` afterward via the existing `PATCH` route, same as any other account. **New regression assertions added to `tests/auth-admin.test.js`'s existing finance-flag block** — logs in as the flagged scratch user and confirms `GET /api/auth/me` now returns it, the exact gap the pre-existing test never exercised (it only ever checked `GET /api/users`, which already had the column selected — precisely why this sat undetected). `tests/auth-admin.test.js` (69), `tests/password-reset.test.js` (30), `tests/sso-enforce-exclusive.test.js` (12) all re-run green — 111 assertions, zero regressions. Clean `vite build`. |
| 2026-09-05 | Full pass of `routes/office-mail.js` (per-office SMTP config: `GET`/`PUT .../mail-settings`, `POST .../mail-settings/test`) — one of several small admin-registry-style files never touched by this audit at all. Given the class of bug this session keeps finding in credential-handling code (the `app_settings` plaintext-secrets leak, the generic-settings-route bypass), does this one hold up: password masking, cross-office isolation, transporter-cache correctness, and the `office_mail_settings` → `offices` delete guard? | **No gap — clean on every angle checked** | `mapOfficeMailSettings` (`lib/mappers.js`) correctly returns only `hasPassword: !!r.smtp_password`, never the raw secret, matching `mapSystemEmailSettings`/`mapEadapterConfig`'s identical established pattern. All 3 routes are `adminOnly`, correctly parameterize every query by `req.params.id` (no cross-office leak point — the `test` route's own fallback-to-stored-password lookup is scoped to the same office id, not a client-suppliable one). `office_mail_settings.office_id` is `UNIQUE REFERENCES offices(id) ON DELETE CASCADE` (`lib/schema.js`) — no orphan risk, confirmed via direct schema read (this was also incidentally confirmed clean by the 2026-09-03 `shipment_side_offices` FK audit, which named `office_mail_settings` as one of the tables that already cascades correctly, unlike the one it was fixing). `lib/mailer.js`'s `transporterCache` is keyed per-office and invalidated on every save (`invalidateTransporterCache`); every real call site of `sendViaOffice` app-wide (`shipment-ops.js`'s document-send route, `server.js`'s 4 scheduled-sweep call sites) derives the office id server-side from a shipment/office row already in hand, never from client input, so there's no way to trick a send into using a different office's credentials. `tests/office-mail.test.js` (31) re-run green, confirming current behavior unchanged by this pass — no code touched, nothing to re-verify beyond the read. |
| 2026-09-05 | Full pass of `routes/eadapter.js` (per-carrier, per-office EDI connectivity config) — the other never-touched credential-holding file. Same checklist as `office-mail.js`: credential masking, the office-delete guard for `carrier_eadapter_configs.office_id` (no DB-level FK on this column — is there at least an app-level one?), carrier-code existence validation, DELETE-route consistency. | **No gap — clean, with 2 minor, deliberately-not-fixed style notes** | `mapEadapterConfig` correctly returns only `hasCredential: !!r.credential`, verified both via code read and a live create→get→blank-PUT round trip (a real secret went in, never came back in any response, and a blank-credential `PUT` correctly preserved it — `hasCredential` stayed `true` throughout). `carrier_eadapter_configs.office_id` has no DB-level FK (`office_id TEXT NOT NULL DEFAULT ''`, confirmed via `lib/schema.js`) — but this exact column was already named and verified as correctly covered by `routes/offices.js`'s app-level delete-guard pre-check in the 2026-09-03 audit pass (re-confirmed by direct grep: `routes/offices.js:78` still queries it before allowing an office delete). `POST /api/eadapter/configs` doesn't validate `carrierCode` against the real `carriers` registry (a typo'd code would silently create a dead, unmatchable config) — but the only real entry point, `EadapterConfigModal.jsx`, already constrains input through `CarrierCombobox`, so this is a defense-in-depth gap reachable only via a direct API call, not a practical one; matches this codebase's own pervasive pattern of trusting a combobox-constrained frontend over duplicating existence checks server-side for every admin config route, not something isolated to this file. `DELETE /api/eadapter/configs/:id` is the one route in the file with no existence check (silently `{ok:true}` on a bogus id, where `PUT`/other config-style deletes elsewhere 404) — a real, minor API-shape inconsistency, but harmless (admin/operator-gated, no data-integrity impact either way) and left as a disclosed style note rather than a fix, to keep this pass scoped to gaps with actual impact. `tests/eadapter.test.js` (47) re-run green. |
| 2026-09-05 | Full pass of `routes/mdm.js` (1035 lines, 48 routes — carriers, vessels, port-locations, linked-ports, carrier-agents + locations + schedules, trade-lanes, regions, countries, unlocodes, commodities). Given this file is the one that already implements every `isRemote()`/`callMdmService()` branch for its own routes (not a bypass-site candidate the way `loop-codes.js`/`command-center.js`/`shipment-ops.js` were), the check here was different: is every branch actually complete and correct, and does every `RESTRICT` FK this file's own tables hold have a matching app-level delete-guard, the same question already answered for `offices.js`/`organization.js`? | **Two real bugs found and fixed, one of them the same delete-guard-gap class already fixed twice this session for other tables. `TKT-2N8E6V`, FIXED same day, per direct authorization.** | **Bug 1 — real gap, same class as the `offices`/`branches`/`port_locations` delete-guard fixes**: `DELETE /api/countries/:iso2` had zero guard for `carrier_agent_locations.country_iso2` — a real `RESTRICT` FK (no `ON DELETE` clause, added when Carrier/Line Agents shipped) grepped directly from `lib/schema.js` (`country_trade_lanes.iso2`'s own FK to the same table is correctly `ON DELETE CASCADE` and needs no guard — only this one does). **Verified live**: created a scratch country, gave a carrier-agent header a country-type location pointing at it, deleted the country — a raw `{"error":"Internal server error"}` 500 (server log confirmed `constraint: 'carrier_agent_locations_country_iso2_fkey'`, code `23503`), not a helpful message. **Fixed**: the same app-level pre-check pattern used for the prior 3 fixes. **Re-verified live**: the same repro now returns a clean 400 naming the cause; removing the reference first lets the country delete cleanly (200), confirming no false positive. **Bug 2 — a smaller, adjacent API-contract bug found while reading the surrounding code**: `PUT /api/port-locations/:unlocode`'s response object used the raw, possibly-blank `countryCode` from the request body instead of `cc` (the same derive-from-unlocode-prefix fallback value the `UPDATE` statement directly above it actually persisted) — a name-only update with no `countryCode` in the body showed `countryCode:''` in the immediate response while the DB row was correctly saved with the real derived code. **Verified live**: PUT with `name` only → response showed `countryCode:""`, an immediate follow-up `GET` on the same port showed the correct derived value. Low real-world impact — the only frontend consumer (`MdmPortLocationsPage.jsx`) discards the PUT response and always refetches — but a genuine, one-line-fix API-contract bug, so fixed anyway; also aligned a minor adjacent inconsistency (this route's own derivation skipped `.trim()` before `.toUpperCase()`, unlike `POST`'s identical derivation). **Re-verified live**: the response now correctly shows the derived code for a blank input, and an explicit `" nl "` input still correctly trims+uppercases to `"NL"`. New regression assertions added to `tests/mdm-crud.test.js`'s existing countries-CRUD block for Bug 1. `tests/mdm-crud.test.js` (94), `tests/carrier-agents.test.js` (41), `tests/mdm-service-toggle.test.js` (25), `tests/organization.test.js` (64) all re-run green — 224 assertions, zero regressions. Clean `vite build`. Every other route in the file (all `isRemote()` branches, `checkLocationConflict`'s local-only helper scope, cascade behavior on `carrier_agents`'s two child tables) checked and confirmed correct — no further gaps found. |
| 2026-09-05 | Full pass of `routes/edi.js` (511 lines — booking-request payload construction, the Test Tools Message Simulator's 3 simulated-response builders, confirm/cancel/link-bl-document). Already touched twice before (the 2026-09-03 systemic shipment-scope sweep confirmed every `:id`-keyed route here inherits `shipmentScopeParamCheck` correctly; the carrier/Line Agent capability-gap fix added a call into this file) — this pass read the whole file end to end rather than one specific question. | **No new gap — one design choice worth naming, deliberately not changed** | `link-bl-document` correctly cross-checks `doc.shipment_id === shipment.id` and `doc_type === 'BL01'` before linking — no cross-shipment document-linking hole. `confirm`/`cancel` both correctly call `supersedeIfCarrierChanged` before acting, matching `upsertPendingBooking`'s own established guard. The credit-hold check ahead of `booking-request` (Shipper/Consignee/Principal only) doesn't also check an assigned NVOCC party's own credit status, even though the NVOCC becomes the payload's `shipperName` when one exists — flagged as a real scope question, not fixed here: an NVOCC is typically the forwarder's own commercial relationship with the carrier, not necessarily gated by the same customer credit-hold mechanic as the cargo owner, and this codebase's own credit-control feature (Epic `TKT-6XFJQM`) never scoped NVOCC into its party coverage in the first place — widening it here would be a scope decision for that epic, not an incidental audit fix. `booking-request`'s payload construction ends with `...(req.body || {})`, letting any field in the request body override a computed one — checked whether this is reachable through the real UI: the only frontend call site (`ShipmentCarrierBookingDetailsPage.jsx`) calls `sendBookingRequest(shipment.id)` with no second argument at all, so `req.body` is empty in every real invocation; the spread is unused flexibility, not an open door, since the route is already `write`-gated to roles that can freely send a real booking anyway. `tests/carrier-booking.test.js` (171) re-run green, confirming current behavior unchanged. |
| 2026-09-05 | Full pass of `routes/customs-filing.js` (208 lines — AES/EEI + ISF/AMS filing lifecycle, mirrors `edi.js`'s carrier-booking shape closely per its own header comment). Every create/submit/respond/reset route checked for the same classes of gap already found elsewhere this session: shipment-vs-filing id cross-scoping, precondition bypass via direct API call, `mdm_source` reads. | **No gap — clean** | Every mutating route (`submit`/`simulate-response`/`reset`) scopes its `SELECT` by both `id=$1 AND shipment_id=$2`, not filing id alone — a filing id can't be acted on through the wrong shipment's URL even before `shipmentScopeParamCheck`'s own `:id`-based enforcement is considered. The create route's broker-role/priced-cargo/USPPI/Consignee preconditions (each with its own code comment citing the exact client-side check it also now enforces server-side) were verified by direct read to actually gate the insert, not just decorate it. No MDM-extracted table is read anywhere in this file (`shipment_parties`, `container_packages`, `customs_filings`, `edi_messages`, `shipments` only), so there's no `mdm_source` bypass surface here at all. `tests/customs-filing.test.js` (39) re-run green, confirming current behavior unchanged. **This closes out the 4-file batch requested this session** (`eadapter.js`, `mdm.js`, `edi.js`, `customs-filing.js`) alongside the earlier `auth.js`/`office-mail.js` pair — every file identified as under-audited from the original 37-file route inventory has now had at least one dedicated pass. |
| 2026-09-05 | Full pass of the 4 never-audited small admin-registry route files: `routes/charge-codes.js`, `routes/container-types.js`, `routes/duty-rates.js`, `routes/invoice-reason-codes.js`. Each route file's own error handling (or lack of it) was cross-checked against what the schema actually enforces. | **Real, live-confirmed double-charge bug on `charge_code_definitions`, the same class silently present in `invoice_status_reason_codes`. `TKT-NSYX9G`, FIXED same day, per direct authorization.** | Both tables use a synthetic `id` as their real key with `code` as a plain, non-unique `TEXT` column — despite each route's own code visibly assuming `code` WAS unique (`invoice-reason-codes.js` already had a dead `isUniqueViolation` catch with a "Code X already exists" message that could never fire; `charge-codes.js` had no catch at all). **Verified live, concretely**: created two active `per_container_split` `charge_code_definitions` rows sharing a `code` but with different labels — both survive `generateInvoices()`'s own per-container auto-inject dedup (`src/utils/invoiceGenerator.js`), which matches by `chargeCode===def.label` against already-created lines in the run, not by definition identity; same-label duplicates happen to get caught as an emergent side effect, but same-code-different-label ones don't, meaning a real customer's per-container invoice could be silently double-charged for what should be one automated fee. **Fixed**: a real `UNIQUE(code)` constraint added to both tables via a guarded migration (`lib/schema.js`) — any pre-existing duplicate beyond the first (oldest by `created_at`) gets renamed with a `-DUP-` suffix rather than silently deleted (this dev DB had zero duplicates, so the rename path never fired); `routes/charge-codes.js`'s `POST`/`PUT` both wrapped in `try/catch` with `isUniqueViolation`, matching the pattern `invoice-reason-codes.js`'s `POST` already had (its own `PUT` was previously unguarded too, and would have 500'd raw once the constraint existed — fixed alongside). **A genuine, pre-existing orphaned test-fixture row was found and cleaned up in the process**: `charge_code_definitions` had a real row (`code='ZTEST'`, created 2026-08-20) left behind by an earlier interrupted run of `tests/definitions-registries.test.js`, whose own hardcoded `"ZTEST"` literal had silently coexisted with zero collision risk before this fix — the new constraint immediately exposed it as a test failure. Deleted the orphan; hardened the test itself to a randomized code (matching this codebase's own established `Math.random().toString(36)` convention) so this can't recur. **Re-verified live**: a duplicate `code` now correctly rejected on both `POST` and a rename-via-`PUT`, for both tables; a genuinely distinct code still creates/updates cleanly. New regression assertions added to `tests/definitions-registries.test.js` and `tests/invoice-collections.test.js`. `duty-rates.js` was also read in full and confirmed already correctly `PRIMARY KEY`'d by its own natural `hs_chapter` code — no gap. `container-types.js` has the identical missing-uniqueness shape, but its own header comment confirms (and a grep confirmed) nothing in the app currently reads from that table — no live business-logic path to corrupt, so left as a disclosed, lower-priority style note rather than fixed in this pass. `tests/definitions-registries.test.js` (27), `tests/invoice-collections.test.js` (34), `tests/container-bulk-import.test.js` (21), `tests/landed-cost.test.js` (33) all re-run green — 115 assertions, zero regressions. Clean `vite build`. **This closes the last 4 never-audited route files from the original 37-file inventory** — every route file in the monolith has now had at least one dedicated audit pass. |
| 2026-09-05 | Continuing into the "partially audited" bucket (files only ever checked for one narrow question, never read end to end): `routes/scheduled-reports.js` (recurring emailed reports) and `routes/ais.js` (AIS status + Test Tools simulator injects). | **No gap — both clean** | `scheduled-reports.js`: `buildShipmentsCsvReport()` (called from the sweep, `server.js`) deliberately bypasses shipment-scope filtering entirely (`queryShipmentRows({roles:["admin"]}, {})`) — traced this to intent, not a bug: `office_id` on a schedule is explicitly documented as "whose mail settings should send this report," never a scope filter, and only an admin (already company-wide-visible) can configure a schedule or its free-text recipient list in the first place, so this grants no one anything they couldn't already export and email manually. `ais.js`: both Test Tools simulate routes funnel through the exact same `ingestAisMessage` the live listener uses, no parallel code path; correctly `write`-gated (admin/operator). `tests/scheduled-reports.test.js` (25) and `tests/ais-integration.test.js` (30) re-run green. |
| 2026-09-05 | Continuing into the "partially audited" bucket: `routes/document-templates.js` (previously only checked for one `mdm_source` question) read end to end, and `routes/share.js` (the public tracking/document-share links) re-read fully now that its dependency `lib/shareToken.js` has its own dedicated fix from 2026-09-03. | **No gap — both clean** | `document-templates.js`: the office+carrier duplicate-scope guard (`IS NOT DISTINCT FROM`, handling the nullable-key case correctly) and the `/resolve` cascading fallback (exact → office-only → generic) both work as documented; `PUT` correctly leaves `docType`/`officeId`/`carrierCode` immutable, matching `eadapter.js`'s own established "delete and re-add to change scope" convention. `share.js`: traced `stored_name` generation (`routes/shipment-ops.js`'s upload route) end to end to rule out a path-traversal risk through the public document-share link — `storedName` is `${Date.now()}_${uid()}${ext}`, and `ext` (the only piece derived from user input, via `path.extname(filename)`) can never itself contain a path separator by definition of what `extname` extracts, so no crafted filename can escape `UPLOADS_DIR`. `GET /api/share/document/:token` deliberately isn't covered by `shipmentScopeParamCheck` (its param is `:token`, not `:id`/`shipmentId`) — correct by design, since its security boundary is the HMAC-signed, expiry-checked token payload itself, not session-based scoping; the classified-location lat/lng stripping on the public tracking response was re-confirmed still present. `tests/document-templates.test.js` (16) and `tests/classified-locations.test.js` (32) re-run green. |
| 2026-09-05 | Continuing into the "partially audited" bucket: `routes/finance.js` (already had 2 real historical fixes — office-scope bypass, GP/margin accrual bug — now re-read end to end for anything new) and `routes/opportunities.js` (previously only checked for one downstream-conversion question, now read in full). | **No gap in either** | `finance.js`: `byCarrier`/`byLane`/`byCustomer`/`overall` are deliberately company-wide for any `canViewFinance` caller regardless of office/branch — confirmed via the route's own explicit code comment ("deliberately NOT applied... `byCarrier`/`byLane`/`byCustomer`... stay company-wide exactly as today"), not an oversight; only `byEntity` is branch-scoped, since a branch is this app's one real legal-entity boundary. `opportunities.js`: the 2026-09-03 pass already confirmed this file "checked out clean" for the specific concern that prompted it (it only ever creates a *quote*, never a shipment, so none of the direct-creation route's own safety checks apply) — this fuller read found nothing further: every lifecycle transition checks `status` before acting, `DELETE` correctly blocks a `Converted` opportunity. `tests/multi-entity-accounting.test.js` (29) and `tests/opportunities.test.js` (36) re-run green. |
| 2026-09-05 | Continuing into the "partially audited" bucket: `routes/sanctions.js`, previously only checked for one compliance-staleness question (2026-09-03), read end to end. Its own code comment already disclosed that neither sync-triggering route was role-restricted — is that still true, and how bad would it actually be? | **CRITICAL — confirmed live in the most direct way possible: it actually happened during verification. `TKT-G4DODH`, FIXED same day, per direct authorization.** | None of `POST /api/sanctions/sync`, `/sync-csl`, `/import-csv` had any role gate — only a per-user rate limiter behind the global authenticated-user check, so any `viewer` could trigger a full destructive replace of the sanctions dataset. **While verifying this, a test payload sent as a scratch viewer account — believed to be a safe no-op (garbage CSV expected to yield zero valid entries) — was actually accepted by the parser as 2 real entries.** `import-csv` proceeded exactly as designed: deleted every real OFAC-SDN row, inserted the 2 garbage ones, then re-indexed and re-screened every active shipment and customer against the now-gutted list — a live, if brief, real-data incident, not a theoretical one. **Recovery**: called the genuine `POST /api/sanctions/sync` (fetches the current list from Treasury) — confirmed restored to 19,329 real entries, matching a normal fresh sync; the separate Consolidated Screening List data (6,736 entries) was untouched throughout, since the delete was correctly scoped to `source='OFAC-SDN'` only. **Fixed**: added `requireRole(['admin'])` to all 3 routes, matching the tier this codebase already uses for its other genuinely destructive action (`routes/admin-reset.js`'s Reset Demo Data). **A matching frontend gap found and fixed in the same pass**: `AppSettingsPage.jsx`'s Import/Sync buttons for both OFAC and CSL rendered with no role check at all — any authenticated user saw and could click them; added the same `isAdmin` gate so a non-admin no longer sees a control that would now silently 403. **Re-verified live**: a fresh scratch `occ_bk` account is correctly rejected (403) on all 3 routes; admin still reaches them normally; the read-only `/entries`/`/status` routes are unaffected for any role, as before. New regression assertions added to `tests/multi-list-screening.test.js` proving the role gate — safe to run unconditionally, since a 403 fires before the route handler body and therefore before any real data touch. `tests/admin-reset.test.js` (13), `tests/customer-compliance-screening.test.js` (25), `tests/customer-service-toggle.test.js` (42), `tests/multi-list-screening.test.js` (14), `tests/screening-service-toggle.test.js` (14) all re-run green — 108 assertions, zero regressions. Clean `vite build`. |
| 2026-09-05 | `routes/document-distribution.js` re-read end to end — previously only checked for one narrow question (2026-09-03, the Test Tools webhook-receiver's missing gate, already found and fixed). | **No new gap** | The 2026-09-03 fix (`shipmentWrite` on the receiver's `POST`/`DELETE`, `GET` deliberately left open to any authenticated role) is still correctly in place. Every `:id`-keyed route correctly inherits `shipmentScopeParamCheck`; `send-edi`/`send-webhook` both write an `_ATTEMPTED` audit event before the real outbound call and an `_SENT`/`_FAILED` one after, so a crash mid-call never leaves a silent gap in the document's own history. The webhook-URL SSRF guard lives in the standalone Document Distribution Service itself (`services/document-distribution/lib/webhookSender.js`, already built and out of scope for a monolith route-file pass) — confirmed still exercised correctly from this side via the existing cloud-metadata-URL rejection test. `tests/document-distribution.test.js` (16) and `tests/document-distribution-gaps.test.js` (18) re-run green. |
| 2026-09-05 | `routes/quotes.js` re-read end to end — previously only checked for its own conversion-path gaps (2026-09-03: missing `creditWarning`, an unread `screening` field, both already fixed). | **No new gap — both prior fixes confirmed still correctly in place** | `POST /api/quotes/:id/convert`'s response still carries both `screening` and `creditWarning` exactly as the 2026-09-03 fix left them. Every lifecycle transition (`send`/`accept`/`decline`/`convert`) checks the quote's current `status` before acting and returns a specific 409 naming the actual blocking state; `DELETE` correctly refuses a `Converted` quote. `expireStaleQuotes()`'s startup ordering (waits on `schemaReady` before its first run, unguarded on the hourly interval after) is sound. `tests/quoting-rfq.test.js` (39) re-run green. |
| 2026-09-05 | `routes/reports.js` (507 lines — `gp-by-geo`, `billing-performance`, `invoice-collections`) read end to end for the first time. Do `gp-by-geo`'s region/country/carrier grouping and `billing-performance`'s office/customer/carrier/lane enrichment respect `mdm_source=remote`, matching the pattern already found and fixed 3 times this session? | **Real gap, same class as the 3 prior fixes — dormant/cosmetic today (`mdm_source` is `local`). `TKT-C5ONL3`, FIXED same day, per direct authorization.** | Both routes independently built their own `countryNames`/`countryLane`/`laneNames`/`carrierNames` lookup maps via a bare, unconditional `SELECT` against all 4 MDM-extracted tables — no `isRemote()`/`callMdmService()` branch in either. **Fixed**: extracted one shared `loadMdmNameMaps()` helper used by both routes — remote mode batches all 4 lookups via `Promise.all` against the MDM Service's existing bulk endpoints (`/internal/countries?limit=300` — 208 real countries fit in one page —, `/internal/country-trade-lanes`, `/internal/trade-lanes`, `/internal/carriers`); local mode unchanged. **A real bug in the fix itself, caught during live verification, not shipped**: `/internal/countries` with no `ids=` filter returns the paginated `{results,total,...}` envelope, not a bare array — the first version crashed both routes in remote mode with a raw 500 (`TypeError: (countries || []).forEach is not a function`) until corrected to read `.results`. **Re-verified live, definitively, after the real fix landed**: flipped `mdm_source` to `remote`, renamed `MAEU` to a deliberately distinct string via the write proxy, confirmed both `gp-by-geo` (grouped by carrier) and `billing-performance` immediately reflected the exact renamed string; restored the name and flipped back to `local`, confirmed the local carrier record was never touched throughout (genuinely independent datastores). `tests/reports.test.js` (30), `tests/billing-performance.test.js` (82), `tests/mdm-service-toggle.test.js` (25) all re-run green — 137 assertions, zero regressions. Clean `vite build`. `invoice-collections` was also read in full and confirmed to read no MDM-extracted table at all — no gap there. |
| 2026-09-05 | `routes/export.js` (659 lines — CSV/XLSX dashboard exports) re-read end to end, given it already carried 2 real historical fixes (the GP/margin `COST_LINE_EFFECTIVE_USD_SQL` bug, its own duplicated `resolveSeaPorts` `mdm_source` bypass). Does it have the same country/carrier/lane-name gap just fixed in `reports.js`? | **No new gap** | Grepped every `FROM {carriers,countries,country_trade_lanes,trade_lanes,port_locations,vessels,linked_ports,regions,commodities}` in the file — the only hit is `resolveSeaPorts`'s own `port_locations` lookup, already correctly `isRemote()`-branched since 2026-09-03. Unlike `reports.js`, this file's CSV/XLSX output only ever shows raw codes (`carrierCode`, `pol`, `pod`) — no resolved carrier/country/lane display name anywhere — so there's no equivalent gap to have. Both export routes (`shipments.csv`, `dashboard/xlsx`) correctly pass the real `req.user` into `queryShipmentRows`, which applies `applyShipmentAccessFilter` before returning — confirmed scoped, not a company-wide leak. `tests/export.test.js` (34) and `tests/export-fields.test.js` (19) re-run green. |
| 2026-09-05 | `routes/contracts.js` (696 lines) read end to end for the first time — previously only checked for one narrow question (2026-09-03, the `PUT` auto-revive logic). | **No gap — thoroughly guarded throughout** | Every mutating route already has real, specific validation this pass tried to poke holes in and couldn't: `publish` checks rate/leg/routing-orphan/validity-window preconditions before allowing `Draft→Active`; `withdraw`/`DELETE` both correctly guard against a still-referenced contract (`shipments.contract_id`/`allocations.contract_id`, no FK possible on either — same "referenced, deactivate instead" pattern `offices.js` already established); the remote-mode branches replicate those same reference guards locally before ever calling the Contract Management Service, since that service owns no shipments/allocations data of its own. `saveLegs`/`saveRates`/`saveContractContainerTypes`/`saveContractImdgClasses` all correctly wrap their delete-then-reinsert in a transaction. `GET /api/entity-events/:type/:id` is correctly covered by `shipmentScopeParamCheck` when `type==='shipment'` (its param is literally named `:id`) and correctly a no-op for every other entity type. `tests/contract-improvements.test.js` (39) and `tests/contract-service-toggle.test.js` (20) re-run green. |
| 2026-09-05 | Full pass of `routes/customers.js` (811 lines) — the 2026-09-03 fix that added `customerWrite` to the screening-override route left its own code comment flagging that a full-file audit was still pending ("This file had NO role-gated route at all before this pass... scoped here to just the screening-override route... not a full-file audit"). This is that pass. | **CRITICAL — confirmed live. `TKT-G68CPB`, FIXED same day, per direct authorization.** | 11 mutating routes had only `auth()` — any authenticated role, `viewer` included — with no role gate at all: `POST`/`PUT`/`DELETE /api/customers(/:id)`, the full identifiers CRUD (`POST`/`PUT`/`DELETE`), the full contacts CRUD (`POST`/`PUT`/`DELETE`), and documents `POST`/`DELETE`. **Verified live, definitively**: a scratch `viewer` account successfully `POST`ed a brand-new customer with `creditLimit:999999999` — 201, a real record created with a $1B credit limit and zero authorization check. **Confirmed NOT affected, by direct read rather than assumption**: `POST /api/customers/:id/credit-hold/release` and `POST /api/shipments/:id/credit-override/approve` both already call `userOwnsLaneForShipment`/`ForCustomer` internally, correctly enforcing the Credit Control Depth epic's own "only the shipment's own lane trade_manager" rule regardless of the outer `auth()`-only gate. **Fixed**: extended the file's own existing `customerWrite` tier (`requireRole(["admin","operator","trade_manager"])`, already defined for the screening-override route) to all 11 previously-ungated routes — confirmed this exactly matches the frontend's own `canManageConfigs` gate (`App.jsx`), so the fix closes a direct-API-only bypass with zero regression for any real UI workflow. **Re-verified live**: a fresh scratch viewer is now correctly rejected (403) creating a customer; admin still succeeds normally. `tests/customer-compliance-screening.test.js` (25), `tests/customer-contacts-roles.test.js` (31), `tests/customer-credit-control.test.js` (83), `tests/customer-hierarchy.test.js` (17), `tests/customer-service-toggle.test.js` (42), `tests/customers-gaps.test.js` (38), `tests/nvocc-carrier-identity.test.js` (21), `tests/landed-cost.test.js` (33), `tests/shipment-edit-lock.test.js` (20) all re-run green — 310 assertions, zero regressions (every test file creates its scratch customers via the admin fixture account already). Clean `vite build`. |
| 2026-09-05 | `routes/shipments.js` (1496 lines) re-read end to end — the same second-order `shipmentScopeParamCheck` bypass class already found and fixed once in `shipment-ops.js` (`TKT-19FU3B`: a route keyed by a sub-resource's own id, not literally `:id`/`:shipmentId`, never triggers the global param guard even though it's a per-shipment sub-resource). Does the pattern recur here, in the file that actually owns containers/parties/side-offices? | **CRITICAL — confirmed live with a real exploit repro, same bug class, much larger surface (12 routes). `TKT-YXKPCS`, FIXED same day, per direct authorization.** | 12 routes were keyed by a bare sub-resource id (`CTR-`/`CEV-`/`PKG-`/`PTY-`/`SOF-` prefixed) with no `:id`/`:shipmentId` param at all: `PUT`/`DELETE /api/containers/:id`, `GET`/`POST /api/containers/:id/events`, `DELETE /api/container-events/:id`, `GET`/`POST /api/containers/:id/packages`, `PUT`/`DELETE /api/container-packages/:id`, `PUT`/`DELETE /api/shipment-parties/:id`, `DELETE /api/shipment-side-offices/:id` — every one skipped `shipmentScopeParamCheck` entirely, so a scoped user (POL/trade-lane/country-restricted) could read or write any OTHER shipment's containers/events/packages/parties/side-offices just by knowing (or enumerating) the sub-resource's own id, with zero regard for whether that shipment was in their scope. **Verified live before fixing**: a scratch `occ_bk` user scoped to `pol=NLRTM` successfully read/wrote sub-resources belonging to a `pol=USNYC` shipment entirely outside their scope, via all 5 route families. **Fixed, matching the `TKT-19FU3B` precedent exactly**: every route nested under `/api/shipments/:shipmentId/...` (`.../containers/:id`, `.../containers/:id/events`, `.../container-events/:id`, `.../containers/:id/packages`, `.../container-packages/:id`, `.../parties/:id`, `.../side-offices/:id`), so each inherits the existing centralized guard automatically instead of a second, parallel enforcement mechanism. Every backing query gained an explicit `AND shipment_id=$2` (or a `JOIN` through the owning table for `container_packages`, which has no direct `shipment_id` column of its own: `JOIN containers c ON c.id=cp.container_id ... AND c.shipment_id=$2`). `src/api.js`'s `containers`/`containerPackages`/`shipmentParties`/`sideOffices` namespaces, all 21 frontend call sites across `App.jsx` and 9 other components/pages, and all 9 affected test files (`container-events`, `container-packages`, `container-bulk-import`, `freight-audit-payment`, `customs-filing`, `landed-cost`, `workflow-improvements`, `shipment-parties`, `customer-compliance-screening`) were migrated to the new nested URL shape. **Re-verified live after the fix, definitively**: the exact same scoped `occ_bk` user + out-of-scope shipment now correctly 404s on all 5 route families (containers, container-events, container-packages, shipment-parties, side-offices); a matching in-scope positive-case check confirmed zero false-positive regression (200, normal edit succeeds). The bare old-shape routes are confirmed fully gone (404 even for admin) — no dead-code fallback left registered. `tests/container-events.test.js` (37), `tests/container-packages.test.js` (46), `tests/container-bulk-import.test.js` (21), `tests/freight-audit-payment.test.js` (27), `tests/customs-filing.test.js` (39), `tests/landed-cost.test.js` (33), `tests/workflow-improvements.test.js` (26), `tests/shipment-parties.test.js` (20), `tests/customer-compliance-screening.test.js` (25) all re-run green — 274 assertions, zero regressions. Clean `vite build`. **Named, not yet investigated**: `GET /api/containers` with no `shipmentId` query param returns every container company-wide with zero scope filtering — noticed in passing, not confirmed as a live bug, flagged as a candidate for a future pass rather than fixed here. |
| 2026-09-06 | Continuing the audit into the last 7 files that had only ever been checked for one narrow question, plus following up on `shipments.js`'s own named-but-unconfirmed candidate above: `GET /api/containers`. Is the missing scope filter actually reachable live, or is it dead surface nobody calls unscoped? | **CRITICAL — confirmed live, and NOT dormant behind a toggle the way most of this session's other gaps were: it fires on every single login for every user. `TKT-TZ5PR9`, FIXED same day, per direct authorization.** | `App.jsx`'s own top-level data-load effect (`~line 482`) calls `api.containers.list()` with no `shipmentId` argument at all, unconditionally, for every logged-in user regardless of role — so this wasn't a hypothetical unscoped call, it's the app's own primary boot sequence. The route applied zero `applyShipmentAccessFilter` in either branch: a bare call returned every container company-wide; `?shipmentId=<id>` (a query param, not a route param, so `shipmentScopeParamCheck`'s `app.param` guard never touches it) returned that shipment's containers directly regardless of scope. **Verified live**: a scratch `viewer` scoped to `pol=NLRTM` correctly saw a company-wide-scoped `GET /api/shipments` (253 shipments, correctly excluding an out-of-scope `pol=USNYC` shipment) — but the exact same user's `GET /api/containers` returned 143 containers including that out-of-scope shipment's own container, cargo description (`"LEAK-TEST-CONFIDENTIAL-CARGO"`) and all; the targeted `?shipmentId=` query returned the same row directly, 200. **Fixed**: matched the already-established pattern from `GET /api/customs-filings` (`routes/customs-filing.js`) — batch-fetch the distinct owning `shipment_id`s from the raw row set, run them through `applyShipmentAccessFilter`, filter containers down to the allowed ids. Zero behavior change for admin/operator (still see everything, matching `applyShipmentAccessFilter`'s own existing bypass). **Re-verified live**: the same scoped user now correctly sees only their 83 in-scope containers (down from the leaked 143), the confidential out-of-scope container no longer appears, and the direct `?shipmentId=` query now correctly returns `[]`; admin re-confirmed unaffected (143 containers, own-shipment query still works, 200 either way). `tests/allocations-crud.test.js` (38), `tests/carrier-booking.test.js` (171), `tests/container-bulk-import.test.js` (21), `tests/container-events.test.js` (37), `tests/container-packages.test.js` (46), `tests/cost-lines-lifecycle.test.js` (39), `tests/customs-filing.test.js` (39), `tests/freight-audit-payment.test.js` (27), `tests/landed-cost.test.js` (33), `tests/merchant-haulage.test.js` (39), `tests/pagination-standardization.test.js` (24), `tests/pickup-service.test.js` (13), `tests/shipment-crud.test.js` (35), `tests/workflow-improvements.test.js` (26) — every test file touching this route — all re-run green, 568 assertions, zero regressions. |
| 2026-09-06 | Full pass of `routes/offices.js` — its `DELETE /api/offices/:id` guard's referencing-table completeness had only ever been checked once, narrowly, on 2026-09-03. Cross-referenced every `office_id`-shaped column across `lib/schema.js` against the route's current guard list (`shipments` emo/imo/controlling, `carrier_eadapter_configs`, `document_templates`, `shipment_side_offices`) rather than trusting it was still exhaustive. | **Real, confirmed gap, same class as the `document_templates`/`shipment_side_offices` fixes already made here. `TKT-PQ89VW`, FIXED same day, per direct authorization.** | Two more `office_id` columns exist with no FK at all and no app-level guard: `shipment_services.office_id` (a Dedicated Service's vendor/office) and `scheduled_reports.office_id` (whose mail settings send a recurring report). **Verified live**: assigned a scratch office to a real `shipment_services` row, deleted the office — succeeded silently (200), the service row was left with a dangling `officeId` and both `officeCode`/`officeName` went blank with zero error anywhere, zero indication anything had gone wrong. `scheduled_reports` has the identical shape: `runScheduledReportsSweep()`'s own `office_mail_settings` lookup (cascaded away with the office) just silently `continue`s forever — a report tied to a deleted office permanently stops sending with no warning surfaced to anyone. **Fixed**: added the same app-level pre-check pattern the other 4 referencing tables in this route already use — query `shipment_services`/`scheduled_reports` for a reference before allowing the delete, return a clean 400 naming the cause. **Re-verified live**: the office delete is now correctly blocked while either reference exists (both tested independently — removing the service still left it blocked by the scheduled report, confirming both guards are real and independent, not one masking the other), and succeeds cleanly (200) once both are removed — no false positive. `tests/billing-performance.test.js` (82), `tests/document-distribution-gaps.test.js` (18), `tests/document-distribution.test.js` (16), `tests/document-templates.test.js` (16), `tests/eadapter.test.js` (47), `tests/invoice-collections.test.js` (34), `tests/multi-entity-accounting.test.js` (29), `tests/office-mail.test.js` (31), `tests/organization.test.js` (64), `tests/scheduled-reports.test.js` (25) all re-run green — 362 assertions, zero regressions. |
| 2026-09-06 | Full pass of `routes/organization.js` — its branch-delete guard's referencing-table completeness had only ever been checked once, narrowly (2026-09-03). Cross-referenced every column referencing `branches(id)` across `lib/schema.js`. | **No gap — confirmed exhaustive** | Only 2 columns reference `branches(id)`: `offices.branch_id` and `org_countries.branch_id`, both `RESTRICT`, both already checked by the existing guard (`offices WHERE branch_id=$1`, `org_countries WHERE branch_id=$1`). Nothing references `org_countries` at all, so `DELETE /api/org-countries/:code` correctly needs no guard either. Every other route in the file (`branches`/`org-countries` CRUD, `offices/:id/stats`) read and confirmed clean — no code change. |
| 2026-09-06 | Full pass of `routes/allocations.js` — two prior narrow questions (a dead-code TEU-formula-duplication check, a `contract_source=remote` fallback question) never covered the whole file. Read every route end to end, cross-checking `DELETE /api/allocations/:id` against the same silent-orphan class just fixed twice above, and every `linked_ports`/MDM-table read against the recurring `mdm_source` bypass class. | **Two real gaps, one of each class already found elsewhere this session. `TKT-V5ELDA` (delete-guard) and `TKT-8IYCMQ` (mdm_source), both FIXED same day, per direct authorization.** | **Gap 1 — silent orphan**: `shipments.allocation_id` has no FK at all, and `DELETE /api/allocations/:id` had zero reference check. **Verified live**: linked a scratch shipment to a scratch allocation, deleted the allocation — succeeded silently (200), the shipment's `allocationId` left permanently dangling; `recomputeSpaceBadge()` gracefully no-ops on a missing allocation (no crash) but the shipment silently and permanently loses its space-consumption tracking with zero indication why. **Fixed**: added the same pre-check-and-400 guard idiom as `offices.js` (`SELECT id FROM shipments WHERE allocation_id=$1 LIMIT 1`) — a shipment can already clear its own `allocationId` via a normal edit, so "unlink it first" is a real, existing path. **Re-verified live**: blocked while referenced, succeeds cleanly once unlinked, no false positive. **Gap 2 — mdm_source bypass**: `GET /api/allocations/conflicts` read `linked_ports` directly in 2 places (its own `isLinked()` helper and its `linkedCodes` computation), instead of the already-`mdm_source`-aware `linkedPortCodes()` helper this same file already imports and uses correctly in the sibling `/match` route just above it — the same recurring bypass class fixed 6+ times already this session, dormant today since `mdm_source` is `local`. **Fixed**: both call sites rewritten to use `linkedPortCodes()`. **Re-verified live in both modes**: a real NLRTM/NLAMS linked-port conflict check produced identical, correct results in local mode and after flipping `mdm_source` to `remote` (a genuine round trip through the MDM Service, not a coincidental match) — restored to `local` afterward. `tests/allocations-crud.test.js` (38), `tests/contract-improvements.test.js` (39), `tests/contract-routings.test.js` (34), `tests/contract-service-toggle.test.js` (20), `tests/contracts-gaps.test.js` (23), `tests/mdm-service-toggle.test.js` (25) all re-run green — 179 assertions, zero regressions. |
| 2026-09-06 | Full pass of `routes/carrier-invoices.js` — its 2 prior narrow questions (the posted-line approve bug and the BUY-side reversal gap, both already fixed) never covered the whole file. Does every route here actually enforce shipment scope, given none of its ids follow the `:id`/`:shipmentId` URL-param convention the systemic 2026-09-03 fix depends on? | **CRITICAL — the most severe finding of the whole audit session: zero shipment-scope enforcement anywhere in the file, full read+write bypass, confirmed live across 6 separate routes. `TKT-Z6C81S`, FIXED same day, per direct authorization.** | Every route operates on a shipment by its own `CINV-`/`CINL-` id or a `shipmentId` in the request body/query, never through a URL `:shipmentId` param — `shipmentScopeParamCheck`'s `app.param` guard only fires on a literal route param, so it never touched this file at all since it was written. **Verified live, comprehensively**: a scoped `occ_bk` user (restricted to one POL) could read every carrier invoice company-wide via `GET /api/carrier-invoices` (list), `GET /api/carrier-invoices/exceptions` (the cross-shipment queue), and `GET /api/carrier-invoices/:id` (direct detail) — all three returned a confidential out-of-scope invoice in full; could **dispute a line** on an out-of-scope shipment's invoice (`POST /api/carrier-invoice-lines/:id/dispute`) — a real write, changing status and recording resolution notes; and could **create a brand-new carrier invoice directly on an out-of-scope shipment** (`POST /api/carrier-invoices`) — 201 Created. Only the admin/operator-only `approve` action was blocked, and only by role, not scope. **Fixed**: a shared `loadScopedShipment(shipmentId, req)` helper (loads the shipment, runs it through `applyShipmentAccessFilter`) is now the single choke point every route goes through — `GET /:id`, `POST` create, `DELETE`, `POST rematch`, `POST approve`, `POST dispute` all resolve to a shipment and 404 if it's out of scope. The two cross-shipment list routes (list, exceptions) now batch-fetch owning shipments and filter through `applyShipmentAccessFilter` before returning, matching the established `GET /api/customs-filings` pattern; the paginated list route filters **before** paginating so both the returned page and the total count are correct for a scoped caller, not just safe (a stricter property than the precedent it's modeled on). **Re-verified live**: every one of the 6 exploit paths above now correctly returns 404 (or "Shipment not found" for create) for the same scoped user against the same out-of-scope shipment; admin access confirmed fully unaffected (unchanged company-wide visibility); the same scoped user's own in-scope invoice/line still reads and disputes correctly (200 both ways) — zero false positive. `tests/cost-lines-lifecycle.test.js` (39), `tests/freight-audit-payment.test.js` (27), `tests/pagination-standardization.test.js` (24) all re-run green — 90 assertions, zero regressions. Clean `vite build`. |
| 2026-09-06 | Direct request: audit the brand-new Rate Reconciliation code (`TKT-NU524E`, same release) for the same gap classes this whole audit has been checking everywhere else — does the new `GET .../cost-lines/reconcile-preview` and the two modified `import-contract`/`update-carrier-costs` routes actually inherit `shipmentScopeParamCheck`, and does `resolveLiveContractRates` degrade safely under `contract_source=remote`? | **No gap — confirmed clean on both questions, verified live rather than assumed from the routing convention** | All 3 routes are registered under `/api/shipments/:id/cost-lines/...` — same `:id` param name the pre-existing sibling routes already used, correctly covered by the global `app.param("id", shipmentScopeParamCheck)` registration. **Verified live**: a scoped `occ_bk` user (restricted to `pol=NLRTM`) correctly got 404 from all 3 routes against a `pol=USNYC` out-of-scope shipment — the preview, and both apply actions. **Verified live** under `contract_source=remote` with a local-only contract unreachable via the Contract Management Service: `reconcile-preview` returned a clean 200 (the existing cost line correctly reported as `'removed'`, since the unreachable contract yields zero live rates) rather than crashing — matching the same "an unreachable remote contract yields no rates, not a hard failure" convention `createRateSnapshot` already established. No code changes needed; both properties were correct by construction (reusing the existing scope-check and rate-resolution machinery rather than reimplementing either). |
| 2026-09-06 | Direct request: audit `ShipmentSchedulesPage.jsx`'s rewrite to a staged-draft model (Save button + validation-gated commit, `navigationGuard.js` wired for auto-save/block-on-invalid, dropping the old "locked schedule leg" concept) plus the same-day live bug fixes it followed (header/routing-pill staleness via a new `legsScheduleBus.js`, `syncShipmentFromLegs` now clearing `pol`/`pod` on the last leg's removal, a multi-leg TSP sailing not cascading its whole-sailing removal). | **3 real findings, all fixed same day; scope/auth properties confirmed clean by construction** | **(1) Real bug, moderate severity — `api.schedules.update` (the "lightweight correction" route, still tested and working at `tests/schedule-catalog.test.js:294-308`, and non-trivial: it re-keys `scheduleKey` and the `sailing_legs`/`schedule_leg_refs` rows) lost its only frontend caller.** The old "Update Schedule" pencil-icon modal (`saveScheduleUpdate`) was deleted along with the "locked leg" concept it existed to work around — but hand-editing a leg's vessel/voyage/ETD/ETA directly (now always possible, no lock) never touched the `shipment_schedules` row at all, silently leaving it stale (Schedule History, `scheduleKey`, and the row's own vessel/dates all frozen at whatever the last real sailing pick set). **Fixed**: `commitDraft()`'s schedule-reconciliation step gained a third branch — if no new sailing was staged and the schedule's own leg still carries vessel/voyage, but a diffed field on that leg actually changed, call `api.schedules.update` with the leg's current vessel/voyage/etd/eta/carrier, keeping the row in sync automatically instead of requiring a dedicated correction UI. **(2) Real bug — `commitDraft()` had no error handling anywhere**, in either of its two callers (`handleSave`, the nav-guard's `trySave`). Since it's a sequence of independent API calls, not one server-side transaction, a failure partway through (e.g. the leg diff succeeds but the schedule save throws) would have been an unhandled promise rejection with zero user-facing toast, while the local draft state kept disagreeing with whatever partially landed on the server. **Fixed**: both call sites now catch, toast the error, and re-sync (`legsVersion`/`historyVersion` bump) from the server so the draft never keeps disagreeing with reality even on failure — a real distributed-transaction rollback is out of scope for this PoC, but silent failure and permanent desync are not acceptable regardless. **(3) Minor race — "Discard" had no `disabled={isSaving}` guard**, unlike "Save," so a rapid Save-then-Discard click could locally revert the UI while the commit was still processing, then get overwritten again once the save's own refresh landed — confusing, not data-corrupting. Fixed with the same guard Save already had. **Confirmed clean, no code change**: every `api.legs.*`/`api.schedules.*` call the new staged-draft commit path uses is a pre-existing, already-scope-checked endpoint (no new backend routes were added), so `shipmentScopeParamCheck` applies automatically; `canEdit={canEdit}` on `<LegsTable>` means a viewer can never produce a dirty draft in the first place, so the Save/Discard bar and its authorization surface are unreachable for a read-only role by construction, not by a separate check. **Verified live via CDP/Puppeteer** (real Central-contract + single-leg and 2-leg TSP fixtures, network-call interception to prove staging vs. commit boundaries): 18 checks across two focused suites plus a dedicated 4-check pass for finding (1) specifically (leg edit → Save → schedule row's `vesselName`/id confirmed updated-in-place, not duplicated or removed) — all passed after the fixes; the one pre-fix failure in the broader suite was independently isolated to a stale toast-text match in the test script itself (the same step's two functional assertions — a real network write, and correct server-side persistence — both passed). `tests/schedules.test.js` gained a new 9-assertion regression test for the `pol`/`pod`-clearing reversal (finding basis for the whole rewrite); full re-run of `schedules.test.js`, `contract-carrier-mismatch.test.js`, `classified-locations.test.js`, `ais-integration.test.js`, `schedule-catalog.test.js`, `command-center.test.js`, `carrier-booking.test.js` — 313 assertions, zero regressions. Three Cypress specs (`schedule-leg-cascade.cy.js`, `schedule-catalog-search.cy.js`, `schedules-happy-path.cy.js`) updated for the new staged/Save flow — not executable in this sandbox (known pre-existing Cypress-binary limitation), so their correctness rests on this review plus the equivalent behavior already proven via the live Puppeteer passes above, not a green CI run. |
