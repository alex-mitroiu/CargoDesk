...
| 2026-09-02 | Freight Audit & Payment (`routes/carrier-invoices.js`) — does approving a carrier-invoice line matched to a cost line that's since been **posted** actually update the shipment's real financial record? | **Real bug, high severity — silent financial-reconciliation loss** | The matching engine (`matchLine()`, line ~116) only ever auto-matches against `status='accrued'` cost lines — so a match is always made while the cost line is still open. But nothing stops the matched cost line from being independently actualized+posted (normal month-end/invoice-to-customer flow) **before** the carrier's own invoice arrives and gets approved — a realistic ordering in real freight operations, where a carrier's invoice routinely arrives weeks after a shipment's own costs are already closed out. The approve route (`POST /api/carrier-invoice-lines/:id/approve`, line ~294) correctly refuses to overwrite a `posted` cost line's `actual_amount` (respects the same lock every other cost-line write path enforces) — but then **silently continues** and marks the invoice line `status:'approved'` anyway, with no adjustment created and nothing in the response indicating the underlying cost line was left untouched. **Verified live end-to-end**: created a scratch shipment, a $1,000 BUY cost line, a carrier invoice line for the real carrier-billed $1,200 (auto-matched to the cost line while still accrued, computed `variancePct: 20`), posted the cost line independently, then approved the invoice line — API returned `status:'approved', varianceUsd:200, variancePct:20` with no error/warning, while a follow-up `GET .../cost-lines` showed the underlying record **unchanged**: `amount:1000, actualAmount:1000, status:'posted'`. The real $1,200 the carrier charged never reaches the shipment's actual cost record or any GP/margin report — the $200 discrepancy is recorded on the invoice line (which has no report of its own) and nowhere else, permanently. Test data cleaned up. **Not fixed this pass** — two plausible fixes (reject with 409 forcing an operator to manually add an adjusting cost line, matching this codebase's own established "posted lines are locked, add an adjusting line" convention elsewhere; or auto-create the adjusting line the same way `PATCH .../actualize`'s sibling routes already do for an unmatched line) are a real design choice, not an obvious one-liner — flagging for a scoping decision before touching it. |
