...
| 2026-09-02 | Milestone auto-completion (`autoCompleteMilestone`, TKT-OZD4V8) — is every one of the 9 milestone steps actually wired to a real trigger, or are some still dormant the way `bl_issued` was before v0.90? | **Real, confirmed gap — `vessel_departed`/`vessel_arrived` are dormant despite AIS already detecting exactly that event** | Grepped every `autoCompleteMilestone(` call site app-wide: `booking_confirmed` (`routes/edi.js`), `si_submitted` (`routes/shipments.js`, VGM), `cargo_gated_in`/`cargo_released` (`routes/shipments.js`, container events), `bl_issued` (`routes/shipment-ops.js`, v0.90), `customs_cleared` (`routes/customs-filing.js`) — 6 of 9 keys covered. `vessel_departed`, `vessel_arrived`, and `delivered` have zero trigger anywhere, including inside `lib/ais-listener.js` itself, which is the one place that already positively confirms a real-world departure/arrival (`shipment_legs.etd_source`/`eta_source` flipping to `'ais'`) — a purpose-built signal for precisely this milestone, sitting unused. Worse: Command Center's own on-time-scorecard and transit-time-variance features (`routes/command-center.js:99-113,180-193`) already **read** `shipment_milestones.vessel_departed`/`vessel_arrived` and compare them against the AIS-confirmed leg dates to compute slippage — meaning those analytics are silently starved unless an operator separately, manually completes the milestone in a way that happens to line up with the AIS date, which nothing prompts them to do. **Verified live**: created a scratch shipment + SEA leg with a real vessel IMO, registered its MMSI via the AIS Simulator, simulated a departure position report — leg's `etdSource` correctly flipped to `'ais'` — but the shipment's `vessel_departed` milestone row stayed `completedAt:"", completedBy:""`. `delivered` has no obvious existing signal to wire to (no proof-of-delivery mechanism exists anywhere in this codebase) — likely a legitimate permanently-manual step, unlike the other two. Not fixed this pass — flagging for the same kind of authorization check the `bl_issued` fix and the cost-line FK fix both got before touching it. |
