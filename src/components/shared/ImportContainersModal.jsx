...
import { IMDG_CLASSES } from "../../tokens";
import { api } from "../../api";
...
const inputStyle = { width: "100%", fontFamily: HZ_BODY, fontSize: 12, background: HZ.bg, color: HZ.text,
  border: `1px solid ${HZ.border}`, borderRadius: 5, padding: "5px 7px", boxSizing: "border-box" };
...
const EditableRow = ({ row, typeOptions, onChange, onRevalidate }) => {
  ...
  return (
    <div data-testid={`shipment-containers-import-row-${row.rowNumber}`} style={{ background: HZ.critBg, border: `1px solid ${HZ.crit}44`, borderRadius: 8,
      padding: "10px 12px", marginBottom: 8 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
        <span style={{ fontFamily: HZ_MONO, fontSize: 11, color: HZ.crit, fontWeight: 700 }}>Row {row.rowNumber}</span>
        <Btn size="sm" variant="secondary" onClick={onRevalidate} data-testid={`shipment-containers-import-row-${row.rowNumber}-recheck-btn`}>Re-check</Btn>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 8, marginBottom: 8 }}>
        {cell("containerNumber", <>
          <div style={{ fontSize: 9.5, color: HZ.textMuted, marginBottom: 2 }}>Container #</div>
          <input style={inputStyle} value={d.containerNumber} onChange={e => set("containerNumber", e.target.value)} />
        </>)}
        {cell("sealNumber", <>
          <div style={{ fontSize: 9.5, color: HZ.textMuted, marginBottom: 2 }}>Seal #</div>
          <input style={inputStyle} value={d.sealNumber} onChange={e => set("sealNumber", e.target.value)} />
        </>)}
        {cell("typeCode", <>
          <div style={{ fontSize: 9.5, color: HZ.textMuted, marginBottom: 2 }}>Type Code *</div>
        ...
        {cell("dgClass", <>
          <div style={{ fontSize: 9.5, color: HZ.textMuted, marginBottom: 2 }}>DG Class</div>
        ...
        {cell("setTemperatureC", <>
          <div style={{ fontSize: 9.5, color: HZ.textMuted, marginBottom: 2 }}>Reefer Temp (°C)</div>
          <input style={inputStyle} value={d.setTemperatureC ?? ""} onChange={e => set("setTemperatureC", e.target.value)} />
        </>)}
      </div>
      <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
        {row.errors.map((e, i) => (
          <div key={i} style={{ fontFamily: HZ_BODY, fontSize: 11.5, color: HZ.crit }}>⚠ {e}</div>
        ))}
      </div>
    </div>
  );
...
const CleanRow = ({ row }) => (
  <div data-testid={`shipment-containers-import-row-${row.rowNumber}`} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 12px",
    borderRadius: 8, background: HZ.bg, border: `1px solid ${HZ.border}`, marginBottom: 6 }}>
    <span style={{ color: HZ.good, flexShrink: 0 }}>✓</span>
    <span style={{ fontFamily: HZ_MONO, fontSize: 10.5, color: HZ.textMuted, flexShrink: 0, width: 44 }}>#{row.rowNumber}</span>
    <span style={{ fontFamily: HZ_MONO, fontSize: 11.5, color: HZ.cyan, fontWeight: 700, flexShrink: 0, width: 110,
      overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
      {row.data.containerNumber || "(no number)"}
    </span>
    <span style={{ fontFamily: HZ_MONO, fontSize: 11, color: HZ.text, flexShrink: 0, width: 50 }}>{row.data.typeCode}</span>
    <span style={{ fontFamily: HZ_BODY, fontSize: 12, color: HZ.textMuted, flex: 1,
      overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
      {row.data.cargoDescription || "—"}
    </span>
    {row.data.isDg && (
      <span style={{ fontFamily: HZ_MONO, fontSize: 9.5, fontWeight: 700, color: HZ.crit,
        background: HZ.critBg, border: `1px solid ${HZ.crit}44`, borderRadius: 4, padding: "1px 6px", flexShrink: 0 }}>
        DG {row.data.dgClass}
      </span>
    )}
  </div>
...
const ImportContainersModal = ({ shipment, onClose, onImported }) => {
  ...
  return (
    <Modal title="Import Containers" onClose={onClose} width={step === "review" ? 900 : 480} data-testid="shipment-containers-import-modal">
      {step === "upload" ? (
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <div style={{ fontFamily: HZ_BODY, fontSize: 13, color: HZ.textMuted, lineHeight: 1.6 }}>
          ...
          <Btn variant="secondary" onClick={handleDownloadTemplate} data-testid="shipment-containers-import-download-template-btn">⬇ Download Template</Btn>
          <div data-testid="shipment-containers-import-file-field">
            <div style={{ fontFamily: HZ_BODY, fontSize: 11, color: HZ.textMuted, marginBottom: 6 }}>Filled-in template (.xlsx)</div>
            <input type="file" accept=".xlsx" onChange={e => setFile(e.target.files?.[0] || null)}
              data-testid="shipment-containers-import-file-input"
              style={{ ...inputStyle, cursor: "pointer" }} />
          </div>
          <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", borderTop: `1px solid ${HZ.border}`, paddingTop: 14 }}>
            <Btn variant="secondary" onClick={onClose} disabled={parsing} data-testid="shipment-containers-import-cancel-btn">Cancel</Btn>
            <Btn onClick={handleParse} disabled={parsing || !file} data-testid="shipment-containers-import-parse-btn">{parsing ? "Reading…" : "Parse File"}</Btn>
          </div>
        </div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div style={{ fontFamily: HZ_BODY, fontSize: 13, color: HZ.text }}>
              <b style={{ color: readyCount === rows.length ? HZ.good : HZ.text }}>{readyCount}</b> of {rows.length} row{rows.length === 1 ? "" : "s"} ready
              {errorCount > 0 && <span style={{ color: HZ.crit }}> · {errorCount} need{errorCount === 1 ? "s" : ""} attention</span>}
            </div>
            <Btn size="sm" variant="secondary" onClick={() => setStep("upload")} data-testid="shipment-containers-import-back-btn">← Upload a different file</Btn>
          </div>

          <div data-testid="shipment-containers-import-rows-list" style={{ maxHeight: 420, overflowY: "auto", paddingRight: 4 }}>
          ...
          <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", borderTop: `1px solid ${HZ.border}`, paddingTop: 14 }}>
            <Btn variant="secondary" onClick={onClose} disabled={committing} data-testid="shipment-containers-import-review-cancel-btn">Cancel</Btn>
            <Btn onClick={handleCommit} disabled={committing || errorCount > 0} data-testid="shipment-containers-import-commit-btn">
