...
const LoadingServicePage = ({ shipment, containers = [], side, serviceType = "Loading", canEdit, onViewDocuments }) => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setLines(freshLines);
      // Trucker (Epic TKT-5XFCAP) — fetched unconditionally; buildLoadingPlanHtml only
      // surfaces it for Pickup/Delivery plans (planLabel-gated), so this is a no-op for
      // Loading/Unloading.
      ...
      const html = buildLoadingPlanHtml({
        ...
        containers: shipmentContainers, loadingPlanLines: freshLines, parties,
      });
      const filename = `${docCode}-${invNumber}-${invDate}.pdf`;
      await api.documents.generate(shipment.id, { html, filename, docType: docCode });
