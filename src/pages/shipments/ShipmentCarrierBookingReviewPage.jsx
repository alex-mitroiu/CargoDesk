...
const ShipmentCarrierBookingReviewPage = ({ shipment, onBack, onRefresh }) => {
  ...
  const load = useCallback(() => {
    ...
    return Promise.all([
      ...
      api.documents.list(shipment.id).then(docs => docs.filter(d => d.docType === "BL01")).catch(() => []),
    ]).then(([b, m, docs]) => { setBooking(b); setMessages(m); setBlDocs(docs); }).finally(() => setLoading(false));
  }, [shipment.id]);
  ...
  // TKT-LAK8P4 — booking-to-B/L traceability. Only ever acts on the current booking (this
  // page never shows an archived one), documentId null clears the link.
  const doLinkBl = async documentId => {
    setBusy(true);
    try {
      await api.carrierBooking.linkBlDocument(shipment.id, documentId);
      toast.success(documentId ? "B/L document linked" : "B/L document unlinked");
      setLinkModal(false);
      await load();
    } catch (e) {
      toast.error(e.message || "Failed to update the linked B/L document");
    } finally { setBusy(false); }
  };
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
        ...
        {canEdit && (
          <div style={{ display: "flex", gap: 8 }}>
            {booking && (
              <Btn variant="secondary" onClick={() => setLinkModal(true)}>
      ...
      {linkModal && (
        <Modal title="Link B/L Document" onClose={() => !busy && setLinkModal(false)} width={420}>
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
              <Btn variant="secondary" onClick={() => setLinkModal(false)} disabled={busy}>Close</Btn>
              {booking?.blDocumentId && (
                <Btn variant="danger" onClick={() => doLinkBl(null)} disabled={busy}>Unlink</Btn>
              )}
            </div>
          </div>
        </Modal>
