...
const STATUS_COLOR = { Draft: "", Submitted: "accent", Confirmed: "success", Rejected: "danger" };
    ...
    <div id="shpsi-page" style={{ maxWidth: 900, margin: "0 auto", display: "flex", flexDirection: "column", gap: 18 }}>
      ...
      <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 10, padding: 18 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4 }}>
          ...
          <span style={{ fontFamily: T.mono, fontSize: 10, fontWeight: 700,
            color: STATUS_COLOR[status] ? T[STATUS_COLOR[status]] : T.textMuted, textTransform: "uppercase" }}>
            {status}
          </span>
        </div>
      ...
      {isDraft && (
        <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 10, padding: 18,
          ...
          <div style={{ display: "grid", gridTemplateColumns: "220px 1fr", gap: 14, alignItems: "start" }}>
            <div>
              <div style={{ fontFamily: T.body, fontSize: 11, color: T.textMuted, marginBottom: 4 }}>SI Cutoff</div>
              <DatePicker id="shpsi-cutoff" value={cutoff} disabled={!canEdit} onChange={setCutoff} placeholder="Not set" withTime />
            </div>
            <div>
              <div style={{ fontFamily: T.body, fontSize: 11, color: T.textMuted, marginBottom: 4 }}>Special Instructions</div>
          ...
          {incompleteContainers.length > 0 && (
            <div style={{ display: "flex", gap: 8, alignItems: "flex-start", padding: "8px 10px",
              borderRadius: 6, background: `${T.warning}14`, border: `1px solid ${T.warning}33` }}>
      ...
      {!isDraft && (
        <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 10, padding: 18,
          ...
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            <span style={{ fontFamily: T.body, fontSize: 11, color: T.textMuted }}>SI Reference</span>
            <span style={{ fontFamily: T.mono, fontSize: 11, color: T.text }}>{si.siReference || "—"}</span>
          </div>
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            <span style={{ fontFamily: T.body, fontSize: 11, color: T.textMuted }}>Submitted</span>
            <span style={{ fontFamily: T.body, fontSize: 11, color: T.text }}>
              {si.submittedAt ? new Date(si.submittedAt).toLocaleString() : "—"}
            </span>
          </div>
          {si.siCutoff && (
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              <span style={{ fontFamily: T.body, fontSize: 11, color: T.textMuted }}>SI Cutoff</span>
              <span style={{ fontFamily: T.body, fontSize: 11, color: T.text }}>{new Date(si.siCutoff).toLocaleString()}</span>
            </div>
          )}
          {status === "Rejected" && (
            <div>
              <div style={{ fontFamily: T.body, fontSize: 11, color: T.textMuted, marginBottom: 3 }}>Rejection Reason</div>
              <div style={{ fontFamily: T.body, fontSize: 12, color: T.danger }}>{si.rejectionReason}</div>
            </div>
          )}
      ...
      <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 10, overflow: "hidden" }}>
        ...
        {shipmentContainers.length === 0 ? (
          <div style={{ padding: "20px 0", textAlign: "center", fontFamily: T.body, fontSize: 12.5, color: T.textMuted }}>
              ...
              <tbody>
                {shipmentContainers.map(c => (
                  <tr key={c.id} style={{ borderBottom: `1px solid ${T.border}` }}>
                    <td style={{ padding: "8px 10px", fontFamily: T.mono, fontWeight: 700, color: T.accent, fontSize: 12.5 }}>
                      {c.containerNumber || "TBC"}
                    </td>
                    <td style={{ padding: "8px 10px", fontFamily: T.mono, fontSize: 12, color: c.sealNumber ? T.text : T.warning }}>{c.sealNumber || "—"}</td>
                    <td style={{ padding: "8px 10px", fontFamily: T.body, fontSize: 12, color: T.textMuted }}>{c.size}ft {c.type}</td>
                    <td style={{ padding: "8px 10px", fontFamily: T.body, fontSize: 12, color: c.marksAndNumbers ? T.text : T.warning }}>{c.marksAndNumbers || "—"}</td>
                    <td style={{ padding: "8px 10px", fontFamily: T.body, fontSize: 12, color: c.cargoDescription ? T.text : T.warning }}>{c.cargoDescription || "—"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
