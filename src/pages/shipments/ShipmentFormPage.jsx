...
export const LegsTable = ({ shipmentId, draftLegs, onDraftLegsChange, onLegsChange, inheritedCarrier, inheritedContractType, inheritedContractRef, loopCode = "", canEdit, showContractCols = true, extraAction = null, lockedSeaLegs = false, onUpdateSchedule = null }) => {
  ...
  useEffect(() => {
    if (isDraft) {
      ...
      setLoading(true);
      api.legs.list(shipmentId).then(result => { setLegs(result); onLegsChange?.(result); })
        .catch(() => {}).finally(() => setLoading(false));
    }
    // draftLegs is intentionally in the deps (isDraft path only): edits made outside this
    // table — Apply Sailing, the bulk carrier-code update, etc. — replace the draftLegs
    // array with a new reference in the parent, and this table has no other way to notice.
    // Safe from feedback loops: this table's own edits (propagate()) pass the very same
    // array reference to both setLegs and onDraftLegsChange, so the reference that flows
    // back down here afterward is unchanged and this effect does not re-fire for them.
  }, [shipmentId, draftLegs]); // eslint-disable-line react-hooks/exhaustive-deps
        ...
        <div style={{ minWidth: totalCols }}>
          ...
          {loading ? (
            <div style={{ padding: "22px 16px", display: "flex", justifyContent: "center" }}>
              <Spinner size="sm" label="Loading legs…" />
            </div>
          ) : legs.length === 0 ? (
