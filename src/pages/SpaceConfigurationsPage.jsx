    ...
    <div>
      ...
      {port?.unlocode ? (
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          ...
          {loadLnk && <span style={{ fontFamily: T.body, fontSize: 11, color: T.border }}>Checking linked locations…</span>}
          {!loadLnk && links.length > 0 && (
            <div style={{ display: "flex", flexWrap: "wrap", gap: 5, alignItems: "center" }}>
              <span style={{ fontFamily: T.body, fontSize: 10.5, color: T.textMuted, flexShrink: 0 }}>Covers linked:</span>
              {links.map(lp => (
                <button key={lp.unlocode} type="button" title={lp.name || lp.unlocode}
                  onClick={() => onSelect({ unlocode: lp.unlocode, name: lp.name || "" })}
                  style={{ fontFamily: T.mono, fontSize: 10.5, fontWeight: 600, color: T.textMuted,
                    background: T.bg, border: `1px solid ${T.border}`, borderRadius: 4,
                    padding: "2px 8px", cursor: "pointer", transition: "border-color .1s, color .1s" }}
                  onMouseEnter={e => { e.currentTarget.style.borderColor = T.accent; e.currentTarget.style.color = T.accent; }}
                  onMouseLeave={e => { e.currentTarget.style.borderColor = T.border; e.currentTarget.style.color = T.textMuted; }}>
                  {lp.unlocode}
                </button>
              ))}
              <span style={{ fontFamily: T.body, fontSize: 10, color: T.border, fontStyle: "italic" }}>— click to switch</span>
            </div>
          )}
        </div>
      ) : (
          ...
          {matches.map(c => (
            <div key={c.id} onClick={() => onSelect(c)}
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
                ...
                <span style={{ fontFamily: T.body, fontSize: 11, color: T.textMuted }}>{c.validFrom} – {c.validTo}</span>
              </div>
              {c.carrierCode && (
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", paddingTop: 4 }}>
        ...
        <Btn onClick={handleSave} disabled={!valid}>{isEdit ? "Save Changes" : "Add Configuration"}</Btn>
      </div>
    </div>
  );
};
