...
module.exports = function customsFilingRoutes(app, ctx) {
  ...
  async function stalenessOf(filing, shipment) {
    ...
    const currentSnapshot = JSON.stringify(await cargoSnapshotFor(shipment.id));
    if ((filing.cargo_snapshot || '[]') !== currentSnapshot) staleFields.push('cargo');
    return { isStale: staleFields.length > 0, staleFields };
  }

  async function mapWithStaleness(filing, shipment) {
    return { ...mapCustomsFiling(filing), ...(await stalenessOf(filing, shipment)) };
  }

  async function insertMessage(shipmentId, filingId, direction, messageType, status, rawPayload, isMock) {
    ...
    await query(`INSERT INTO edi_messages (id, shipment_id, carrier_code, direction, message_type,
      ...
      [id, shipmentId, "", direction, messageType, "JSON", JSON.stringify(rawPayload),
           status, filingId, !!isMock, now, now]);
    const [msg] = await query("SELECT * FROM edi_messages WHERE id=$1", [id]);
    return mapEdiMessage(msg);
  }

  app.get("/api/shipments/:id/customs-filings", auth(), async (req, res) => {
    const rows = await query("SELECT * FROM customs_filings WHERE shipment_id=$1 ORDER BY created_at ASC", [req.params.id]);
    if (rows.length === 0) return ok(res, []);
    const [shipment] = await query("SELECT * FROM shipments WHERE id=$1", [req.params.id]);
    ok(res, await Promise.all(rows.map(r => mapWithStaleness(r, shipment))));
  });
  ...
  app.get("/api/customs-filings", auth(), async (req, res) => {
    ...
    const rows = status
      ? await query("SELECT * FROM customs_filings WHERE status=$1 ORDER BY updated_at DESC", [status])
      : await query("SELECT * FROM customs_filings ORDER BY updated_at DESC");
    if (rows.length === 0) return ok(res, []);
  ...
  app.post("/api/shipments/:id/customs-filings", write, async (req, res) => {
    ...
    try {
      await query(`INSERT INTO customs_filings (id, shipment_id, filing_type, status, created_at, updated_at)
        VALUES ($1,$2,$3,'Draft',$4,$5)`, [id, shipment.id, filingType, now, now]);
    } catch (e) {
    ...
    const [created] = await query("SELECT * FROM customs_filings WHERE id=$1", [id]);
    ok(res, mapCustomsFiling(created), 201);
  });
  ...
  app.post("/api/shipments/:id/customs-filings/:filingId/submit", write, async (req, res) => {
    ...
    // reference number.
    const cargoSnapshot = await cargoSnapshotFor(shipment.id);
    await query(`UPDATE customs_filings SET status='Filed', filing_reference=$1, filed_at=$2, filed_by=$3,
      carrier_code=$4, vessel_name=$5, voyage_number=$6, export_date=$7, cargo_snapshot=$8, updated_at=$9 WHERE id=$10`,
      [filingRef, now, req.user?.name || req.user?.email || "",
           shipment.carrier_code || '', shipment.vessel || '', shipment.voyage || '', shipment.etd || '',
           JSON.stringify(cargoSnapshot), now, filing.id]);
    const sent = await insertMessage(shipment.id, filing.id, "out", "customs_filing_submission", "sent", {
    ...
    const [freshFiling] = await query("SELECT * FROM customs_filings WHERE id=$1", [filing.id]);
    ok(res, { sent, filing: await mapWithStaleness(freshFiling, shipment) }, 201);
  });
  ...
  app.post("/api/shipments/:id/customs-filings/:filingId/simulate-response", write, async (req, res) => {
    ...
    if (outcome === "accepted") {
      ...
      await query(`UPDATE customs_filings SET status='Accepted', confirmation_number=$1, responded_at=$2, updated_at=$3 WHERE id=$4`,
        [confNum, now, now, filing.id]);
      await insertMessage(shipment.id, filing.id, "in", "customs_filing_acceptance", "accepted",
      ...
      if (filing.filing_type === 'ISF_AMS') {
        await autoCompleteMilestone(shipment.id, 'customs_cleared', `ISF/AMS import filing accepted (confirmation ${confNum})`);
      }
    } else {
      const rejReason = reason?.trim() || "Filing rejected — data did not pass regulatory validation.";
      await query(`UPDATE customs_filings SET status='Rejected', rejection_reason=$1, responded_at=$2, updated_at=$3 WHERE id=$4`,
        [rejReason, now, now, filing.id]);
      await insertMessage(shipment.id, filing.id, "in", "customs_filing_rejection", "rejected",
    ...
    const [freshFiling] = await query("SELECT * FROM customs_filings WHERE id=$1", [filing.id]);
    ok(res, { filing: await mapWithStaleness(freshFiling, shipment) }, 201);
  });
  ...
  app.patch("/api/shipments/:id/customs-filings/:filingId/reset", write, async (req, res) => {
    ...
    await query(`UPDATE customs_filings SET status='Draft', filing_reference='', rejection_reason='',
      ...
      updated_at=$1 WHERE id=$2`, [now, filing.id]);
    await logEntityEvent('customs_filing', filing.id, 'RESET_TO_DRAFT', null, 'Rejected', 'Draft');
    const [fresh] = await query("SELECT * FROM customs_filings WHERE id=$1", [filing.id]);
    ok(res, mapCustomsFiling(fresh));
  });
};
