...
| users | Authenticated users: id, email, name, password_hash, role (legacy single), roles (JSON array — `parseUserRoles()`), is_active, last_login, all_offices (global office-visibility bypass), can_view_finance |
| user_scope_items | Per-user, per-role scope restrictions. `item_type`: `trade_lane`/`pol`/`country` (shipment pol/pod/lane matching, `matchesScopeItem()`) or, since v0.91.1 (User Management redesign), `branch_office`/`country_office` — a Branch/Country **office-visibility grant** cascading to every office under it (`resolveEffectiveOfficeIds()`, server.js), dual-path on country (an office's own `country_code` OR its branch's — ~75% of real offices have no `branch_id` set). `excluded` (v0.91.1) carves a grant back down (only ever overrides a cascaded grant, never a direct `user_offices` row). A real server-side `itemType` whitelist now exists on `POST /api/users/:id/scope` (routes/auth.js) — previously any string was silently accepted and silently matched nothing. Grants are role-blind (matches `user_offices`' own role-blind nature) — see `USER-MANAGEMENT-REDESIGN.md` for the full model |
...
- **Shipment scope guard (2026-09-03 audit)**: every route with a `:id`/`:shipmentId` URL param is automatically scope-checked against `applyShipmentAccessFilter` via a global `app.param()` callback (`shipmentScopeParamCheck`, `server.js`, registered right after `applyShipmentAccessFilter`'s own definition) — a leading `SHP-` check makes it a no-op for every other entity's id (offices, contracts, customers, ...), so it's safe as a single app-wide registration despite this codebase having no per-file sub-routers to scope it to instead. Closed a real, confirmed gap where 70+ per-shipment sub-resource routes (cost-lines, documents, milestones, customs-filings, ...) never replicated the same check `GET /api/shipments/:id` always had. **A new route with a `:id`/`:shipmentId` param is covered automatically — nothing to add by hand.** Applies to every HTTP method, not just reads.
...
- **useAuth()**: `import { useAuth } from './AuthContext'` → `{ user, activeRole, canEdit, isAdmin, isViewer, isOccBk, isTradeManager, isSales, activeOffice, userOffices, allOffices }`; `canEdit = effectiveRole !== "viewer"` — use this single boolean to gate all write actions
- **Role hierarchy**: `{ viewer: 0, occ_bk: 1, trade_manager: 1, sales: 1, operator: 2, admin: 3 }` (`ROLE_RANK_SV`/`ROLE_RANK`, server.js/App.jsx) — occ_bk/trade_manager/sales are co-equal, non-hierarchical specialty roles tied at rank 1, not rungs on the admin/operator/viewer ladder; a multi-role user's `X-Active-Role` downgrade is membership-based (`jwtRoles.includes(requestedRole)`), not rank-based — a rank comparison silently fails to treat switching between two tied roles as a real downgrade (2026-09-12 fix, see User Management redesign below)
...
creation/editing that it prompted. 8 real findings across both passes, all fixed same day.
- **User Management redesign** — `user_scope_items` gains `branch_office`/`country_office` item
  types (a Branch/Country office-visibility grant cascading to every office under it, dual-path
  country matching since ~75% of real offices have no `branch_id`) plus an `excluded` flag to
  carve a narrower exception back out. Grants are role-blind and widen which offices a user can
  switch *into* one at a time — the existing single-active-office model is unchanged, not made a
  simultaneous union. `quotes`/`opportunities` gain `office_id` and now run through a new
  `applyOfficeScopedAccessFilter` (`server.js`) — previously neither had ANY access scoping. New
  `sales` role (rank 1, tied with `occ_bk`/`trade_manager`) owns the quoting pipeline with
  deliberately no booking authority. See `USER-MANAGEMENT-REDESIGN.md` for the full model.
- **3 bugs found by this redesign's own pre-ship QA pass**: `requireRole` never respected a
  deliberate `X-Active-Role` downgrade (a `sales`+`occ_bk` user switched to "Sales" could still
  create a real shipment) — fixed with membership-based downgrade logic after a rank-based first
  attempt silently failed for co-equal-rank roles specifically; login/`/api/auth/me` never learned
  about the new grants, so a grant-only user got an empty office picker and either saw everything
  or nothing depending on the route — fixed by merging grant-derived offices into both endpoints;
  the office backfill migration matched almost none of the real dataset (no real account has a
  default office marked) — widened to fall back to a user's earliest assignment.
- **5 bugs found by the shipment creation/editing deep-dive this prompted** (2 pre-existing,
  unrelated to the redesign itself, surfaced by the same pass): the generic
  `PUT /api/shipments/:id` accepted EMO/IMO/Controlling office reassignment with zero
  authorization check, department validation, or audit trail — the ordinary Edit Shipment form
  could silently do what the dedicated `reassign-office` endpoint correctly blocked; now enforces
  the identical checks. The same route was a silent full-replace — omitting any of ~30 fields
  blanked it as though deliberately cleared; now a true partial update. `POST /api/shipments`'s
  create response and quote-conversion's `shipment` field both skipped the shared enrichment other
  shipment responses already had (blank port/office names, zeroed margin/teu) — both now share one
  fragment via `ctx`. Quote-line `quantity` had no validation (`0` silently became `1`, negatives
  flowed into real negative revenue) — now rejected with a clean 400.
- Full 73-file backend suite (2452 assertions) and frontend Vitest suite (10/10) re-run green
  after every fix in both passes; clean `vite build` throughout.
