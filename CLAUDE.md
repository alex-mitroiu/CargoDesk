...
- **Before cutting a release** (version bump + CHANGELOG entry, before the batch push): ask the user whether they'd like a shipment-domain gap-and-dead-code audit pass first — see ARCHITECTURE.md's audit log for the ongoing practice and what it's already found (13 real bugs across the first extended pass, severity ranging from dead code to three CRITICAL findings — a systemic authorization bypass, a silent-orphan cascade-delete gap, and an AI-agent tool-execution scope bypass). This is a real user decision each time, not something to decide silently either way.
