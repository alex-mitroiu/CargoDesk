...
- **Before cutting a release** (version bump + CHANGELOG entry, before the batch push): ask the user whether they'd like a shipment-domain gap-and-dead-code audit pass first — see ARCHITECTURE.md's audit log for the ongoing practice and what it's already found (17 real bugs across the first extended pass, severity ranging from dead code to four CRITICAL findings — a systemic authorization bypass, a silent-orphan cascade-delete gap, an AI-agent tool-execution scope bypass, and a live-reproduced Sankey-diagram crash on the GP Overview/Reports pages). This is a real user decision each time, not something to decide silently either way.
