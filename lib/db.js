...
function acquirePgliteLock(dataDir) {
  ...
  try {
    ...
    // A lock that names OUR OWN pid can only be a leftover: this function runs once, at startup, before
    // this process has taken anything, so it cannot already be the holder. That is not a corner case in a
    // container, where the server is always PID 1 — a hard stop (`docker kill`, an OOM kill, a crash
    // followed by `restart: unless-stopped`, which reuses the same writable layer) leaves a lock that
    // says "1", and the probe below would then find PID 1 alive (itself) and refuse to start, forever.
    let stillAlive = heldByPid !== process.pid;
    if (stillAlive) { try { process.kill(heldByPid, 0); } catch { stillAlive = false; } }
    if (stillAlive) {
      throw new Error(
