...
await query(`
  UPDATE allocations SET contract_id = NULL
  WHERE contract_id IS NOT NULL AND NOT EXISTS (SELECT 1 FROM contracts c WHERE c.id = allocations.contract_id)
`);
...
if (!allocationContractFkExists) {
  await query(`ALTER TABLE allocations ADD CONSTRAINT allocations_contract_id_fkey FOREIGN KEY (contract_id) REFERENCES contracts(id)`);
  console.log("  ✔ allocations.contract_id now protected against a dangling contract reference");
}
