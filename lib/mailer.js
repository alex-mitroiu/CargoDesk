...
async function getTransporterForOffice(query, officeId) {
  if (transporterCache.has(officeId)) return transporterCache.get(officeId);
  const [row] = await query("SELECT * FROM office_mail_settings WHERE office_id = $1", [officeId]);
...
async function sendViaOffice(query, officeId, mailOptions) {
  const transporter = await getTransporterForOffice(query, officeId);
