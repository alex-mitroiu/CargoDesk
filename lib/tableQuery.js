...
function applyColumnFilters(rows, query, columns) {
  ...
  for (const [key, get] of Object.entries(columns)) {
    ...
    out = out.filter(r => values.includes(String(get(r) ?? "")));
  }
  return out;
}
...
function filterOptions(rows, columns, { maxOptions = 2000 } = {}) {
  ...
  for (const [key, get] of Object.entries(columns)) {
    const distinct = [...new Set(
      rows.map(get).filter(v => v !== null && v !== undefined && v !== "").map(String)
...
module.exports = { applyColumnFilters, filterOptions, applySearch, applySort, paginate };
