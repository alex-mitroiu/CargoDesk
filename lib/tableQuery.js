...
function paginate(rows, { limit, offset } = {}, { defaultLimit = 50, maxLimit = 200 } = {}) {
  // Clamped on BOTH ends (2026-09-23 QA finding) — a negative limit used to flow straight into
  // Array.slice's end argument, where a negative end counts back from the array's own end, so
  // e.g. limit=-5 silently returned "every row but the last 5" as if it were a normal page,
  // with `total` bearing no relation to `results.length`. Minimum of 1, same as the array can't
  // ever be usefully zero/negative-length.
  const lim = Math.max(Math.min(parseInt(limit) || defaultLimit, maxLimit), 1);
