...
function createMappers({ portLanesMap, CUTOFF_WARNING_DAYS }) {
  ...
  const mapContainerEvent = r => ({ id: r.id, containerId: r.container_id, shipmentId: r.shipment_id, eventType: r.event_type, location: r.location || '', occurredAt: r.occurred_at, recordedBy: r.recorded_by || '', notes: r.notes || '', conditionNotes: r.condition_notes || '', damageFlag: r.damage_flag === 1, chassisProvider: r.chassis_provider || '', createdAt: r.created_at });
  ...
  const mapScheduledReport = r => ({ id: r.id, reportType: r.report_type, frequency: r.frequency, recipients: r.recipients || '', officeId: r.office_id, officeName: r.office_name || '', isActive: r.is_active === 1, lastRunAt: r.last_run_at || null, createdBy: r.created_by || '', createdAt: r.created_at });
  const mapContainerTypeDefinition = r => ({ id: r.id, code: r.code, size: r.size, type: r.type, teu: r.teu, label: r.label, description: r.description || '', sortOrder: r.sort_order, isActive: r.is_active === 1, createdAt: r.created_at });
  ...
  // mapOfficeMailSettings above; only a hasCredential boolean crosses the wire.
  const mapEadapterConfig = r => ({ id: r.id, carrierCode: r.carrier_code, transportType: r.transport_type,
