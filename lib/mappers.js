...
// Same rule as costLineEffectiveUsd() below, expressed as a raw SQL CASE for any query that
// aggregates shipment_cost_lines across shipments in the database rather than in JS (SQL can't
// call the JS helper directly) — a single shared string so a future report can reference the
// same literal instead of retyping it and risking the two drifting apart. This exact
// amount*exchange_rate-with-no-actual-preference bug turned up independently duplicated in 5
// separate places before this (2026-09-02/03 audit): routes/finance.js, routes/export.js x2,
// routes/reports.js, routes/shipments.js.
...
function createMappers({ portLanesMap, CUTOFF_WARNING_DAYS }) {
  ...
  const mapShipmentLeg = r => ({
    ...
    pol: r.pol || '', pod: r.pod || '',
    polName: r.pol_name || '', podName: r.pod_name || '',
  ...
  // The one number every GP/margin AGGREGATE should sum for a cost line: the actualized/posted
  // amount once one exists (a verified real figure — from a reconciled carrier invoice, a
  // manual correction, whatever actually happened), falling back to the accrual estimate only
  // while a line is still open. Every server-side aggregate report (Dashboard Margin tab,
  // XLSX export, GP-by-Trade-Area) had independently reimplemented amount*exchange_rate with no
  // actual-preference at all, so an actualized/posted line's real cost was silently never
  // reflected in any company-wide GP figure — only the per-shipment GpBreakdownPanel (frontend,
  // via mapCostLine's own actualAmountUsd below) got this right (2026-09-02 audit). Takes a raw
  // shipment_cost_lines row, not the mapped shape, since every call site here works with raw SQL.
  function costLineEffectiveUsd(r) {
    return r.actual_amount != null
      ? roundCents(r.actual_amount * (r.actual_exchange_rate ?? r.exchange_rate))
      : roundCents(r.amount * r.exchange_rate);
  }
  ...
  const mapCostLine     = r => {
    ...
    return {
      ...
      adjustsCostLineId: r.adjusts_cost_line_id || '',
    };
  };
  ...
  const mapContainer    = r => ({ id: r.id, shipmentId: r.shipment_id, containerNumber: r.container_number || '', sealNumber: r.seal_number || '', size: r.size, type: r.type, hsCode: r.hs_code || '', cargoDescription: r.cargo_description || '', marksAndNumbers: r.marks_and_numbers || '', grossWeightKg: r.gross_weight_kg ?? null, volumeCbm: r.volume_cbm ?? null, isDg: !!r.is_dg, dgClass: r.dg_class || '',
    vgmWeightKg: r.vgm_weight_kg ?? null, vgmStatus: r.vgm_status || 'Pending', vgmCutoff: r.vgm_cutoff || '', vgmMethod: r.vgm_method || '', vgmRejectionReason: r.vgm_rejection_reason || '',
  ...
  const mapContainerPackage = r => ({ id: r.id, containerId: r.container_id, parentId: r.parent_id || null, description: r.description, quantity: r.quantity, position: r.position, packTypeId: r.pack_type_id || null, isDg: !!r.is_dg, dgClass: r.dg_class || '', unitValue: r.unit_value ?? null, currency: r.currency || '', hsCode: r.hs_code || '', unitValueUsd: r.unit_value_usd ?? null, createdAt: r.created_at, scheduleBNumber: r.schedule_b_number || '', eccn: r.eccn || '', licenseType: r.license_type || '', licenseValue: r.license_value || '' });
  ...
  const mapHsCode = r => ({ code: r.code, description: r.description, hsChapter: r.hs_chapter,
    chapterName: r.chapter_name || '', isActive: !!r.is_active, createdAt: r.created_at, updatedAt: r.updated_at });
  ...
  const mapDocumentTemplate = r => ({ id: r.id, docType: r.doc_type, officeId: r.office_id || null, officeCode: r.office_code || '', officeName: r.office_name || '', carrierCode: r.carrier_code || null, name: r.name || '', pageSize: r.page_size || 'A4', fields: r.fields ? JSON.parse(r.fields) : [], isActive: !!r.is_active, createdAt: r.created_at, updatedAt: r.updated_at, createdBy: r.created_by || '' });
  const mapAllocation   = r => ({ id: r.id, carrierCode: r.carrier_code, allocatedTEU: r.allocated_teu, effectiveDate: r.effective_date || '', endDate: r.end_date || '', tradeLane: r.trade_lane || '', notes: r.notes || '', alertThreshold: r.alert_threshold ?? 80, pol: r.pol || '', pod: r.pod || '', originLane: r.origin_lane || '', destLane: r.dest_lane || '', contractId: r.contract_id || '', contractNumber: r.contract_number || '', minimumTEU: r.minimum_teu ?? null });
  ...
  const mapScopeItem    = r => ({
    ...
    value:     r.value,
    label:     r.label    || '',
    // Only meaningful for item_type 'branch_office'/'country_office' (User Management redesign,
    // 2026-09-12) — carves an office back out of a broader Branch/Country grant. Always false for
    // trade_lane/pol/country rows.
    excluded:  !!r.excluded,
    createdAt: r.created_at,
  });
  ...
  const mapCarrierBooking = r => ({
    ...
    carrierBookingRequestReference: r.carrier_booking_request_reference || '',
  });
  // Multi-Carrier Integration Framework (TKT-KG4E49) — same "never return the actual secret"
  // rule as mapEadapterConfig above; only hasCredential/hasWebhookSecret booleans cross the wire.
  const mapCarrierIntegration = r => ({
    id: r.id, carrierCode: r.carrier_code, adapterKey: r.adapter_key,
    isActive: !!r.is_active,
    capabilities: (() => { try { return JSON.parse(r.capabilities || '[]'); } catch { return []; } })(),
    baseUrl: r.base_url || '', authHeaderName: r.auth_header_name || '',
    hasCredential: !!r.credential, hasWebhookSecret: !!r.webhook_secret,
  ...
  const mapShippingInstructions = r => ({
    id:                 r.id,
    shipmentId:         r.shipment_id,
    status:             r.status || 'Draft',
    siCutoff:           r.si_cutoff || '',
    specialInstructions: r.special_instructions || '',
    siReference:        r.si_reference || '',
  ...
  const mapLoopCode     = r => ({ id: r.id, code: r.code, name: r.name, carrierCode: r.carrier_code || '',
    frequencyDays: r.frequency_days ?? null, roundTripDays: r.round_trip_days ?? null,
    isActive: !!r.is_active, portCount: r.port_count ?? 0, createdAt: r.created_at, updatedAt: r.updated_at });
  const mapLoopCodePort = r => ({ id: r.id, loopCodeId: r.loop_code_id, portUnlocode: r.port_unlocode,
    portName: r.port_name || '', countryCode: r.country_code || null,
    latitude: r.latitude ?? null, longitude: r.longitude ?? null,
    sequenceOrder: r.sequence_order, transitDayOffset: r.transit_day_offset ?? null,
    direction: r.direction || 'EB' });
  ...
  const mapChargeDefaultSetup = r => ({
    id:             r.id,
    principalId:    r.principal_id   || '',
    principalName:  r.principal_name || '',
    movementType:   r.movement_type  || '',
    locationGlobal: !!r.location_global,
    originRegion:   r.origin_region   || '',
    originCountry:  r.origin_country  || '',
    originLocation: r.origin_location || '',
    destRegion:     r.dest_region     || '',
    destCountry:    r.dest_country    || '',
    destLocation:   r.dest_location   || '',
    isActive:       !!r.is_active,
    createdAt:      r.created_at,
  ...
  const mapQuote = r => ({
    ...
    consigneeId: r.consignee_id || '', consigneeName: r.consignee_name || '',
    principalId: r.principal_id || '', principalName: r.principal_name || '',
    notifyId: r.notify_id || '', notifyName: r.notify_name || '',
    ...
    declaredValue: r.declared_value ?? null, declaredValueCurrency: r.declared_value_currency || 'USD',
    freightTerms: r.freight_terms || 'Prepaid', sourceOpportunityId: r.source_opportunity_id || '',
  ...
  const mapOpportunity = r => ({
    ...
    assigneeId: r.assignee_id || '', officeId: r.office_id || null, notes: r.notes || '',
  ...
  return {
    ...
    mapCarrierAgent, mapCarrierAgentLocation, mapCarrierAgentScheduleRow, mapTradeLane, mapScopeItem, mapOffice, mapOfficeMailSettings,
