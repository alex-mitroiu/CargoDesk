...
async function renderHtmlToPdf(html) {
  ...
  try {
    r = await fetch(`${PDF_RENDER_SERVICE_URL}/internal/render`, {
      ...
      // A real render can take longer than a typical internal call — and, on top of the render
      // itself, a cold browser launch on the pdf-render service's side (spawn + CDP-ready poll)
      // can add real seconds on a loaded/shared machine. The service now warms its browser
      // eagerly at its own boot (see its server.js) so this should rarely be paying that cold-
      // start cost at all; this is defensive headroom on top of that fix, not a replacement for it.
...
async function getActiveSigningCert(query) {
  const [row] = await query("SELECT * FROM org_signing_certs WHERE status = 'active' ORDER BY created_at DESC LIMIT 1");
