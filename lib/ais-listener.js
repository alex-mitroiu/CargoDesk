...
function createAisListener({ query, getSettings, broadcastMessage, logEntityEvent, uid, syncShipmentFromLegs, callMdmService, autoCompleteMilestone, schemaReady }) {
  let ws = null;
  ...
  // Bulk-fills portCoords for a batch of unlocodes — the async counterpart to getPortCoords'
  // synchronous cache-miss-returns-null-and-refills-in-background shape. Safe to await here since
  // refreshTrackedLegs runs off its own timer, never inline in the hot handlePositionReport path.
  // Under node:sqlite this was moot (a 'local'-mode cache miss inside getPortCoords itself was a
  // real synchronous DB read — the coords were simply already there on first use); Postgres made
  // every read async, so a cold cache now returns null for a port's very first sighting unless
  // something warms it ahead of time. This closes that gap for every port a tracked leg actually
  // needs, well before a real position report — or the Test Tools simulator, which force-refreshes
  // tracked legs immediately before injecting one — ever reaches getPortCoords for it.
  async function warmPortCoords(unlocodes) {
    const needed = [...new Set(unlocodes.filter(Boolean))].filter(u => !portCoords.has(u));
    if (!needed.length) return;
    if ((cachedSettings.mdm_source || "local") === "remote") {
      if (!callMdmService) return;
      try {
        const rows = await callMdmService("GET", "/internal/port-coords");
        for (const r of rows) if (!portCoords.has(r.unlocode)) portCoords.set(r.unlocode, (r.latitude && r.longitude) ? { lat: r.latitude, lon: r.longitude } : null);
      } catch (e) { console.error("AIS listener: port-coords warm from MDM Service failed:", e.message); }
      return;
    }
    try {
      const ph = needed.map((_, i) => `$${i + 1}`).join(",");
      const rows = await query(`SELECT unlocode, latitude, longitude FROM port_locations WHERE unlocode IN (${ph})`, needed);
      const found = new Set();
      for (const r of rows) { portCoords.set(r.unlocode, (r.latitude && r.longitude) ? { lat: r.latitude, lon: r.longitude } : null); found.add(r.unlocode); }
      for (const u of needed) if (!found.has(u)) portCoords.set(u, null);
    } catch (e) { console.error("AIS listener: port-coords warm failed:", e.message); }
  }
  ...
  async function refreshTrackedLegs() {
    ...
    await warmPortCoords(rows.flatMap(r => [r.pol, r.pod]));
    trackedLegs = next;
  }
  ...
  async function applyActual(leg, field, eventDate, imo, lat, lon) {
    ...
    // Vessel Departed/Vessel Arrived sat dormant despite this being exactly the real-world signal
    // they represent — no trigger anywhere wired the two together (2026-09-02 shipment-domain
    // audit). autoCompleteMilestone() is itself already a no-op once completed, so this is safe on
    // every AIS confirmation, not just the first.
    await autoCompleteMilestone(leg.shipmentId, field === "etd" ? "vessel_departed" : "vessel_arrived",
      `Auto-completed — AIS confirmed the vessel's ${field === "etd" ? "departure" : "arrival"}`);
  ...
  //
  // createAisListener() itself runs eagerly at server.js module-load time (before schema
  // creation resolves — only restartAisListener/applySettings is gated behind
  // schemaReadyPromise), so the very first refresh must wait on schemaReady too, or it hits a
  // real "relation does not exist" error on every fresh boot. The recurring interval is left
  // unguarded — TRACKED_LEGS_REFRESH_MS (60s) already comfortably outlasts schema creation, and
  // each tick already catches its own failure without crashing anything.
  const readyGate = schemaReady && typeof schemaReady.then === "function" ? schemaReady : Promise.resolve();
  readyGate
    .then(() => refreshTrackedLegs())
    .catch(e => console.error("AIS listener: initial tracked-legs load failed:", e.message));
