...
[![Version](https://img.shields.io/badge/version-0.90.3-blue)](.)
...
|---------|----------|---------|
| 0.86.0 | Slate | **Admin "Reset Demo Data" panel** — wipes all demo/business data back to a clean slate on demand, keeping MDM reference data, sanctions data, and app config intact, so the `.db` file could eventually be committed and a fresh clone starts from a known-good baseline. The natural in-place sibling of Zero-Script Onboarding (v0.79.0) — that one seeds a clean DB only on first boot; this resets an already-running one on demand. 19 tables preserved, computed dynamically as `(live schema) − PRESERVE_TABLES` so a future new table defaults to being wiped, not silently preserved. `users` is wiped then immediately reseeded to the generic default account — not preserved as-is, so a shareable baseline never carries the acting admin's real credentials. New admin-only "Danger Zone" settings tab with a preview of exactly what's preserved/reset and a "type RESET to confirm" gate. The actual destructive wipe+reseed path was deliberately not exercised in the automated suite (same precedent as `POST /api/sanctions/sync*`) — verified instead via a real backup/restore cycle against the live dev database, confirmed byte-for-byte correct. |\n| 0.85.0 | Approach | Four pieces of work, bundled into one unified release. **CRM / pre-sales pipeline** — every named competitor (CargoWise Opportunity Manager, Magaya CRM, Descartes' forwarder-CRM) bundles a lead/opportunity-tracking layer ahead of the shipment lifecycle; new `opportunities` entity precedes and converts into a Quote (New → Qualified → Converted \| Lost), built entirely from the Quoting/RFQ feature as a structural template. **Space Consumption Split** — a space configuration's `consumedTEU` lumped every linked shipment together regardless of carrier-booking status; split into Confirmed (deducts, driven by the operator's own Confirm click)/Pending/Rejected buckets, zero schema migrations. Also ships a real third carrier-response outcome, "Confirmed with Changes," and a new Sent-vs-Received comparison table on the Carrier Booking Review tab. **Consumption Dashboard follow-up** — the page titled "Consumption Dashboard" had never adopted that same Confirmed/Pending/Rejected split; six surfaces fixed to match. **Idle-timeout auto-logout, multi-tab aware** — after 30 minutes of no activity anywhere, the operator is logged out, redirected to login with a "Logged out due to inactivity" notice, and returned to their own last screen on the next sign-in; the 30-minute threshold is hardcoded in a new `config/app-settings.yaml`. Multi-tab awareness reuses a shared `localStorage` activity timestamp and the native `storage` event (broadcasting a logout to every other tab automatically) rather than a new sync protocol. 100+ new assertions across the four pieces, all live-CDP-verified end-to-end. |
| 0.84.0 | Capstone | **Customer/Organization Service extraction (Epic 5)** — the fifth and final planned database-per-domain cut, completing the 5-epic Organization Model roadmap begun at v0.56.0. New standalone `services/customers/` (port 3008) owns customers/identifiers/contacts/screenings. The hardest technical problem: sanctions screening's MATCH decision can never leave the monolith (depends on the in-memory `sanctionsMap`), so only the already-decided result's WRITE branches on the toggle — `screenShipmentById` becoming async rippled through every call site across 6 files. Two real bugs found and fixed along the way: `screenCustomer()` was reading the customer row from the local table even in remote mode (a customer created after cutover would never get screened), and `routes/customers.js` used a helper it never actually imported, crashing the whole server the moment a remote-mode test hit that path. Also surfaced: `npm test`'s chain silently stops at the first failing file, meaning every past "full suite green" claim in this project's history only ever confirmed the first handful of files — a real, disclosed gap in the test runner itself, logged not fixed. 230+ new assertions, every directly relevant existing suite re-verified green. This completes the full five-cut microservices extraction plan. |
| 0.83.0 | Berth | **eAdapter scoped per office, not just per carrier** — a real carrier EDI relationship is negotiated per country/branch, not once globally, and a low-volume office is exactly the one a carrier is least inclined to bother configuring EDI for. `carrier_eadapter_configs` moves to `UNIQUE(carrier_code, office_id)`; `isEdiBookable()` now requires the shipment's own Export Managing Office to match a real configured office. The config modal gained Country/Office pickers (Country narrows Office); found and fixed a real gap in the shared `Sel` primitive along the way (no `disabled` prop support at all). While regression-testing this, found and fixed two unrelated, long-standing test-hygiene bugs that had silently leaked 198 orphaned rows into the dev DB across many prior runs. 47 assertions, full suite green, live-CDP-verified end-to-end. |
| 0.82.0 | Gantry | **Kanban/Testing Service extraction** — the third and final of three planned database-per-domain cuts (MDM v0.80.0, Screening v0.81.0), same local/remote toggle pattern. New standalone `services/kanban/` (port 3007) owns tickets/ticket links/test cases/story links/board projects/versions/columns. Two genuinely new patterns: a shared `resolveAssigneeNames()` batch-resolves assignee names locally since the service owns no `users` table, and the ops-automation sweep's ticket-dedup check gets a real atomicity fix — a new `UNIQUE(source_type, source_id)` constraint the monolith's own schema never had, backing an atomic `POST /internal/tickets/ensure` that closes a narrow concurrent-sweep race outright in remote mode. Story↔TestCase links and the Command Center's ticket alert card needed zero special-casing — both tables move to the same service, and the alert card already goes through the monolith's own proxy. 60 new assertions, full suite green, live-verified with all four extracted services running together. This completes the three-cut extraction plan proposed this session. |
| 0.81.0 | Warden | **Screening Service extraction** — second of three planned database-per-domain cuts, same local/remote toggle pattern as MDM. New standalone `services/screening/` (port 3006) owns sanctions data (OFAC SDN + the US Consolidated Screening List) plus both sync jobs and their auto-sync schedulers. The 5 sanctions routes, which never had their own file, were extracted into `routes/sanctions.js` first. A real pre-existing bug was found and fixed along the way: the in-memory sanctions cache was being reassigned instead of mutated in place, so a route file that had already captured a reference silently never saw a sync that happened after boot. 25 new assertions, full suite green, live-verified with all three extracted services running together. |
| 0.80.0 | Atlas | **MDM Service extraction** — the first of three planned database-per-domain cuts to actually get built (design docs published earlier: `documentation/splitting-mdm-first.html` and two follow-ups), following the pattern `services/contract-management/` proved at v0.68.0. New standalone `services/mdm/` (port 3005) owns carriers/vessels/ports/linked ports/trade lanes/regions/countries/commodities/carrier agents, reached via a new `mdm_source` toggle (`local`/`remote`) — a one-way cutover lever, local tables never removed. Two real cross-cutting risks resolved deliberately: the in-memory port-lane/port-country caches stay caches (never a live per-request fetch) in either mode, and the live AIS listener's vessel-write/port-coords-read paths degrade gracefully (fire-and-forget, background refill) rather than ever blocking on the network. New committed `services/mdm/mdm.sample.db` seed and a chunked, idempotent migration script. 44 new assertions, full suite green, live-verified end-to-end including proving local/remote are genuinely independent datastores. |
